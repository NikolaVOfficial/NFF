...
turf/Stuffwtf
	...
	RedCarpet
	icon = 'Icons/Carpet.dmi'
	icon_state = "1"

	XiverseThrone
	icon = 'Icons/ThroneA.dmi'
	icon_state = "1"

	TorchNew
	icon = 'Icons/Torch.dmi'//'Icons/Torch.dmi'
	icon_state = "1"
	density = 1
		...
		Enter(A)
			if(ismob(A))
				...
				if(usr.GateIn!=""||usr.Pill!="")
					usr<<"Your body is under too much strain to rest right now!"
					return 0
				if(!M.client)
					M.loc=src
					return
				if(M.Clone)
					M.Death(M)
					del(M)
//NEWGOODBYE				usr<<"You Sit down and relax."
				M.loc=src
				var/time=0
				while(M.loc==src&&usr.client&&time<4&&M.loc==src)
					time++
					if(M.loc!=src)
						return
					sleep(10)
			...
			Enter(A)
				if(ismob(A))
					...
					if(M.Clone)
						...
						del(M)
					M.dir=SOUTH
//NEWGOODBYE					usr<<"You lay down and relax."
...
turf/Dojos/LeafDojo
	ToLeafDojo
		...
		/*		var/BreakingIn=0
				...
				var/AlertVillage=0
				var/Invader=""
			//	if((M.TaijutsuKnowledge+M.GenjutsuKnowledge+M.NinjutsuKnowledge<1600||M.rank=="Hokage"||M.key=="CobraT1337"||M.key=="Ishuri")&&(M.rank=="Genin"||M.rank=="Chuunin"||M.rank=="S.Jounin"||M.rank=="Hokage")&&M.Village=="Leaf")
				if(((M.rank=="Genin"&&M.Village=="Leaf")||M.rank=="Hokage"||M.key=="CobraT1337"||M.key=="Ishuri")||LeafOnlyGeninDojo==0)
...
turf/ShopPortals/KageHouse
	ToKagehouse
		...
		Enter(A)
			if(ismob(A))
				var/mob/M = A
				M.loc=locate(67,132,11)
			else if(isobj(A))
				var/obj/B=A
				B.loc=locate(67,131,11)
			else
				..()
	Outkagehouse
		density=0
	...
	LavaSound
		Entered(A)
			if(ismob(A))
				...
				if(M.client)
					spawn()
						M.client.sound_system.PlaySound('LavaFlow.wav')
	...
	WaterFallSound
		Entered(A)
			if(ismob(A))
				...
				if(M.client)
					spawn()
						M.client.sound_system.PlaySound('Waterfall2.wav')
	...
	WaterFallLightSound
		Entered(A)
			...
			//		M<<sound('waterfall.wav',1,0,8,65)
					spawn()
						M.client.sound_system.PlaySound('waterfall.wav')
		Exited(A)
			if(ismob(A))
				var/mob/M=A
				if(M.client)
	...
	NearBeach
		Entered(A)
			if(ismob(A))
				...
				if(M.client)
					spawn()
						M.client.sound_system.PlaySound('Beach.wav')
		Exited(A)
			if(ismob(A))
				var/mob/M=A
				if(M.client)
