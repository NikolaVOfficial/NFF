		...
		Enter(A)
			if(ismob(A))
				...
				if(!M.client)
					...
					if(usr.HunterNinMember||usr.rank=="Amekoutei")
						if(usr.Yonkami)
							usr<<"nop"
							return 0
	...
	UchihaClanHideOut
		Access
			...
			//					M<<sound('UchihaHideout.wav',1,0,8,90)
								M.client.sound_system.PlayMusic('SFX/UchihaHideout.ogg')
			...
			UchihaHideOutExit
				Enter(A)
					...
					//	M<<sound(null,0,0,8,90)
						if(M.client)
							spawn()
								M.client.sound_system.EndMusic()
		...
		LargeUchihaSymbol
			...
			density=0
		CloudSymbol
			icon = 'Map/Turfs/CloudSymbol.png'
			layer=TURF_LAYER
			density=0
		UchihaSignHideOut1
			icon = 'Scenary.dmi'
...
turf
	Blank
		...
		luminosity = 1
		Enter(A)
			if(istype(A, /mob/Kyomou/AkametsukiTrack))
				var/mob/M=A
				var/mob/O = M.Owner
				O.AkametsukiTrack=0
				O.firing=0
			...
			if(ismob(A))
				...
				if(!M.knockedout)
					...
					if(M.TypeLearning=="Taijutsu"||M.TypeLearning=="Suiton"||M.TypeLearning=="Genjutsu")
						M.exp+=80
						if(M.TypeLearning=="Suiton"||M.TypeLearning=="Taijutsu")
							M.exp+=50
						if(M.Subscriber)
							M.exp+=25
	waterblock1
...
turf
	Sand
		icon = 'Map/Turfs/BeachTurfsNew.dmi'
		icon_state = "Sand"
	Sand1
		icon = 'Map/Turfs/BeachTurfsNew.dmi'
		icon_state = "Sand1"
...
turf
	dirtmountain45
		...
		density=1
		icon='turfs.dmi'
		icon_state="dirtmountain45"
turf
	muddymountain45
		name="Mountain"
		density=1
		icon='turfs.dmi'
		icon_state="muddymountain45"
turf
	muddymountain11
		name="Mountain"
		density=1
		icon='turfs.dmi'
		icon_state="muddymountain11"
turf
	dirtmountain11
		name="Mountain"
		density=1
		icon='turfs.dmi'
		icon_state="dirtmountain11"
turf
	dirtmountain89
		name="Mountain"
		density=1
		icon='turfs.dmi'
		icon_state="dirtmountain89"
turf
	muddymountain45
		name="Mountain"
		density=1
