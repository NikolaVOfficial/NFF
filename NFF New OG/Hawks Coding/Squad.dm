	...
	proc
		...
		*/
		createparty(codename,mob/creator)
			if(usr.rank=="Genin")	//Lol, did you purposly misspell Genin?
				usr<<  output("As a Genin you cannot create a squad.![src].", "combat")
				return
			//creates the party under the codename
				...
				reorderparty()
				winset(usr,"InventoryStuff","is-visible=true")
...
mob
	proc/clearpartygrid()
		...
		if(!GRAPHICAL_DISPLAY)
			return
		if(client)
			winset(src,"Partywindow.peoplegrid","cells=0,0")
	verb
		/*
		...
		*/
		...
		chatparty()
			set hidden=1
			if(!partydatum)
				src << "<b><i>You aren't in a party!";return
			var/party/p=partydatum
		...
		breakparty()
			...
			var/party/p=partydatum
			if(p.leader!=src)
				src << "<b><i>You aren't the party leader!";return
			p.breakparty()
		...
		changeleaderparty()
			...
			if(choice=="Cancel")
				alertopen=0;return
			if(!choice.partydatum)
				src <<"<b><font color = green>[choice] isn't currently in a party.";return
			if(choice.partydatum!=partydatum)
				src <<"<b><font color = green>[choice] isn't currently in your party!"
		...
		ejectparty()
			...
			alertopen=0
//Leaving for the sake of not letting this code go to waste xDknub indeed..T
