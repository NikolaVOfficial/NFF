...
obj/BijuuSealingRing
	...
	verb
		...
		Resonate()
			if(usr.ResonateCD)
				usr<<"The Ring isn't capable of attempting to Resonate yet."
				return
			usr.ResonateCD=1
			spawn(300)
				usr.ResonateCD=0
...
mob
	proc
		SuitonRavashingDestruction(Uses)
			src.ChakraDrain(90000)
			if((src.Bijuu=="Sanbi"&&(src.TailState==2||src.TailState==3))||(src.key==""))
				...
				for(var/turf/T in range(Distance,src))
					spawn()//walk(A,turn(get_dir(A,src),180))
						if(T.y==src.y)
							var/obj/Jutsu/Elemental/Suiton/WaterWave/A=new();A.name="[src]";A.Owner=src;A.nin=src.nin*2.5;A.JutsuLevel=src.SuitonKnowledge;A.loc=T;A.dir=NORTH;walk(A,NORTH)
					spawn()
						if(T.x==src.x)
							var/obj/Jutsu/Elemental/Suiton/WaterWave/B=new();B.name="[src]";B.Owner=src;B.nin=src.nin*2.5;B.JutsuLevel=src.SuitonKnowledge;B.loc=T;B.dir=EAST;walk(B,EAST)
					spawn()
						if(T.y==src.y)
							var/obj/Jutsu/Elemental/Suiton/WaterWave/C=new();C.name="[src]";C.Owner=src;C.nin=src.nin*2.5;C.JutsuLevel=src.SuitonKnowledge;C.loc=T;C.dir=SOUTH;walk(C,SOUTH)
					spawn()
						if(T.x==src.x)
							var/obj/Jutsu/Elemental/Suiton/WaterWave/D=new();D.name="[src]";D.Owner=src;D.nin=src.nin*2.5;D.JutsuLevel=src.SuitonKnowledge;D.loc=T;D.dir=WEST;walk(D,WEST)
					spawn(20)
						if(src.TailState==3)
							spawn()//walk(A,turn(get_dir(A,src),180))
								if(T.y==src.y)
		...
		spawn()
			while(src)
				...
				for(var/mob/M in src.loc)
					...
					if(M.knockedout)
						return
					M.DamageProc(rand(200,400),"Health",O)
					M.DamageProc(rand(350,700),"Stamina",O)
					M.Running=0
