...
mob/proc/CheckIfFrozen()
	if(src.FrozenBind!=""||src.Frozen||src.UsingHealthPack||src.resting==1||src.CastingGenjutsu||src.Status=="Asleep"||src.UsingBow||src.Kumosenkyuu||src.Guarding||src.Stun>0||src.knockedout||src.SoundSpinningKick||src.icon_state=="handseal"||src.icon_state=="BeingCarried"||src.icon_state=="Power"||src.alreadytalkingtohim)
...
mob/proc/CheckForAutoRun()
  if(src.Clone||(src.shika&&src.stamina>(src.maxstamina*0.15))||src.GateIn!=""||src.intank||src.ingat||src.Banpaia||src.SageMode!=""||src.EffectsofDrinking||src.HoshiAutoRun||src.ARank=="Infiltrator"||(src.Bijuu=="Nibi"&&src.TailState!=0))
    return 1
  else
    return 0
/////////////////////////////////////////////////
