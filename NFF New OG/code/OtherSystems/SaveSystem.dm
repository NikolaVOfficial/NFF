...
*/
mob/var/StamChakraBuff=0
mob/proc/StaminaandChakraRebuff()
	if(src.Age>=15&&src.StamChakraBuff==0)
		var/Ageinquire=(src.Age-10)
		var/Givethemthis=(Ageinquire*(rand(100,150)))
		src.StamChakraBuff=1
		src.Mchakra+=(Givethemthis)
		src.maxstamina+=(Givethemthis)
		src<<"Your Stamina and Chakra was increased by [Givethemthis] as a change to stats has taken place!"
		return
	else
		src.StamChakraBuff=1
		return
...
//	src.UpdateNinjaInfo()
	...
	//	var/savefile/F = new("Saves/[firstletter]/[src.ckey].sav")
		var/savefile/F = new("Saves/[firstletter]/[src.ckey]/Characters/[savename].sav")
		src.xco = src.x
		src.yco = src.y
		src.zco = src.z
		Write(F)
...
mob
	proc
		LoadPlayer()
			...
			var/regex/R  = new(@".sav\Z")
			var/list/characters = list()
			for(var/a in flist("Saves/[firstletter]/[src.ckey]/Characters/"))
				if(R.Find(a))
					characters+="[a]"
			if(!length(characters))
				src<<"You don't have a saved file."
				return
			var/save=input("Select which character you'd like to log in.","Character Loading") as null|anything in characters
			if(isnull(save)) return
			save = copytext(save,1,length(save)-3)
			var/savepath="Saves/[firstletter]/[src.ckey]/Characters/[save].sav"


			if("[src.ckey]/[save]" in DeadPeople)
			//if(src.key in DeadPeople)
				//var savefile/F
				src<<"<font size=3>The reincarnation process will begin now, don't logout or you'll lose this opportunity.</font>"
				src<<sound('SFX/click1.wav',0)
				var/SaveFile="Saves/[firstletter]/[src.ckey]/Characters/[save].sav"
				if(fexists(SaveFile))
					fcopy(SaveFile,"Rebirthed/[firstletter]/[src.ckey]/Characters/[save].sav")
					fdel(SaveFile)
					F=new("Rebirthed/[firstletter]/[src.ckey]/Characters/[save].sav")
				src.remaking=1
				//world << "[src.ckey]/[src.savename]"
				//DeadPeople-="[src.ckey]/[src.savename]"
				//DeadPeople-=src.key
				DeadPeople-="[src.ckey]/[src.savename]"
				src.Reincarnate()
				//src.RebirthRPL(F["rpl"]/2)
				src.SaveK()
				return
			/*if(src.key in DeadPeople)
				src<<"<font size=3>The reincarnation process will begin now, don't logout or you'll lose this opprotunity.</font>"
				src<<sound('SFX/click1.wav',0)
		...
		/*	if(fexists("Saves/[firstletter]/[src.ckey].sav"))
			...
			if(fexists(savepath))
				...
				*/
				...
				src.client.eye = src
				if(src.Village == "Leaf")
					//if(ScrollIsIn == "Sound"||ScrollIsIn == "Rain"||ScrollIsIn == "Rock"||!ScrollIsIn == "Leaf")
					src.DBLEXPCD=0
				if(src.Village == "Sound")
					//if(ScrollIsIn == "Leaf"||ScrollIsIn == "Rain"||ScrollIsIn == "Rock"||!ScrollIsIn == "Sound")
					src.DBLEXPCD=0
				if(src.Village == "Rain")
					//if(ScrollIsIn == "Leaf"||ScrollIsIn == "Sound"||ScrollIsIn == "Rock"||!ScrollIsIn == "Rain")
					src.DBLEXPCD=0
				if(src.Village == "Rock")
					//if(ScrollIsIn == "Leaf"||ScrollIsIn == "Rain"||ScrollIsIn == "Sound"||!ScrollIsIn == "Rock")
					src.DBLEXPCD=0
	...
	//				src.GmRule()
				if(src.GM)//||src.key=="") //Ivel asked for a silent login due to some circumstances
					switch(input(src,"Do you want to announce your arrival?") in list("Yes","No"))
						...
						//		world<<"<font size=72>^=)</font>"
						if("No")
							src<<"No one knows you logged in...woohoo!"
				else
					world<<"<font size=1><font color=red><B>Info: <font color=white>[src.HandleName]([src.key]) has logged in."
					src.CheckPassives()
				//if(src.key in DAList)//searchs for your key in the list
					//src.DAPunish()//and punishes you
		...
		//			src.GiveSellVerbs()
			...
			//	if(src.pafk)
				...
				//src<<"Sound Immersion System: Testing. Crickets, Wolf Howls, and Owl Hoots at Night (More sounds Soon during the day also). Most Sound Volumes altered, New Environmental Sounds such as heavy winds, light winds, and thunder strikes. Holophonic Sounds for combat Testing (Best Used with Headphones)"

				spawn(10)
					if(src.TaijutsuKnowledge > highestTaijutsuKnowledge)
						highestTaijutsuKnowledge = src.TaijutsuKnowledge
					if(src.NinjutsuKnowledge > highestNinjutsuKnowledge)
						highestNinjutsuKnowledge = src.NinjutsuKnowledge
					if(src.GenjutsuKnowledge > highestGenjutsuKnowledge)
						highestGenjutsuKnowledge = src.GenjutsuKnowledge
					if(src.KatonKnowledge > highestKatonKnowledge)
						highestKatonKnowledge = src.KatonKnowledge
					if(src.SuitonKnowledge > highestSuitonKnowledge)
						highestSuitonKnowledge = src.SuitonKnowledge
					if(src.RaitonKnowledge > highestRaitonKnowledge)
						highestRaitonKnowledge = src.RaitonKnowledge
					if(src.RaitonKnowledge > highestRaitonKnowledge)
						highestRaitonKnowledge = src.RaitonKnowledge
					if(src.DotonKnowledge > highestDotonKnowledge)
						highestDotonKnowledge = src.DotonKnowledge
					if(src.FuutonKnowledge > highestFuutonKnowledge)
						highestFuutonKnowledge = src.FuutonKnowledge
					if(src.KunaiMastery > highestKunaiMastery)
						highestKunaiMastery = src.KunaiMastery
					if(src.ShurikenMastery > highestShurikenMastery)
						highestShurikenMastery = src.ShurikenMastery
					src.StaminaandChakraRebuff()
					src.UpdateInv()
	...
	proc
		DeleteChar()
			...
			if(!length(characters))
				...
				src<<sound('SFX/click2.wav',0)
				return
			var/char=input("Select a character to delete","Delete character") as null|anything in characters
			if(isnull(char))
				return
			//if(fexists("Saves/[firstletter]/[src.ckey]/[src.savename].sav"))
			var/sure=alert(src,"Are you sure you want to delete your character?","Confirmation","Yes","No")
			if(sure=="Yes")
				sleep(0)
				src<<sound('SFX/click1.wav',0)
				fdel("Saves/[firstletter]/[src.ckey]/Characters/[char]")
				src << "Character Deleted"
			else
				src << "Canceled deleting '[src.key]/[char]'"
				src<<sound('SFX/click2.wav',0)
/*mob
	proc
		DeleteChar()
