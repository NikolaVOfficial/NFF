...
mob/proc
	GenjutsuSight()
		if(prob((src.GenjutsuSight*10)+5))
			src.client.screen+=new/obj/Effects/XMash
		...
		spawn()
			while(src)
				...
				for(var/mob/M in src.loc)
					if(M.knockedout||M==O||M.InGenjutsu!=""||M.Status=="Asleep"||!M.client)
						continue
					if(M.Clan=="Fuuma")
						if(prob(M.Awareness*5))
							M<<"You shrug off the Genjutsu."
							del(src)
...
*/
					if(src.TypeOfGenjutsu=="Ocean")
						O<<"You cast a mass torrent of water in the surronding area!"
						M.GenjutsuSight()
						var/X=(round(4-(O.ChakraRadiator/2)))
						if(X<3)
							X=3;
					...
					if(src.TypeOfGenjutsu=="BurizadoAOE")
						O<<"You cast Burizado in the surronding area!"
						M.GenjutsuSight()
						var/X=(round(4-(O.ChakraRadiator/2)))
						if(X<2)
							X=2;
					...
					if(src.TypeOfGenjutsu=="KokuangyoAOE")
						O<<"You cast Kokuangyo in the surronding area!"
						M.GenjutsuSight()
						var/X=3
						if(X<2)
							X=2;
	...
	if(WhichGenjutsu=="Ovan")
		...
		while(X>0)
			if(src.InGenjutsu!="Ovan")
				X=0
				usr << output("<font color=red>You feel the temperature increasing.</font>", "Attack")
				src.health-=50
				src.stamina-=DamageS
			X--
			sleep(11)
...
mob/proc/GenjutsuProc(GenjutsuName,GenjutsuAffect,GenjutsuType="Target",ChakraCost=100,GenjutsuCastTime=30,TimeUntilGenjutsuTakesPlace,ChakraDrain,StaminaDrain,Kaiable=0)
	if(src.GENJUTSUPROCING)
		return
	src.GENJUTSUPROCINGTIME = world.time
	var/curr_GENJUTSUPROCINGTIME = src.GENJUTSUPROCINGTIME
	src.GENJUTSUPROCING = 1
	...
	if(src.chakra<ChakraLoss)
		src<<"You don't have enough chakra to utilize this genjutsu!"
		src.GENJUTSUPROCING = 0
		return
	src.chakra-=ChakraLoss
	...
	if(GenjutsuType=="Area")
		...
		while(src.icon_state=="handseal"&&src.CastingGenjutsu)
			...
			if(src.GENJUTSUPROCINGTIME != curr_GENJUTSUPROCINGTIME) break
			sleep(11)
	...
	if(GenjutsuType=="Target")
		...
		if(get_dist(src,M)>(2+G))
			...
			return
		if(M.knockedout)
			M<<"They're knocked out, no need to cast a genjutsu on them."
			src.GENJUTSUPROCING = 0
			return
		if(M.ImmuneToDeath)
			M<<"That person has recently just awoken from  being unconcious making their sense of chakra weak, it is impossible to put him under a genjutsu at this time"
			src.GENJUTSUPROCING = 0
			return
			...
			if(src.CastingGenjutsu)
				...
				if(M.Clan=="Fuuma")
					if(prob(M.Awareness*5+1))
						M<<"You shrug off the Genjutsu!"
						src<<"They didn't feel the effects of your Genjutsu!"
						src.GENJUTSUPROCING = 0
						return
...
*/
		...
		//			src<<"Was not registered as handseal."
		//			src.GenjutsuCanceling()
				src.icon_state="handseal"
				src.CastingGenjutsu=1
				...
				if(M.GenjutsuCounterMode&&(M.GenjutsuKnowledge-src.GenjutsuKnowledge)>=20)
					...
					//			return
							...
							src.stamina-=stamdamage
							if(prob(10))
								src<<"You feel your body weaken as you fall to one knee..."
...
mob/proc/Fear()
	src.nin*=0.25;
	src.tai*=0.25;
	src<<"Your knees begin to tremble as fear consumes you.."
	src.Running=0;
	sleep(200)
...
//	M.FrozenBind="Vortex"
	...
	src.icon_state="handseal"
	M<<"A strange vortex begins to form.."
	if(M.GenjutsuKnowledge>src.GenjutsuKnowledge)
		M<<"You release the Genjutsu.!";src.Frozen=0;M.FrozenBind="";return
...
//	if(M.Age<=10)
//		return
	M<<"You been hit by Kunai for [damage] damage";
...
mob/proc/FearofProjectiles()
	...
	src.icon_state="handseal"
	var/TimeTillCast=max((11-(src.GenjutsuKnowledge/100))*10,2)//Min of 2 seconds always
	src<<"You have [TimeTillCast] seconds before the Genjutsu is casted!"
	spawn(TimeTillCast)
		src<<"The Genjutsu was a success!"
	while(TimeTillCast>0)
		if(M.gencounter)
			if(M.GenjutsuKnowledge>(src.GenjutsuKnowledge/(M.GenjutsuSight+1)))
				src<<"[M] cancelled your Genjutsu!"
				M<<"You release the Genjutsu!"
				return;
		TimeTillCast--;
		sleep(10)//one second delay
