...
mob/var/tmp/SeeChargeDelay=0
mob/var/ArcherMastery=0
mob/var/tmp/ChargingLHP=0
mob/var/tmp/ChargingRHP=0
mob/var/tmp/
	HoldingA=0
	HoldingL=0
...
mob
	var
		tmp
			GVERBLOCK = 0
			WHILEGUARDINGLOCK = 0
			WHILEGUARDINGCHARGINGLOCK = 0

mob/GainedAfterLogIn/verb
	G()
		set instant = 1
		set hidden=1
		if(usr.GVERBLOCK > world.time)
			return
		usr.GVERBLOCK = world.time + 1
		if(usr.UsingStakes)
			usr.UsingStakes=0
		if(usr.ControllingPuppet1)
			for(var/mob/Puppet/Dragon/A in oview(15,src))
				...
				if(A.Owner==usr&&A.AdjustableRoomCollapsation&&A.Carrying==1)
					A.Frozen=1
					view()<<"The Dragon Puppet begins to let off a fuse!"
	...
	//				del(A)
		...
		if(usr.AutoSphere)
			if(usr.sphere)
				...
				return
		if(usr.sphere)
			for(var/obj/Jutsu/Sand/Sphere/AAAC in usr.loc)
				if(AAAC.Owner==usr)
					usr.SandAttack()
		if(usr.Mogu||usr.firing)
			...
			usr.Guarding=1
			if(usr.mangekyou&&usr.SusanooIn!=1&&locate(/obj/SkillCards/Susanoo) in src.LearnedJutsus)
				usr.Susanoo(usr.mangekyouC,"Bones",300,usr.SusanooMastery,usr.chakra)
			...
			if(usr.Charging)
				if(usr.RightHandSheath||usr.LeftHandSheath)
					...
					if(istype(B,(/obj/WEAPONS/BoomStaff)))
						usr.HaveBoStaffEquipped=2
			if(!usr.WHILEGUARDINGLOCK)
				usr.WHILEGUARDINGLOCK = 1
				if(!usr.WHILEGUARDINGCHARGINGLOCK)
					usr.WHILEGUARDINGCHARGINGLOCK = 1
					while(usr.Guarding)
						...
						while(usr.Charging)
							...
							if(usr.HaveBoStaffEquipped==1)
								...
								sleep(100)
					usr.WHILEGUARDINGCHARGINGLOCK = 0
				usr.WHILEGUARDINGLOCK = 0
	...
	GRelease()
				...
				for(var/obj/Jutsu/Elemental/Doton/DorouDomu/Objects in range(10,usr))
					spawn()
						if(Objects.Owner==usr)
							...
							sleep(11)
							del(Objects)
			usr<<"You stop using Iwakazushi and release the Earth."
		...
		if(usr.TimeToAttack==7)
			...
			var/style = "<style>BODY {margin:0;font:arial;background:black;\
				color:white;}</style>"
			sd_Alert(usr, "You guarded!","[src]",,,,0,"400x150",,style)
		...
		if(usr.UsingSpaceTimeRasengan)
			if(usr.knockedout)
				usr << "<font color=#d0bd1a><b>You were knocked out and lost your charge"
				usr.UsingSpaceTimeRasengan = 0
				return
				/*if (!A in oview(10,src))
					src << "<font color=#d0bd1a><b>Your target ran out of range"
					src.SealPlacedOn -=A
					del(I)
					return*/
			for(var/mob/A in view(10))
				if (A in usr.SealPlacedOn)

					if(A.DotonArmor>=1)
						usr << "<font color=#d0bd1a><b>You Break [A]'s Doton armor with your Space Time Ninjutsu"
						A << "<font color=#d0bd1a><b>Your armor has been broken by Space Time Ninjutsu!"
						usr.chakra-=(usr.Mchakra*0.20)
						A.DotonArmor=0
						usr.RasenganD = (usr.RasenganD / 2)


					//usr.ChakraDrain(40000)
					usr.chakra-=(usr.Mchakra*0.10)
					flick("zan",usr)
					WarpEfx(usr.x,usr.y,usr.z)
					usr.loc = locate(A.x,A.y+1,A.z)
					usr.dir=get_dir(usr,A)
					for(var/atom/E in oview(30,src))
						if(E in src.SealPlacedOn)
					...
					usr.AttackVerb()
					WarpEfx(A.x,A.y,A.z)
					WarpEfx(A.x+1,A.y,A.z)
					WarpEfx(A.x-1,A.y,A.z)
					WarpEfx(A.x,A.y+1,A.z)
					WarpEfx(A.x,A.y-1,A.z)
					WarpEfx2(A.x,A.y,A.z)
					WarpEfx2(A.x+1,A.y,A.z)
					WarpEfx2(A.x-1,A.y,A.z)
					WarpEfx2(A.x,A.y+1,A.z)
					WarpEfx2(A.x,A.y-1,A.z)
					//usr.UsingSpaceTimeRasengan=0
				//	usr.overlays-='SageModeAura.dmi'
					spawn(10)
					if(usr.DRasenganCounter==1)
						usr.chakra-=(usr.Mchakra*0.10)
						flick("zan",usr)
						WarpEfx(usr.x,usr.y,usr.z)
						usr.loc = locate(A.x+1,A.y,A.z)
						usr.dir=get_dir(usr,A)
						usr.AttackVerb2()
						WarpEfx(A.x,A.y,A.z)
						WarpEfx(A.x+1,A.y,A.z)
					...
					src << "You were too far away or didnt place your marker on them"
					usr.Rasenganon=0;usr.RasenganType=0; usr.RasenganCharge=0
					usr.RasenganD=0; usr.DRasenganCounter=0
					src.UsingSpaceTimeRasengan = 0
					usr.SealPlacedOn -=A
					//usr.overlays-='SageModeAura.dmi'
					for(var/atom/E in oview(30,src))
						if(E in src.SealPlacedOn)
				...
				//var/image/I=image('Icons/Jutsus/MinatoAura.dmi')
				//	for(I in usr.client.images)
				//		if(I in oview(usr))
					//if(I.loc == A.loc)
					//		del(I)
					var/B='Icons/Jutsus/Rasengan2.dmi'
					var/ED='Icons/Jutsus/Rasengan4.dmi'
					src.overlays-=B
					src.overlays-=ED
					return
mob/var/tmp
...
mob/GainedAfterLogIn/verb
	LSheath()
		...
		if(usr.LeftHandSheath)
			usr<<"You sheath the weapon in your left hand."
			usr.LeftHandSheath=0
		else
			usr<<"You unsheath the weapon in your left hand!"
			...
			if(usr.TimeToAttack==13)
				var/style = "<style>BODY {margin:0;font:arial;background:black;\
				color:white;}</style>"
				sd_Alert(usr, "You unsheathed the weapon!","[src]",,,,0,"400x150",,style)
				usr.TimeToAttack=14
...
mob/GainedAfterLogIn/verb
	RSheath()
		...
		if(usr.RightHandSheath)
			usr<<"You sheath the weapon in your right hand."
			usr.RightHandSheath=0
		else
			usr<<"You unsheath the weapon in your right hand!"
	//		viewers(4)<<sound('SFX/Unsheath.wav',0,volume=35)
			usr.SoundEngine('SFX/Unsheath.wav',5)//Might be causing lag amg
...
mob/GainedAfterLogIn/verb
	A2Down()
		...
		if(usr.UsingStakes)
			usr.UsingStakes=0
		if(usr.Jailed||usr.knockedout||src.LHP>0||usr.LHP>0||usr.ChargingLHP)
			if(usr.ChargingLHP)
				...
				usr.AttackVerb2(0)
				return
			return//src.LHP>0) return
		usr.ChargingAttack="Left"
		usr.ChargingLHP=1
		if(usr.target)
			var/mob/M=usr.target
			if((M.shari&&M.SharinganMastery>500||M.bya&&M.ByakuganMastery>5000)&&!M.SeeChargeDelay)
				M.SeeChargeDelay=1
				spawn(25)
					M.SeeChargeDelay=0
		...
		if(usr.Trait!="Powerful"&&usr.RaiArmor!=2)
			if(usr.GateIn!="View")
				while(usr.LHP<25&&usr.ChargingAttack=="Left"&&usr.ChargingLHP)
					if(usr.runningspeed>0)
						if(usr.key=="DemonicK"&&usr.key=="")
							usr<<"Your charge is at [LHP]."
						usr.LHP+=pick(0,0.5)
...
mob/GainedAfterLogIn/verb
	ADown()
		...
		if(src.KBunshinOn)
			...
			usr=src
		if(usr.Jailed||usr.knockedout||src.RHP>0) return
		usr.ChargingAttack="Right"
		if(usr.UsingStakes)
			usr.UsingStakes=0
		if(usr.target)
			var/mob/M=usr.target
			if((M.shari&&M.SharinganMastery>500||M.bya&&M.ByakuganMastery>2000)&&!M.SeeChargeDelay)
				M.SeeChargeDelay=1
				spawn(25)
					M.SeeChargeDelay=0
		...
		//		usr.invisibility=0
			...
			spawn()
				while(usr.invisibility==99)
					usr.ChakraDrain(5000)
					sleep(15)
		if(src.KBunshinOn)
			usr=src.controlled
		else
			usr=src
		if(usr.knockedout||usr.Jailed) return
...
mob/proc/AttackVerb2(var/Power)
	...
	if(!src.Kumosenkyuu)
		...
		spawn(10-src.TaijutsuMastery)
			src.AttackDelay=0
	if(src.FrozenBind!="")
		if(src.FrozenBind!="Syrup"&&src.FrozenBind!="Spider Web")
			return
...
//Iwazuka Stuff Start
			...
			if(src.WeaponInLeftHand=="Wall"||src.WeaponInLeftHand=="Mine"||src.WeaponInLeftHand=="Spear"||src.WeaponInLeftHand=="ClayKunai"||src.WeaponInLeftHand=="ClaySenbon"||src.WeaponInLeftHand=="Flower"||src.WeaponInLeftHand=="Bird"||src.WeaponInLeftHand=="Spider"||src.WeaponInLeftHand=="Grasshopper")
				if(src.CreationDelayLeft)
					return
				var/HaveClay=0
				for(var/obj/RandomEquipment/Clay/AS)
					HaveClay=1;
					AS.Ammount-=1
					spawn()
						AS.ReCheckAmount()
				if(!HaveClay)
					usr<<"You've ran out of Clay!";return
				...
				if(src.WeaponInLeftHand=="Wall")
					...
					if(src.CSet=="C1")
						src.chakra-=100
					if(src.CSet=="C2")
						src.chakra-=550;A.icon='Icons/Jutsus/IwazukaTechniques2.dmi'
					if(src.CSet=="C4")
						src.chakra-=4000;A.icon='Icons/Jutsus/IwazukaTechniques2.dmi'
				...
				if(src.WeaponInLeftHand=="Mine")
					...
					var/obj/SmokeCloud/S=new()
					if(src.CSet=="C1")
						src.chakra-=250;var/mob/Kibaku/Mine/A=new();A.Owner=src;A.CSet=src.CSet;A.layer=src.layer;A.loc=src.loc;A.density=0;
					if(src.CSet=="C2")
					...
					if(src.CSet=="C1")
						...
						for(var/mob/Kibaku/C1Spider/S in world)
							if(S.Owner==src)
								counter++
						if(counter>=21)
							src<<"You've have enough C1 spiders out. Explode them.";return
						sleep(5)
					if(src.CSet=="C2")
						sleep(30)
					if(src.CSet=="C4")
...
//				TargetArea=oview(2,src)
			if(src.WeaponInLeftHand=="DotonSpear")
				...
				if(src.SageMode=="Toad")
					A=oview(2)
				for(var/mob/M in A)//get_step(src,src.dir))
					if(M.Mogu)
						return
					src.DodgeCalculation(M,"Kenjutsu")
					if(M.Dodging)
					...
					if(M.Clan=="Yotsuki"&&prob((M.DeflectionZ+M.Buffet+M.Teamwork+M.SpeedDice)*3)&&M.RightHandSheath||M.Deflection||M.WeaponMaster>6&&M.LeftHandSheath||M.WeaponMaster>6&&M.RightHandSheath)
						M<<output("Your weapons deflect each other!","Attack");src<<output("Your weapons deflect each other!","Attack");
						for(var/mob/player/C in view())
							if(C.ckey!=null)
								spawn()
									C.client.sound_system.PlaySound('SFX/SwordClash.wav')
					//	viewers()<<sound('SFX/SwordClash.wav',0,volume=40);
						M.Deflection=0;
						src.Deflection=0
						spawn() AttackEfx4(M.x,M.y,M.z,M.dir);AttackEfx4(src.x,src.y,src.z,src.dir)
						if(prob(1)||(src.Clan=="Yotsuki"&&prob(3)))
							M.DeflectionZ+=pick(0.1,0.3);if(M.DeflectionZ>5) M.DeflectionZ=5
						return
					else
						var/Type="Normal"
						...
						for(var/mob/player/C in view())
							if(C.ckey!=null)
								spawn()
									C.client.sound_system.PlaySound('SFX/SwordClash.wav')
					//	viewers()<<sound('SFX/SwordClash.wav',0,volume=40);
						M.Deflection=0;src.Deflection=0
						spawn() AttackEfx4(M.x,M.y,M.z,M.dir);AttackEfx4(src.x,src.y,src.z,src.dir)
						if(prob(1)||(src.Clan=="Yotsuki"&&prob(3)))
							M.DeflectionZ+=pick(0.1,0.3);if(M.DeflectionZ>5) M.DeflectionZ=5
						return
				...
				if(A.WhatDoesItDo=="Throw")
					...
					if(istype(A,(/obj/WEAPONS/Shuriken)))
						src.ShurikenThrow("Left",Power,4)
						if(prob(3))
						...
						if(A.ammount<=0)
							...
							del(A)
						return
					if(istype(A,(/obj/WEAPONS/Shuriken3)))
						src.ShurikenThrow("Left",Power,3)
						if(prob(3))
							src.ShurikenMastery+=pick(0.1,0.2)
						spawn(9-src.Swift)
							src.LeftHandDelay=0
						if(A.ammount<=0)
							...
							del(A)
						return
					if(istype(A,(/obj/WEAPONS/Shuriken8)))
						src.ShurikenThrow("Left",Power,8)
						if(prob(3))
							src.ShurikenMastery+=pick(0.1,0.2)
						spawn(13-src.Swift)
							src.LeftHandDelay=0
				...
				//				return
					...
					//			return
							if(M.knockedout)
								src<<"You're unable to steal their Chakra while they're knocked out!"
								return
							M.chakra-=loal
					...
					if(istype(A,/obj/WEAPONS/OriginalSamehade))
						for(var/mob/M in oview(6))
							...
							if(M.Kaiten)
								M<<"The chakra for the rotation has been drained?"
								M.Kaiten=0
								M<<"The Blade disables your Rotation, even draining you of your chakra?!"
								M.chakra-=500
					for(var/obj/Jutsu/Sand/Sphere/AAAC in get_step(src,src.dir))
						for(var/mob/ZZZ in OnlinePlayers)
							if(AAAC.Owner==ZZZ)
								if(ZZZ.CastingGenjutsu)
									ZZZ.GenjutsuCanceling()
								if(prob(1))
									ZZZ.AutoProtection+=pick(0.1,0.5);if(ZZZ.AutoProtection>20) ZZZ.AutoProtection=20
								if(ZZZ.AutoProtection>=20)
									ZZZ.SandAttack()
						src<<output("The attack was canceled!","Attack");viewers()<<sound('SFX/Guard.wav',0,volume=25);return
	...
	//							M.LeftHandSheath=0
	//							M.RightHandSheath=0
					for(var/obj/AA in get_step(src,src.dir))
						if(AA.icon=='Map/Turfs/log.dmi')
							flick("Hit",AA);
							src<<"You slice the log!"
							for(var/mob/player/C in view())
								if(C.ckey!=null)
									spawn()
										C.client.sound_system.PlaySound('SFX/SwordClash.wav')
					//		viewers()<<sound('SFX/SwordClash.wav',0,volume=35)
	...
	spawn()
		if(src.TimeToAttack==1)
			...
			var/style = "<style>BODY {margin:0;font:arial;background:black;\
				color:white;}</style>"
			sd_Alert(usr, "You impressively punched the air!","[src]",,,,0,"400x150",,style)
	...
	if(src.Kumosenkyuu)
		if(src.HasArrow)
			src<<"You have an arrow made already! Fire it!";src.LeftHandDelay=0;return
		spawn(40-src.Swift)
			src.LeftHandDelay=0
		src<<"You begin to develop the gold within your mouth.";sleep(4);src.icon_state="Jutsu"
...
/////////////////////////////////////
		...
		if(src.KagDance=="Karamatsu")
			...
			for(var/mob/M in oview(src,1))
				if(M.KagDance=="Karamatsu")
					viewers(M)<<"[src]'s bones clash with [M]'s bones and no damage is dealt!"
				else
					spawn() src.Kara(M)
			return
		else
			...
			if(prob(4))//was prob(2)
				if(src.Swift<6)
					...
					spawn(15)
						src.overlays-=I
					if(src.Swift>6) src.Swift=6
			if(!src.Chidorion&&!src.Rasenganon&&!src.RasenShurikenOn&&!src.Raikirion&&usr.ChargedChakraForPunch==0)
				for(var/obj/Jutsu/Sand/Sphere/A in get_step(src,src.dir))
					for(var/mob/ZZZ in OnlinePlayers)
						...
						if(ZZZ.AutoProtection>=20)
							ZZZ.SandAttack()
					src<<output("The attack was canceled!","Attack");
					for(var/mob/player/C in view())
						if(C.ckey!=null)
							spawn()
								C.client.sound_system.PlaySound('SFX/Guard.wav')
			...
			var/B=get_step(src,src.dir)
			if(src.SageMode=="Toad")
				B=oview(2)
			for(var/mob/M in B)//get_step(src,src.dir))
				if(M.Mogu)
					return
				src.DodgeCalculation(M)
				if(M.Dodging)
				...
				if(src.DoingPalms==1)
					...
					return
				else if(src.DoingPalms==5)
					src.HakkeRokujonyonshou(M,Power*1.5)
					return
				...
				if(M.Clan=="Hyuuga")
					if(src.dir==NORTH&&M.dir!=NORTH&&M.dir!=NORTHEAST&&M.dir!=NORTHWEST)
						if(prob(M.Rejection*4))
							...
							if(prob(50)) flick("Icons/Jutsus/HyuugaRejection.dmi",src)
							else flick("Icons/Jutsus/HyuugaRejection2.dmi",src)
							step(src,src.dir);src<<"You've been pushed back!";return
					...
					if(src.dir==WEST&&M.dir!=WEST&&M.dir!=SOUTHWEST&&M.dir!=NORTHWEST)
						if(prob(M.Rejection*4))
							...
							step(src,src.dir);src<<"You've been pushed back!";return
				if(M.KagDance=="Karamatsu")
					src<<output("You try to strike [M] but their bones stop you from attacking, wounding you back!","Attack");M<<output("You block out [src]'s attack, wounding them!","Attack");src.DamageProc(src.maxhealth*0.03,"Health",M);spawn()Blood(src.x,src.y,src.z);return
				if(M.DotonArmor)
					if(M.CastingGenjutsu)
						M.GenjutsuCanceling()
					src<<output("You try to strike [M] but the rocks surrounding their body stop you from harming them!","Attack");M<<output("You block out [src]'s attack, thanks to the rocks surrounding your body.","Attack");return
				if(M.Guarding&&M.AutoProtection>0||M.Guarding&&M.Rejection>=5&&M.ByakuganMastery>1500)
					if(M.AutoProtection==0&&M.chakra>0)
						for(var/mob/player/C in view())
							if(C.ckey!=null)
								spawn()
									C.client.sound_system.PlaySound('SFX/JyukenClash.wav')
					//	viewers()<<sound('SFX/JyukenClash.wav',0,volume=45)
						...
						if(M.chakra>speedcheck/2)
							M<<output("The sand blocks their attack!","Attack");src<<output("The Sand Protects them!","Attack");M.overlays -= 'Icons/Jutsus/Sshushin.dmi';M.overlays += 'Icons/Jutsus/Sshushin.dmi';sleep(9);M.overlays -= 'Icons/Jutsus/Sshushin.dmi';M.chakra-=round(speedcheck/2);return
					...
					if(src.RasenShurikenOn)
						src.RasenShurikenProc(M)
						return
					if(src.scalpel)
						spawn()
							src.ChakraMesu(M)
		...
		for(var/obj/A in get_step(src,src.dir))
			if(istype(A,(/obj/Totem)))
				var/obj/Totem/M=A
				oview(8)<<"[src] hits the Totem!"
				if(M.Effect==0&&M.CoolDown==0)
					oview(8)<<"The Totem begins to glow Mysteriously..."
					sleep(10)
					M.Effect=rand(1,10)
					M.CoolDown=150
				else
					src<<"There is already an Effect occuring in place!"
			if(A.icon=='Map/Turfs/log.dmi')
				flick("Hit",A);src<<"You hit the log!"
				src.stamina-=rand(10,35);spawn()AttackEfx(A.x,A.y,A.z);
				viewers()<<sound('SFX/Guard.wav',0,volume=10)
				if(src.z==39||src.z==32)
					src.stamina-=rand(10,35);spawn()AttackEfx(A.x,A.y,A.z);viewers()<<sound('SFX/Guard.wav',0,volume=10)
					if(prob(8))
			...
			//				src.maxstamina++
					if(src.TypeLearning=="Taijutsu")
						src.exp+=rand(30,50)*src.Combos*1.75
					else
						src.exp+=rand(20,30)*src.Combos
					if(src.Subscriber)
					...
					if(prob(1))
						if(src.Rush<20)
							src.Rush+=0.01;if(src.Rush>20) src.Rush=20
					if(src.TypeLearning=="Taijutsu")
						src.exp+=rand(20,30)*src.Combos*1.75
						if(src.Subscriber)
			...
			if((A.icon=='Map/Turfs/Tree.dmi'||A.icon=='Map/Turfs/NewTree3.dmi')&&A.icon_state=="tree10")
				src<<"You hit the tree!"
				src.stamina-=rand(25,50);spawn()AttackEfx(A.x,A.y,A.z);viewers()<<sound('SFX/Guard.wav',0,volume=15)
				if(prob(0.1))
					if(src.Rush<20)
						src.Rush+=0.01;if(src.Rush>20) src.Rush=20
				if(prob(9))
					if(usr.TaijutsuKnowledge<1000)
						usr.TaijutsuKnowledge+=1
				if(src.TypeLearning=="Taijutsu")
					src.exp+=rand(50,70)*src.Combos*1.75
				return
...
//	world<<"[Power]."
	...
	if(src.FrozenBind!="")
		if(src.FrozenBind!="Spider Web"&&src.FrozenBind!="Syrup")
			return
	if(src.RasenShurikenOn)
		src.RasenShurikenThrowProc()
		return
	if(src.HakuriCube)
		src.HakuriCubeLaunch()
		return
	if(src.GenkaiHakuri)
		src.JintonGenkaiHakuriShoot()
		return
...
//Iwazuka Stuff Start
			if(src.WeaponInRightHand=="Wall"||src.WeaponInRightHand=="Mine"||src.WeaponInRightHand=="Spear"||src.WeaponInRightHand=="ClayKunai"||src.WeaponInRightHand=="ClaySenbon"||src.WeaponInRightHand=="Flower"||src.WeaponInRightHand=="Bird"||src.WeaponInRightHand=="Spider"||src.WeaponInRightHand=="Grasshopper")
				...
				if(src.WeaponInRightHand=="Mine")
					...
					var/obj/SmokeCloud/S=new()
					if(src.CSet=="C1")
						src.chakra-=250;var/mob/Kibaku/Mine/A=new();A.Owner=src;A.CSet=src.CSet;A.layer=src.layer;A.loc=src.loc;S.loc=A.loc;A.density=0;
					if(src.CSet=="C2")
...
//Iwazuka Stuff End
				...
				if(A.WhatDoesItDo=="Throw")
					...
					if(istype(A,(/obj/WEAPONS/Senbon)))
						...
						return
					if(istype(A,(/obj/WEAPONS/Shuriken)))
						src.ShurikenThrow("Right",Power,4)
						if(prob(5))
							src.ShurikenMastery+=pick(0.1,0.2)
						spawn(10-src.Swift)
							src.RightHandDelay=0
						...
						return
					if(istype(A,(/obj/WEAPONS/Shuriken3)))
						src.ShurikenThrow("Right",Power,3)
						if(prob(5))
							src.ShurikenMastery+=pick(0.1,0.2)
						spawn(9-src.Swift)
							src.RightHandDelay=0
						...
						return
					if(istype(A,(/obj/WEAPONS/Shuriken8)))
						src.ShurikenThrow("Right",Power,8)
						if(prob(5))
							src.ShurikenMastery+=pick(0.1,0.2)
						spawn(12-src.Swift)
							src.RightHandDelay=0
				...
				if(A.WhatDoesItDo=="Slash")
					...
					if(istype(A,/obj/WEAPONS/Shibuki)||istype(A,/obj/WEAPONS/BoomStaff))
						if(Power>=15)
							...
							return								//In addition, if it explodes, it doesn't do the rest.
					if(istype(A,/obj/WEAPONS/SamehadeNew))
						for(var/obj/Jutsu/Spider/web/E in oview(1))
							del(E)
						for(var/mob/M in oview(src.SamehadeAbsorbtion))
							var/loal=rand(120,180)
							if(M.name=="Kikaichu"&&!M.client)
		...
		//						return
							...
							if(M.Kaiten)
								...
								M.Kaiten=0
					if(istype(A,/obj/WEAPONS/OriginalSamehade))
						for(var/obj/Jutsu/Spider/web/D in oview(1))
							del(D)
						for(var/mob/M in oview(6))
							var/loal=rand(150.230)
							if(M.name=="Kikaichu"&&!M.client)
							...
							if(M.Kaiten)
								M<<"The chakra for the rotation has been drained?"
								M.Kaiten=0
								M<<"Your rotation has been drained of it's chakra! Along with you!"
								M.chakra-=500
					for(var/obj/Jutsu/Sand/Sphere/AAAC in get_step(src,src.dir))
						for(var/mob/ZZZ in OnlinePlayers)
							if(AAAC.Owner==ZZZ)
							...
							if(ZZZ.AutoProtection>=20)
								ZZZ.SandAttack()
						src<<output("The attack was canceled!","Attack");viewers()<<sound('SFX/Guard.wav',0,volume=15);return
					for(var/mob/Sand/SunaNoTate/AAAC in get_step(src,src.dir))
						for(var/mob/ZZZ in OnlinePlayers)
							if(AAAC.Owner==ZZZ)
								if(ZZZ.CastingGenjutsu)
								...
								if(prob(1))
									ZZZ.AutoProtection+=pick(0.1,0.5);if(ZZZ.AutoProtection>20) ZZZ.AutoProtection=20
						src<<output("The attack was canceled!","Attack");viewers()<<sound('SFX/Guard.wav',0,volume=15);return
					var/SageRange=get_step(src,src.dir)
					if(src.SageMode=="Toad")
						SageRange=oview(2)
					for(var/mob/M in SageRange)//get_step(src,src.dir))
						if(M.Mogu)
							return
						src.DodgeCalculation(M,"Kenjutsu")
						if(M.Dodging || M.InHydro)
				...
				if(A.WhatDoesItDo=="Claws")
					...
					var/SageRange2=get_step(src,src.dir)
					if(src.SageMode=="Toad")
						SageRange2=oview(2)
					for(var/mob/M in SageRange2)//get_step(src,src.dir))
						if(M.Mogu)
							return
						src.DodgeCalculation(M)
						if(M.Dodging || M.InHydro)
				...
				if(A.WhatDoesItDo=="Staff")
					...
					var/SageRange3=get_step(src,src.dir)
					if(src.SageMode=="Toad")
						SageRange3=oview(2)
					for(var/mob/M in SageRange3)//get_step(src,src.dir))
						if(M.Mogu)
							return
						src.DodgeCalculation(M)
						if(M.Dodging || M.InHydro)
	...
	if(src.KagDance=="Karamatsu")
		...
		if(prob(4))
			if(src.Focus<750&&src.Focus>400)
				src.Focus++
			if(src.Swift<6)
				var/image/I = new('Icons/Jutsus/Skillcards.dmi');I.pixel_y=32;I.layer=6;I.icon_state="SwiftYes"
				src.overlays+=I
				spawn(15)
					src.overlays-=I
				src.Swift+=0.01;if(src.Swift>6) src.Swift=6
		if(!src.Chidorion&&!src.Rasenganon&&!src.RasenShurikenOn&&!src.Raikirion&&usr.ChargedChakraForPunch==0&&src.ZankuuSphere==0)
			for(var/obj/Jutsu/Sand/Sphere/A in get_step(src,src.dir))
				for(var/mob/ZZZ in OnlinePlayers)
					if(A.Owner==ZZZ)
						if(ZZZ.CastingGenjutsu)
				...
				for(var/mob/ZZZ in OnlinePlayers)
					if(A.Owner==ZZZ)
						if(prob(1))
							ZZZ.AutoProtection+=pick(0.1,0.5);if(ZZZ.AutoProtection>20) ZZZ.AutoProtection=20
				src<<output("The attack was canceled!","Attack");viewers()<<sound('SFX/Guard.wav',0,volume=15);return
		var/SageRange4=get_step(src,src.dir)
		if(src.SageMode=="Toad")
			SageRange4=oview(2)
		for(var/mob/M in SageRange4)//get_step(src,src.dir))
			if(M.Mogu)
				return
			if(src.DoingPalms==1)
				src.HakkeRokujonyonshou(M,Power)
			...
			if(M.Dodging)
				if(src.ZankuuSphere)
					src.ZankuuSphere=0;src<<"You missed!"
				...
				M.Dodge();return
			if(M.KagDance=="Karamatsu")
				src<<output("You try to strike [M] but their bones stop you from attacking, wounding you back!","Attack");M<<output("You block out [src]'s attack, wounding them!","Attack");src.DamageProc(src.maxhealth*0.03,"Health",M);M.Bloody();return
			if(M.DotonArmor)
				if(M.CastingGenjutsu)
					M.GenjutsuCanceling()
				src<<output("You try to strike [M] but the rocks surrounding their body stop you from harming them!","Attack");M<<output("You block out [src]'s attack, thanks to the rocks surrounding your body.","Attack");return
			if(M.Guarding&&M.AutoProtection>0||M.Guarding&&M.Rejection>=5&&M.ByakuganMastery>1500)
				if(M.AutoProtection==0)
					viewers()<<sound('SFX/JyukenClash.wav',0,volume=15)
					M.dir=get_dir(M,src)
					var/a=pick("Attack1","Attack2")
					flick(a,M)
					spawn() AttackEfx3(M.x,M.y,M.z,M.dir)
	...
	//				src.RasenShurikenThrowProc()
		...
		for(var/obj/A in get_step(src,src.dir))
			...
			if(A.icon=='Map/Turfs/log.dmi')
				flick("Hit",A);src<<"You hit the log!"
				if(src.z==39||src.z==32)
					src.stamina-=rand(10,35);spawn()AttackEfx(A.x,A.y,A.z);viewers()<<sound('SFX/Guard.wav',0,volume=15)
					if(prob(9))
						if(usr.TaijutsuKnowledge<2000)
							usr.TaijutsuKnowledge+=1
				//	if(prob(2))
...
//		viewers()<<sound('SFX/AttackSwish3.wav',0,volume=35)
	...
	if(src.TimeToAttack==3)
		...
		var/style = "<style>BODY {margin:0;font:arial;background:black;\
			color:white;}</style>"
		sd_Alert(src, "You've successfully kicked the air! Wooho?","[src]",,,,0,"400x150",,style)
...
mob/proc/SenbonThrowing(Hand)
	...
	viewers()<<sound('SFX/Throw.wav',0,0,1,20)
	...
	for(var/obj/WEAPONS/Senbon/O in src.contents)
		...
		if(O.ammount<=0)
			...
			O.name="[O.oname]"
			O.suffix= " ([O.ammount])"
mob/proc/ShurikenThrow(Hand,Charge,Sides)
	if(Sides==4)
		...
		if(number<1||number==null)
			number=1
		var/max=round(src.ShurikenMastery/10)
		if(src.ShurikenMastery>60&&src.ShurikenMastery<3000) max=6
		if(countr>max) countr=max
		if(countr>number) countr=number
		if(countr<1) countr=1
		while(countr>0)
			if(prob(1)&&src.ThrowingStrength<5)
				src.ThrowingStrength+=pick(0.1,0.3)
				if(src.ThrowingStrength>5) src.ThrowingStrength=5
			var/obj/WEAPONS/Shuriken3/L=new()
			L.loc=src.loc
			if(prob(45))
				if(prob(50))
					var/d=turn(src.dir,45);L.loc=get_step(src,d)
			...
			if(src.WeaponExploding)
				...
				if(counter<=0)
					...
					L.Exploding=1
					L.icon_state="throwe3"
			if(src.ThrowingStrength>=1&&src.ThrowingStrength<2)
				L.Move_Delay=0.9;L.TileMax=3
			if(src.ThrowingStrength>=2&&src.ThrowingStrength<3)
				L.Move_Delay=0.6;L.TileMax=2
			if(src.ThrowingStrength>=3&&src.ThrowingStrength<4)
				L.Move_Delay=0.3;L.TileMax=2
			if(src.ThrowingStrength>=4&&src.ThrowingStrength<5)
				L.Move_Delay=0.2;L.TileMax=1
			if(src.ThrowingStrength>=5)
				L.Move_Delay=0;L.TileMax=1
			if(prob(10)&&src.WeaponMaster<5&&src.Clan=="Fuuma")
				src.WeaponMaster+=0.1;if(src.WeaponMaster>5) src.WeaponMaster=5
			...
			if(Hand=="Right")
				flick("Attack2",src)
			L.icon_state="3bladedshurikenthrow"

			L.pixel_x=rand(-6,28);L.pixel_y=rand(4,24)
		//	if(Hand=="Right")
		//		L.pixel_x=rand(10,12);L.pixel_y=rand(14,16)
			for(var/obj/WEAPONS/Shuriken3/O in src.contents)
				O.ammount-=1
				if(O.ammount<=0)
					if(src.WeaponInLeftHand==O)
						src.WeaponInLeftHand=""
		...
		if(number<1||number==null)
			number=1
		var/max=round(src.ShurikenMastery/10)
		if(src.ShurikenMastery>30&&src.ShurikenMastery<3000) max=3
		if(countr>max) countr=max
		if(countr>number) countr=number
		if(countr<1) countr=1
		while(countr>0)
			if(prob(1)&&src.ThrowingStrength<5)
				src.ThrowingStrength+=pick(0.1,0.3)
				if(src.ThrowingStrength>5) src.ThrowingStrength=5
			var/obj/WEAPONS/Shuriken8/L=new()
			L.loc=src.loc
			if(prob(45))
				if(prob(50))
					var/d=turn(src.dir,45);L.loc=get_step(src,d)
			...
			if(src.WeaponExploding)
				...
				if(counter<=0)
					...
					L.icon_state="throwe3"
			if(src.ThrowingStrength>=1&&src.ThrowingStrength<2)
				L.Move_Delay=1.4;L.TileMax=3
			if(src.ThrowingStrength>=2&&src.ThrowingStrength<3)
				L.Move_Delay=1.2;L.TileMax=2
			if(src.ThrowingStrength>=3&&src.ThrowingStrength<4)
				L.Move_Delay=1;L.TileMax=2
			if(src.ThrowingStrength>=4&&src.ThrowingStrength<5)
				L.Move_Delay=0.7;L.TileMax=1
			if(src.ThrowingStrength>=5)
				L.Move_Delay=0.3;L.TileMax=1
			if(prob(10)&&src.WeaponMaster<5&&src.Clan=="Fuuma")
				src.WeaponMaster+=0.1;if(src.WeaponMaster>5) src.WeaponMaster=5
			...
			if(Hand=="Right")
				flick("Attack2",src)
			L.icon_state="8bladedshurikenthrow"

			L.pixel_x=rand(-6,28);L.pixel_y=rand(4,24)
		//	if(Hand=="Right")
		//		L.pixel_x=rand(10,12);L.pixel_y=rand(14,16)
			for(var/obj/WEAPONS/Shuriken8/O in src.contents)
				O.ammount-=1
				if(O.ammount<=0)
					if(src.WeaponInLeftHand==O)
						src.WeaponInLeftHand=""
...
//		Damage=Damage*Muscles
		...
		if(src.EyesOfHell==2&&src.ConversusinThirdStage)
			if(Power>=20)
				src<<"Going to get someone to recolor this..."
...
/////////////
		...
		if(M.JC>0)
			...
			return
		if(M.NenkinOn)
			viewers()<<sound('SFX/Guard.wav',0,volume=15);return
		if(M.FrozenBind=="DoBind")
			src<<output("Your fists bounce harmlessly off the rocks...","Attack");M<<output("[src]'s fist bounce harmlessly off the rocks...","Attack")
			return
		if(M.Kaiten)
			src<<output("[M]'s rotation shot you back!","Attack");M<<output("Your rotation shot [src] back!","Attack");step(M,src.dir);sleep(1);step(M,src.dir);src.DamageProc(rand(1,Damage),"Health",M);M.RushLimitz=0;src.Death(M)
			if(M.Buffet>=1)
				M.BuffetGain+=1;if(M.BuffetGain>round(M.Buffet)) M.BuffetGain=round(M.Buffet)
			return
		if(M.ingat)
			src<<output("[M]'s spiral shot you back!","Attack");M<<output("Your spiral shot [src] back!","Attack");step(M,src.dir);sleep(1);step(M,src.dir);src.DamageProc((src.maxhealth*0.01),"Health",M);M.RushLimitz=0
		else
			if(src.ManBeastClaws||src.ConversusinSecondStage||(src.Bijuu=="Nibi"&&src.TailState>=1)||(src.Bijuu=="Kyuubi"&&src.TailState>=1))
				src<<"You slash [M] with your claws!"
				...
				if(M.Clan=="Hoshigaki")
					if(prob(25))
						spawn()src.Bloody()
						src.health-=10
				if(Damage>50&&src.TailState==0)
					Damage=50
				if(src.ManBeastClaws)
					//Damage+=rand(25,30)
					if(((M.dir==turn(src.dir,-90))||M.dir==turn(src.dir,90)))
						Damage*=1.25
						if(prob(60))
							spawn()
								M.Bloody()
					if(src.dir==M.dir)
						...
						if(prob(30))
							spawn()
								M.InternalClawsBleed()
				if(src.Bijuu=="Nibi"&&src.TailState==2&&M)
					Damage+=40
					M.Bloody();M.Bloody()
				M<<output("[src] slashes you with their claws!","Attack")
				if(prob((src.Focus+1)/(M.Buff+1))*10)
					if(!M.Clan=="Kaguya"||M.AkimichiSpirit!=50||M.AkimichiSpirit==50&&!M.calories<=1000)
						...
						M<<output("You were critically hit!","Attack");spawn()Blood(M.x,M.y,M.z)
						M.DamageProc(Damage,"Health",src)
				viewers()<<sound('SFX/Slice2.wav',0,volume=20)
				...
				if(M.Bijuu=="Sanbi"&&(M.TailState==2||M.TailState==3))
					if(prob(70))
						...
						src.health-=20
				if(M.Guarding)
					viewers()<<sound('SFX/Guard.wav',0,volume=15)
				else
					if(prob(src.Focus-M.Buff))
						M.StunAdd(1)
					if(Damage<25)
						viewers()<<sound('SFX/HitWeak.wav',0,volume=10)
					if(Damage>=25&&Damage<100)
						viewers()<<sound('SFX/HitMedium.wav',0,volume=13)
					if(Damage>=100)
						viewers()<<sound('SFX/HitStrong.wav',0,volume=15)
				...
				if(Style=="Jyuken")
					...
					if(prob(5)&&src.TenketsuAccuracy<20&&M.knockedout==0&&M.Dead==0&&M.client)
						src.TenketsuAccuracy+=pick(0.01,0.03,0.04);if(src.TenketsuAccuracy>20) src.TenketsuAccuracy=20
					Damage=Damage/10
				...
				if(src.Kengan==1)
					...
					if(Damage>500)
						Damage=500
					M.HitBack(2,src.dir)
				if(src.ParticleMelee>0&&src.Akametsuki)
					src.ParticleImpact(M)
					src.ParticleMelee-=1
					src.ChakraDrain(8000)
				if(src.ChargedChakraForPunch>0&&src.OukashouHits>0)
					...
					if(Damage<1)
						Damage=1
					if(src.OukashouHits==1)
						src.ChargedChakraForPunch=0
					else if(src.OukashouHits>1)
						src.ChargedChakraForPunch/=2
					src.OukashouHits--
					viewers(src)<<"[src] hits [M] with a massive chakra-empowered blow!"
				if(src.FuuinControl&&M.icon_state!="Block"&&!M.knockedout)
					M.FuuinControlled=1
					src.ChakraDrain(15000)
					src.FuuinControl=0
					src<<"You land the Control Seal on [M]!"
					M<<"[src] has placed a seal on you?"
					...
					if(src.knockedout&&M.FuuinControlled)
						M<<"Your chakra rates go back to normal."
						M.FuuinControlled=0
					spawn(300)
				...
				if(src.FiveElementSeal==1&&!M.knockedout&&M.icon_state!="Block")
					...
					src<<"You land the Five Element Seal on [M]!"
					if(M.Focus>=250&&prob(60))
						M<<"Did [src] just place their hands on your body...?"
					M<<"Your use of Chakra for manipulating into Elemental manipulation has been.....blocked..?"
					if(M.client.computer_id==src.client.computer_id)
						return
					if(prob(20))
						src.FuuinjutsuKnowledge+=pick(0.1,0.5,0.6,1)
				if(src.CurseSealParalysis&&!M.knockedout&&M.icon_state!="Block")
					...
					src<<"You land the Curse Seal Paralysis on [M]! Press Z when you want to set it off on [M], but make sure they're within range!"
					if(M.Focus>=505)
						M<<"Did [src] just place their hands on your body...?"
					if(M.client.computer_id==src.client.computer_id)
						return
				...
				M<<output("[src] hit you for [Damage] Damage!","Attack")
				src<<output("[M] has been hit for [Damage] Damage!","Attack")
				src.RushLimitz+=1
				if(src.EyesOfHell==2&&src.ConversusinThirdStage)
					if(prob(30))
						M.InternalBleed()
				...
				if(src.RushLimitz>=src.Rush)
					src.RushLimitz=0
				if(Style=="Jyuken")
					if(src.Focus>(M.Focus-10)&&prob(50+min(src.TenketsuAccuracy,20)))
						M.StunAdd(2)
			...
			if(!M.ImmuneToDeath&&!M.knockedout)
				spawn(JyuukenTime)
					...
					//			M.DamageProc(Damage2,"Health",src)
								if(prob((src.TenketsuAccuracy*0.5)+8)&&M)
...
mob/proc
	Attacking15(mob/M)
		...
		var/Mlocation=locate(M.x,M.y,M.z)
		if(M.NenkinOn)
			viewers()<<sound('SFX/Guard.wav',0,volume=10);return

		if(M.ingat)
			src<<output("[M]'s spiral shot you back!","Attack");M<<output("Your spiral shot [src] back!","Attack");step(M,src.dir);sleep(1);step(M,src.dir);src.health-=src.maxhealth*0.01;M.RushLimitz=0
		if(M.FrozenBind=="DoBind")
			src<<output("Your fists bounce harmlessly off the rocks...","Attack");M<<output("[src]'s fist bounce harmlessly off the rocks...","Attack")
			viewers()<<sound('SFX/Guard.wav',0,volume=15);return
			return
		if(M.Kaiten)
			src<<output("[M]'s rotation shot you back!","Attack");M<<output("Your rotation shot [src] back!","Attack");step(M,src.dir);sleep(1);step(M,src.dir);src.health-=rand(1,damage);M.RushLimitz=0;src.Death(M)
			if(M.Buffet>=1)
				M.BuffetGain+=1;if(M.BuffetGain>round(M.Buffet)) M.BuffetGain=round(M.Buffet)
			return
		else
			...
			if(prob((src.Focus+1)/(M.Buff+1))*10)
				if(!M.Clan=="Kaguya"||M.AkimichiSpirit!=50||M.AkimichiSpirit==50&&!M.calories<=1000)
					src<<output("Critical Hit!","Attack");M<<output("You were critically hit!","Attack");M.DamageProc(damagehit,"Stamina",src)
				if(!M.Guarding)
					if(M.Stun<=0)
						M.StunAdd(rand(1,2))
				if(prob(1))
				...
				if(prob(1)&&M.client)
					...
					src.Focus+=0.1
				spawn()AttackEfx(Mlocation)
			src<<output("You hit [M] for [damagehit] damage!","Attack");M<<output("[src] hit you for [damagehit] damage!","Attack")
			if(M.Guarding)
				viewers()<<sound('SFX/Guard.wav',0,volume=20)
				damagehit=damagehit*0.5
			else
				var/Z=rand(1,3);if(Z==1)
					viewers()<<sound('SFX/HitWeak.wav',0,volume=10)
				if(Z==2)
					viewers()<<sound('SFX/HitMedium.wav',0,volume=12)
				if(Z==3)
					viewers()<<sound('SFX/HitStrong.wav',0,volume=15)
...
mob/proc
	KunaiHit(mob/M)
		if(prob(1))
			src.KunaiMastery+=pick(0.1,0.2)
		var/damage=M.maxhealth*(0.005+(src.KunaiMastery*0.0005))
		if(damage>M.maxhealth*0.01)
			damage=M.maxhealth*0.01
		if(!M)
			return
		if(M.NenkinOn)
			viewers()<<sound('SFX/SwordClash.wav',0,volume=15);return
		if(M.sphere)
			viewers()<<sound('SFX/Guard.wav',0,volume=15);return
		if(M.ingat)
			src<<output("[M]'s spiral shot you back!","Attack");M<<output("Your spiral shot [src] back!","Attack");step(M,src.dir);sleep(1);step(M,src.dir);src.health-=src.maxhealth*0.01;M.RushLimitz=0
		if(M.FrozenBind=="DoBind")
			src<<output("Your weapon bounces harmlessly off the rocks...","Attack");M<<output("[src]'s weapon bounces harmlessly off the rocks...","Attack")
		...
		if(M.Kaiten)
			...
			M<<output("[src] slashes you with the kunai!","Attack")
			if(prob((src.Focus+1)/(M.Buff+1))*10)
				if(!M.Clan=="Kaguya"||M.AkimichiSpirit!=50||M.AkimichiSpirit==50&&!M.calories<=1000)
					src<<output("Critical Hit!","Attack")
			...
			for(var/mob/player/C in view())
				if(C.ckey!=null)
					spawn()
						C.client.sound_system.PlaySound('SFX/Slice2.wav')
	//		viewers()<<sound('SFX/Slice2.wav',0,volume=35)
			src<<output("You slash [M]!","Attack")
			spawn()Blood(M.x,M.y,M.z)
			if(M.RaiArmor)
				src.StunAdd(10);src.Death(M)
...
mob/proc
	ClawsHit(mob/M,Damage,Type,Power)
		var/doodat=(src.Reflex*2)+1/(M.Reflex+1)
		...
		damage=damage*((Power/3)*0.1)         //Factor in the Strength put into the slash. Charge for 2 seconds to do 100% damage.
		...
		if(M.FrozenBind=="DoBind")
			...
			return
		if(M.Kaiten)
			src<<"[M]'s rotation shot you back!";M<<"Your rotation shot [src] back!"
			step(src,M.dir);sleep(1);step(src,M.dir)
			src.DamageProc(rand(1,damage),"Health",M)
			return
		if(M.ingat)
			src<<"[M]'s spiral shot you back!";M<<"Your spiral shot [src] back!"
			step(src,M.dir);sleep(1);step(src,M.dir)
			src.DamageProc(src.maxhealth*0.01,"Health",M)
			return
		else
			if(M.KagDance=="Karamatsu"&&prob(M.DanceMastery+(M.BoneMastery/2)))
				viewers(M,7)<<sound('SFX/SwordClash.wav',0,volume=15)
				M<<output("You block out [src]'s attack with Karamatsu!","Attack");M.chakra-=rand(10,15)
				src<<output("Your attack was blocked!","Attack")
				return
			M<<output("[src] slashes you!","Attack")
			if(prob((src.Focus+1)/(M.Buff+1))*10)
				if(!M.Clan=="Kaguya"||M.AkimichiSpirit!=50||M.AkimichiSpirit==50&&!M.calories<=1000)
					src<<output("Critical Hit!","Attack");M<<output("You were critically hit!","Attack")
...
//		var/DamagePlus=rand(50,80)
		...
		if(M.sphere)
			if(M.CastingGenjutsu)
				M.GenjutsuCanceling()
			viewers()<<sound('SFX/Guard.wav',0,volume=17);return
		if(M.FrozenBind=="DoBind")
			src<<output("Your weapon bounces harmlessly off the rocks...","Attack");M<<output("[src]'s weapon bounces harmlessly off the rocks...","Attack")
			viewers()<<sound('SFX/Guard.wav',0,volume=15);return
			return
		...
		if(M.ingat)
			...
			if(M.DotonArmor)
				if(M.CastingGenjutsu)
					M.GenjutsuCanceling()
				src<<output("You try to strike [M] but the rocks surrounding their body stop you from harming them!","Attack");M<<output("You block out [src]'s attack, thanks to the rocks surrounding your body.","Attack");return
			M<<output("[src] hits you with the Nunchuks!","Attack")
			if(prob((src.Focus+1)/(M.Buff+1))*10)
				if(!M.Clan=="Kaguya"||M.AkimichiSpirit!=50||M.AkimichiSpirit==50&&!M.calories<=1000)
					src<<output("Critical Hit!","Attack");M<<output("You were critically hit!","Attack")
				spawn()
			...
			spawn()
				viewers()<<sound('SFX/HitWeak.wav',0,volume=12)
			spawn()
				if(M.RaiArmor)
					spawn()
						src.StunAdd(10);
			...
			if(M.Buffet>=1)
				M.BuffetGain+=1;if(M.BuffetGain>round(M.Buffet)) M.BuffetGain=round(M.Buffet)
			if(prob(src.TaijutsuMastery*2))
				spawn()
					M.StunAdd(pick(1,1.5))
	//		return
	...
	StaveHit(mob/M,Damage,Type,Power,Cap)
		...
		if(M.Clan=="Yotsuki"&&prob((M.DeflectionZ+M.Buffet+M.Teamwork+M.SpeedDice)*1.5))
			if(M.LeftHandSheath||M.RightHandSheath)
				M<<output("Your weapons deflect each other!","Attack");src<<output("Your weapons deflect each other!","Attack");viewers()<<sound('SFX/SwordClash.wav',0,volume=20);M.Deflection=0;src.Deflection=0
				spawn() AttackEfx4(M.x,M.y,M.z,M.dir);AttackEfx4(src.x,src.y,src.z,src.dir)
			...
			if(M.DotonArmor)
				if(M.CastingGenjutsu)
					M.GenjutsuCanceling()
				src<<output("You try to strike [M] but the rocks surrounding their body stop you from harming them!","Attack");M<<output("You block out [src]'s attack, thanks to the rocks surrounding your body.","Attack");return
			M<<output("[src] hits you with the Staff!","Attack")
			if(prob((src.Focus+1)/(M.Buff+1))*10)
				if(!M.Clan=="Kaguya"||M.AkimichiSpirit!=50||M.AkimichiSpirit==50&&!M.calories<=1000)
					src<<output("Critical Hit!","Attack");M<<output("You were critically hit!","Attack")
				M.DamageProc(damage,"Stamina",src)
			src<<output("You hit [M]!","Attack")
			viewers()<<sound('SFX/HitStrong.wav',0,volume=20)
			if(M.RaiArmor)
			...
			if(prob(7.75*(src.HeavySlam+1)))
				...
				M.Running=0
				src<<output("You landed a large slam on [M]!","Attack");M<<output("[src] smacks you fiercly with their Bo!","Attack")
			M.DamageProc(damage,"Stamina",src)
			var/random=0.06
			if(M.client)
				random*=10
		...
		if(M.Clan=="Yotsuki"&&prob((M.DeflectionZ+M.Buffet+M.Teamwork+M.SpeedDice)*1.5))
			if(M.LeftHandSheath||M.RightHandSheath)
				if(prob(100-ReflexDecreaser))
					M<<output("Your weapons deflect each other!","Attack");
					src<<output("Your weapons deflect each other!","Attack");
					if(prob(1))
						src.Kenjutsu+=pick(0.2,0.3)
						var/image/I = new('Icons/Jutsus/Skillcards.dmi');I.pixel_y=32;I.layer=6;I.icon_state="SwiftYes"
						src<<I
						spawn(15)
							del(I)
						...
						if(src.Clan!="Yotsuki"&&src.Kenjutsu>100)
							...
							src.Kenjutsu=125
					viewers()<<sound('SFX/SwordClash.wav',0,volume=15);
					M.Deflection=0;
					src.Deflection=0
					spawn() AttackEfx4(M.x,M.y,M.z,M.dir);AttackEfx4(src.x,src.y,src.z,src.dir)
					...
					M<<output("You tried to deflect [src]'s sword but failed!")
		else if(M.Deflection||M.WeaponMaster>=6&&M.LeftHandSheath||M.WeaponMaster>=6&&M.RightHandSheath)
			if(M.dir!=src.dir&&src.dir!=turn(M.dir,45)&&src.dir!=turn(M.dir,-45))
				if(prob(100-ReflexDecreaser))
					M<<output("Your weapons deflect each other!","Attack");src<<output("Your weapons deflect each other!","Attack");viewers()<<sound('SFX/SwordClash.wav',0,volume=15);M.Deflection=0;src.Deflection=0
					spawn() AttackEfx4(M.x,M.y,M.z,M.dir);AttackEfx4(src.x,src.y,src.z,src.dir)
					if(prob(1))
						src.Kenjutsu+=pick(0.2,0.3)
						var/image/I = new('Icons/Jutsus/Skillcards.dmi');I.pixel_y=32;I.layer=6;I.icon_state="SwiftYes"
			...
			if(M.KagDance=="Karamatsu"&&prob(M.DanceMastery+(M.BoneMastery/2)))
				if(prob(100-ReflexDecreaser))
					...
					M<<output("[src]'s sword broke through your defenses!")
			if(M.DotonArmor)
				if(M.CastingGenjutsu)
					M.GenjutsuCanceling()
				src<<output("You try to strike [M] but the rocks surrounding their body stop you from harming them!","Attack");M<<output("You block out [src]'s attack, thanks to the rocks surrounding your body.","Attack");return
			...
			if(SizeRating==1)
				for(var/mob/player/C in view())
					if(C.ckey!=null)
						spawn()
							C.client.sound_system.PlaySound('SFX/Slice2.wav')
			//	viewers()<<sound('SFX/Slice2.wav',0,volume=30)
			else if(SizeRating==2||SizeRating==3)
				for(var/mob/player/C in view())
					if(C.ckey!=null)
						spawn()
							C.client.sound_system.PlaySound('SFX/Slice.wav')
...
mob/proc
	SamehadeNew(mob/M,Power)
		...
		if(M.Clan=="Yotsuki"&&prob((M.DeflectionZ+M.Buffet+M.Teamwork+M.SpeedDice)*1.5))
			...
			spawn()
				Blood(Mlocation);viewers()<<sound('SFX/Slice.wav',0,volume=15);M.DamageProc(damage,"Health",src);sleep(1)
			spawn(1)
				Blood(Mlocation);viewers()<<sound('SFX/Slice.wav',0,volume=15);M.DamageProc(damage,"Health",src)
			sleep(1)
			if(M.RaiArmor)
				src.StunAdd(1);src.Death(M)
			if(prob(50)&&M)
				spawn()Blood(Mlocation);viewers()<<sound('SFX/Slice.wav',0,volume=15);M.DamageProc(damage,"Health",src)
			M.RushLimitz=0
			if(M.Buffet>=1)
				M.BuffetGain+=1;if(M.BuffetGain>round(M.Buffet)) M.BuffetGain=round(M.Buffet)
			if(Power>5)
...
mob/proc
	...
	//		return
		...
		if(M.Clan=="Yotsuki"&&prob((M.DeflectionZ+M.Buffet+M.Teamwork+M.SpeedDice)*1.5))
			...
			if(!M){return}
			src<<output("You shaved with the Samehade!","Attack")
			spawn()Blood(Mlocation);viewers()<<sound('SFX/Slice.wav',0,volume=15);M.DamageProc(damage,"Health",src);sleep(1)
			spawn(1)Blood(Mlocation);viewers()<<sound('SFX/Slice.wav',0,volume=15);M.DamageProc(damage,"Health",src)
			if(prob(60))
...
mob/proc
	ChidoriA(mob/M)
		...
		if(M.Kaiten)
			view() << "<i><font size = 2>[src] attempts to peirce through [M]'s defense but fails!</font></i>"
			return
		if(M.sphere)
		...
		if(M.RaiArmor)
			src<<output("[M] absorbs the shock!","Attack");M<<output("[M] absorbs the shock!","Attack");src.overlays-='Icons/Jutsus/Chidori.dmi';src.ChidoriD=0;src.Chidorion=0;src.Raikirion=0;src.Normal();return
		if(src.Chidorion)
		...
		M.DamageProc(damage,"Health",src)
		M.Bloody();viewers()<<sound('SFX/Slice2.wav',0,volume=15)
...
mob/proc
	ChakraMesu(mob/M)
		...
		if(M.ingat)
			...
			M<<output("[src] slashes you with their scapels for [damage]!","Attack")
			if(prob((src.Focus+1)/(M.Buff+1))*10)
				if(!M.Clan=="Kaguya"||M.AkimichiSpirit!=50||M.AkimichiSpirit==50&&!M.calories<=1000)
					src<<output("Critical Hit!","Attack")
				M<<output("You were critically hit!","Attack")
			...
			viewers()<<sound('SFX/Slice2.wav',0,volume=15);spawn()Blood(Mlocation)
			M.RushLimitz=0
			if(M.Buffet>=1)
				M.BuffetGain+=1;if(M.BuffetGain>round(M.Buffet)) M.BuffetGain=round(M.Buffet)
			M.DamageProc(damage,"Health",src)
...
mob/proc
	Yanagi(mob/M)
		...
		if(M.ingat)
			...
			src.Death(M)
			return
		if(M.Clan=="Yotsuki"&&prob((M.DeflectionZ+M.Buffet+M.Teamwork+M.SpeedDice)*1.5)&&src.SageMode!="Bone")
			if(M.LeftHandSheath||M.RightHandSheath)
				M<<output("Your weapons deflect each other!","Attack");src<<output("Your weapons deflect each other!","Attack");viewers()<<sound('SFX/SwordClash.wav',0,volume=15);M.Deflection=0;src.Deflection=0
				spawn() AttackEfx4(M.x,M.y,M.z,M.dir);AttackEfx4(src.x,src.y,src.z,src.dir)
				if(prob(1))
					M.DeflectionZ+=pick(0.1,0.3);if(M.DeflectionZ>5) M.DeflectionZ=5
				return
		else if(M.Deflection&&prob(40)&&src.SageMode!="Bone")
			M<<output("Your weapons deflect each other!","Attack");src<<output("Your weapons deflect each other!","Attack");viewers()<<sound('SFX/SwordClash.wav',0,volume=15);M.Deflection=0;src.Deflection=0
			spawn() AttackEfx4(M.x,M.y,M.z,M.dir);AttackEfx4(src.x,src.y,src.z,src.dir)
			if(prob(1))
				M.DeflectionZ+=pick(0.1,0.3);if(M.DeflectionZ>5) M.DeflectionZ=5
			...
			src<<output("You slash [M]!","Attack")
			M.Bloody()
			viewers()<<sound('SFX/Slice2.wav',0,volume=15)
			M.RushLimitz=0
			if(M.Buffet>=1)
				M.BuffetGain+=1;if(M.BuffetGain>round(M.Buffet)) M.BuffetGain=round(M.Buffet)
			M.DamageProc(damage,"Health",src)
		...
		if(M.Clan=="Yotsuki"&&prob((M.DeflectionZ+M.Buffet+M.Teamwork+M.SpeedDice)*1.5)&&src.SageMode!="Bone")
			if(M.LeftHandSheath||M.RightHandSheath)
				...
				if(prob(1))
					M.DeflectionZ+=pick(0.1,0.3);if(M.DeflectionZ>5) M.DeflectionZ=5
				return
		else if(M.Deflection&&src.SageMode!="Bone"||M.WeaponMaster>6&&M.LeftHandSheath&&src.SageMode!="Bone"||M.WeaponMaster>6&&M.RightHandSheath&&src.SageMode!="Bone")
			M<<output("Your weapons deflect each other!","Attack");src<<output("Your weapons deflect each other!","Attack");viewers()<<sound('SFX/SwordClash.wav',0,volume=20);M.Deflection=0;src.Deflection=0
			spawn() AttackEfx4(M.x,M.y,M.z,M.dir);AttackEfx4(src.x,src.y,src.z,src.dir)
			if(prob(1))
				M.DeflectionZ+=pick(0.1,0.3);if(M.DeflectionZ>5) M.DeflectionZ=5
