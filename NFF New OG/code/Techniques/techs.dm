...
obj/Move()
	...
	if(src.speeding <= 0||src.Move_Delay<0)
		...
		for(var/atom/movable/A in get_steps(src,src.dir,2))
			if(ismob(A))
				if(A:key=="DemonicK"||(A:key=="Shiharu"&&prob(15))||(A:key=="TheUndyingOne"&&prob(15)))
				//	A:TestDodge(src)
					A:WarpDodge(src)
		...
		while(usr.Charging && !usr.IntensityChakra && usr.ChargingInstance == Instance)
			if(usr.Guarding)
				...
				if(usr.meditating)
					...
					if(usr.BaikaCharged>(5+usr.SizeCap))
					...
					if(!usr.ChidoriCharge&&!usr.RasenganCharge&&usr.GateIn=="")
						...
						if(usr.ChakraPool<=0&&usr.Trait2=="Adamant")
							...
							if(usr.SageMode=="Slug")
								Drain*=2.5
							if(usr.CursedSealType=="Heaven"&&usr.CursedSealActivated)
								Drain*=3.5
							usr.chakra+=Drain
...
mob/proc
	...
	//		gain=35
		...
		if(prob(25))
			src.exp+=gain
	//	if(prob(30)&&damage>300)
	//		src.exp+=gain
	//	else if(prob(20)&&damage>50)
	//		src.exp+=gain/3
obj/Weapons
	name = null
	icon = 'Weapons.dmi'
			...
			if(src.icon_state=="shuriken"||src.icon_state=="shuriken2"||src.icon_state=="shuriken3")
				...
				if(counta<=0)
					...
					for(var/obj/WEAPONS/Shuriken/O in usr.contents)
						...
						O.name= "[O.name] ([O.ammount])"
						del(src);del(A)
			if(src.icon_state=="shuriken4"||src.icon_state=="shuriken5"||src.icon_state=="shuriken6")
				for(var/obj/Weapons/W in view(0,src))
					if(W!=src)
						if(W.icon_state=="shuriken4"||W.icon_state=="shuriken5"||W.icon_state=="shuriken6")
							if(!W.ShadowClone)
								counter++
								del(W)
				if(counter==1) usr<<"You picked up a 3 Sided Shuriken."
				else if(counter>1) usr<<"You picked up [counter] 3 Sided Shurikens."
				var/obj/WEAPONS/Shuriken3/A=new()
				A.icon_state="Shuriken"
				src.icon_state=""
				var/counta=0
				...
				if(counta<=0)
					A.loc=usr
					A.ammount+=counter;A.name= "3 Sided Shuriken ([A.ammount])"
					del(src)
				else
					for(var/obj/WEAPONS/Shuriken3/O in usr.contents)
						O.ammount+=counter
						O.name= "[O.name] ([O.ammount])"
						del(src);del(A)
			if(src.icon_state=="shuriken7")
				for(var/obj/Weapons/W in view(0,src))
					if(W!=src)
						if(W.icon_state=="shuriken7")
							if(!W.ShadowClone)
								counter++
								del(W)
				if(counter==1) usr<<"You picked up a 8 Sided Shuriken."
				else if(counter>1) usr<<"You picked up [counter] 8 Sided Shurikens."
				var/obj/WEAPONS/Shuriken8/A=new()
				A.icon_state="Shuriken"
				src.icon_state=""
				var/counta=0
				...
				if(counta<=0)
					A.loc=usr
					A.ammount+=counter;A.name= "8 Sided Shuriken ([A.ammount])"
					del(src)
				else
					for(var/obj/WEAPONS/Shuriken8/O in usr.contents)
						O.ammount+=counter
...
//	if(usr.meditating)
//		return
	if(usr.Frozen||usr.knockedout||usr.Stun>=1||usr.DoingHandseals||usr.doingG||usr.inso||usr.Kaiten||usr.ingat||usr.firing||usr.FrozenBind!=""||usr.doingG||usr.Shibari||usr.UnableToChargeChakra)
		return
	if(usr.IntensityChakra)
