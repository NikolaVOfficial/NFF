...
mob/var/BodyFlickerDistanceTrained=0
mob/var/BodyFlickerMasterTrained=0
mob/var/BodyFlickerRepetitionTrained=0
...
mob/GainedAfterLogIn/verb
	...
	ClanSkill()
		...
		if(usr.Skill_Tree||!usr.loggedin||!usr.cansave)
			...
			if(usr.Clan=="Kieru")
				usr<<"Your view has changed, but you are still in the same spot.";usr.Skill_Tree=1;client.perspective=EYE_PERSPECTIVE|EDGE_PERSPECTIVE;client.eye=locate(78,182,42);return
			if(usr.Clan=="Kumojin")
		...
		HengeTechnique
			...
			Click()
				...
				//		color:white;}</style>"
					...
					usr.LearnJutsu("Henge No Jutsu",200,"Henge","Henge No Jutsu is a basic ninja technique that disguises yourself in another's appearance.","Ninjutsu")
		KawaTechnique
			...
			Click()
				...
				//		color:white;}</style>"
					...
					usr.LearnJutsu("Kawarimi No Jutsu",200,"Kawarimi","Kawarimi No Jutsu is a basic jutsu that substitutes you with a log to escape harm.","Ninjutsu")
		ExpKawaTechnique
			...
			Click()
				...
				//		color:white;}</style>"
					...
					usr.LearnJutsu("Exploding Kawarimi No Jutsu",6000,"ExpKawarimi","Exploding Kawarimi No Jutsu is a more advanced substitution jutsu that replaces you with an exploding log.","Ninjutsu")
		ShunshinTechnique
			...
			Click()
				...
				//		color:white;}</style>"
					...
					usr.LearnJutsu("Shushin No Jutsu",500,"Shushin","Shushin No Jutsu is a body flicker technique for high speed movement over a short distance.","Ninjutsu")
		NawanukeTechnique
			name="Escaping Skill"
			icon_state="Nawanuke"
			Click()
				...
				//		color:white;}</style>"
					...
					usr.LearnJutsu("Escaping Skill",1000,"Nawanuke","A basic Ninpo technique which allows a ninja to escape from wires. Very useful..","Ninjutsu")
		Choke
			name="Choke"
			icon_state="Choke"
			Click()
				...
				//		color:white;}</style>"
					...
					usr.LearnJutsu("Choke",8500,"Choke","Choke is a powerful physical technique to pick up an opponent by the neck, and suffocate them.","Taijutsu")
		HiddenLotus//HiddenLotus
			...
			Click()
				...
				if(/obj/SkillCards/HiddenLotus in P)
					...
					usr<<"You need to learn the Sixth Gate to learn this jutsu!";return
				else
					usr.LearnJutsu("Hidden Lotus",100000,"HiddenLotus","Hidden Lotus is a jutsu Requiring the use of Gates to use, the user is able to rush at the speed of light, destroying the Opponent's body like a pingpong ball.","Taijutsu")
		GateAssault//GateAssault
			...
			Click()
				...
				if(/obj/SkillCards/GateAssault in P)
					...
					usr.LearnJutsu("Gates Assault",75000,"GateAssault","Gates Assault is a jutsu that allows you to beat your opponent with brutal Force at fast speed so they have no time to defend themselves.","Taijutsu")
		ChokeSlam
			name="Choke"
		...
		FingerPush
			...
			Click()
				...
				//		color:white;}</style>"
					...
					usr.LearnJutsu("Finger Push",5000,"FingerPush","Finger Push is a physical technqiue that requires little effort but extreme taijutsu. Using one finger it flicks an opponent quickly across a large area very quickly, dealing small damage.","Taijutsu")
		ExplodingFormation
			...
			Click()
				...
				//		color:white;}</style>"
					...
					usr.LearnJutsu("Exploding Formation",5000,"ExplodingFormation","Exploding Formation is a ninjutsu technique that explodes all of the user's Exploding Tags at the same time instead of seperately.","Ninjutsu")
		PersonSwapJutsu
			...
			Click()
				...
				//		color:white;}</style>"
					...
					usr.LearnJutsu("Person Swap Jutsu",10000,"BodySwitch","The Person Swap Jutsu is a technique which allows you to swap positions quickly with another person granted that they are not skilled enough to stop you.","Ninjutsu")
		BunshinSubstitution
			...
			Click()
				...
				//	var/style = "<style>BODY {margin:0;font:arial;background:black;\
					...
					usr.LearnJutsu("Clone Trick",5000,"BunshinSubsitutionJutsu","A quick substitution jutsu which creates a shadow clone to take your place while you are able to make a quick escape.","Ninjutsu")
...
turf
	Skilltree/Passives/ElementalPassives
		...
		ConvertHardWorker
			icon_state="Convert"
			Click()
				if(usr.Trait!="Hard-Worker")
					usr<<"This feature is only available for those with the Hard-Worker trait."
					return
				if(usr.ElementalPoints>=6&&!usr.clicking)
					usr.clicking=1
					switch(input(usr,"Would you like to convert some of your Elemental Points to Starting Points? WARNING: This cannot be reversed..") in list("No","Yes"))
						if("Yes")
							var/Number=input(usr,"How many Elemental Points do you wish to convert?") as num
							if(Number<1)
								Number=1
							if(Number<6)
								usr<<"You need to convert at least 6 Elemental Points.";
								usr.clicking=0
								return
							if(Number%6!=0)
								usr<<"You need to convert an amount of Elemental Points that are divisible by 6.(6,12,18,24,30, and so on)";
								usr.clicking=0
								return
							if(Number>usr.ElementalPoints)
								usr<<"You don't have that many Elemental Points."
								usr.clicking=0
								return
			//					Number=usr.StartingPoints

							switch(input(usr,"CONFIRM: Convert [Number] Elemental Points to [Number/6] Starting Points for a total of [(Number/6)+usr.StartingPoints] Starting Points and [usr.ElementalPoints-Number] Elemental Points?") in list("No","Yes"))
								if("Yes")
									usr.ElementalPoints-=Number;
									usr.StartingPoints+=(Number/6)
									alert(usr,"You turned your [Number] Elemental Points into [Number/6] Starting Points. Conversion Successful.")
					usr.clicking=0
				else if(usr.clicking)
					alert(usr,"Slow down. One pop up at a time.");return
				else
					alert(usr,"You don't have enough Elemental Points to convert to Starting Points!");return
		Jutsus
			icon='Icons/Jutsus/Skillcards.dmi'
			//Katon
			Hikibou
				...
				//		usr<<"You need at least 1 / 20 Fire Mastery before you can learn this jutsu.";return
					if((usr.ElementalPoints >=3)&&(usr.Mnin >= 15))
						usr.LearnJutsu("Katon Hikibou No Jutsu",10000,"Hikibou","Katon Hikibou No Jutsu is a technique where the user heats the air around the target. After heating the air, that region will combust in flames. It is very useless at the beginning, but the more you use it the stronger it gets. 3 Elemental Points","Katon",3)
			...
			RaseKa
				...
				Click()
					...
					if((usr.ElementalPoints >=4)&&(usr.Mnin >=15))
						usr.LearnJutsu("Katon Spinning Fire",20000,"SpinningFire","Katon RaseKa is a jutsu that shoots a ball of fire torwards the opponent, engulfing them in flames if hit and nearby katon jutsu also. 4 Elemental Points","Katon",4)
			...
			//			usr<<"You need at least 1 / 20 Fire Mastery before you can learn this jutsu.";return
					if((usr.ElementalPoints>=3)&&(usr.Mnin >= 15))
						usr.LearnJutsu("Katon Housenka No Jutsu",10000,"Housenka","Katon Housenka No Jutsu is a jutsu that shoots multiple fire balls from the mouth that explode in fire on contact of a target. 3 Elemental Points","Katon",3)
					//	usr.ElementalPointDeduction =3
						usr.Frozen = 0
						return
					else
			...
			HousenkaTsumabeni
				...
				Click()
					...
					if((usr.ElementalPoints>=3)&&(usr.Mnin>=15))
						usr.LearnJutsu("Katon: Pheonix Sage Flower",25000,"PhoenixSageFlower","Katon Pheonix Sage Flower is a jutsu that allows the opponent to utilize their Chakra and Shuriken to create flaming shurikens. 3 Elemental Points","Katon",3)
					//	usr.ElementalPointDeduction=3
						usr.Frozen=0
						return
					else
			...
			//			usr<<"You need at least 1/20 Fire Mastery before you can learn this jutsu.";return
					if((usr.ElementalPoints>=3)&&(usr.Mnin>=15))
						usr.LearnJutsu("Katon Endan",15000,"KatonFlameBullet","Katon Endan is a jutsu utilizing a small fire projectile that explodes on contact. 3 Elemental Points","Katon",3)
				//		usr.ElementalPointDeduction=3
						usr.Frozen=0
						return
					else
			...
			KatonFlameBulletAdvance
				...
				Click()
					...
					if((usr.ElementalPoints>=4)&&(usr.Mnin>=19))
						usr.LearnJutsu("Katon Dai Endan",30000,"KatonFlameBulletAdvance","Katon Dai Endan is like Katon Endan but the user utilizes even more powerful projectiles. 4 Elemental Points","Katon",4)
				//		usr.ElementalPointDeduction=4
						usr.Frozen=0
						return
					else
			...
			KatonFlameEmber
				...
				Click()
					...
					if((usr.ElementalPoints>=3)&&(usr.Mnin>=21))
						usr.LearnJutsu("Katon Flame Ember",20000,"FlameEmber","Katon Flame Ember is similar to the jutsu like Endan, however this one has a severe chance to burn the opponent. 3 Elemental Points","Katon",3)
					//	usr.ElementalPointDeduction=3
						usr.Frozen=0
						return
					else
			...
			Goukakyuu
				...
				Click()
					if((usr.ElementalPoints >=3)&&(usr.Mnin >= 22))
						usr.LearnJutsu("Katon Goukakyuu No Jutsu",20000,"Goukakyuu","Katon Goukakyuu No jutsu is a field ranged jutsu that releases fire from the mouth. It covers a small radius, that can improve, so it can be very useful in combat. 3 Elemental Points","Katon",3)
					//	usr.ElementalPointDeduction = 3
						usr.Frozen = 0
						return
					else
			...
			AshPileBurning
				...
				Click()
					...
					if((usr.ElementalPoints>=4)&&(usr.Mnin >= 24))
						usr.LearnJutsu("Katon: Ash Pile Burning",25000,"AshPileBurning","Katon Ash Pile Burning is a jutsu that allows the user to create their katon into a foggy like substance, dealing less damage, but having a larger chance to burn. Also isn't destroyed by water. 4 Elemental Points","Katon",4)
				//		usr.ElementalPointDeduction = 4
						usr.Frozen = 0
						return
					else
			...
			Gouenkyuu
				...
				Click()
					...
					if((usr.ElementalPoints>=5)&&(usr.Mnin>=25))
						usr.LearnJutsu("Katon: Gouenkyuu",45000,"KatonHoukaAdvanced","Katon Gouenkyuu is an advanced version of Goukakyuu, allowing the user to shoot the fire projectile, which then pushes anything it catches, until it engulfes in flames. 5 Elemental Points","Katon",5)
			...
			GoukaMekkyaku
				...
				Click()
					...
					if((usr.ElementalPoints>=5)&&(usr.Mnin>=28))
						usr.LearnJutsu("Katon: Great Majestic Fire Annihilation",75000,"GoukaMekkyakuNoJutsu","A technique once used by the Legendary Uchiha Madara, this technique destroys anything in it's past with a huge wave of Fire. 5 Elemental Points","Katon",5)
					//	usr.ElementalPointDeduction=5
						usr.Frozen=0
						return
					else
			...
			Ryuusenka
				...
				Click()
					...
					if((usr.ElementalPoints>=6)&&(usr.Mnin>=25))
						usr.LearnJutsu("Katon: Ryuusenka",75000,"KatonDragonWar","Katon Ryuusenka is truly a dangerous jutsu. Allowing the user to combine Houka and Katon Ryuuka into one fierce and deadly combination. 6 Elemental Points","Katon",6)
			...
			Ryuuka
				...
				Click()
					...
					if((usr.ElementalPoints >=3)&&(usr.Mnin >= 25))
						usr.LearnJutsu("Katon Ryuuka No Jutsu",30000,"Ryuuka","Katon Ryuuka No Jutsu is a concentrated form of Karyuu Endan that shoots a concentrated blast that explodes on contact of a physical object. 3 Elemental Points","Katon",3)
					//	usr.ElementalPointDeduction = 3
						usr.Frozen = 0
						return
					else
			...
			Houka
				...
				Click()
					...
					if((usr.ElementalPoints >=4)&&(usr.Mnin >= 22))
						usr.LearnJutsu("Katon Houka",20000,"KatonHouka","Katon Houka is a powerful fireball technique that can be considered an enhancement to Housenka. It works by shooting a giant fireball that explodes on contact or through time to an even bigger flame. 4 Elemental Points","Katon",4)
					//	usr.ElementalPointDeduction = 4
						usr.Frozen = 0
						return
					else
			...
			ChouHouka
				...
				Click()
					...
					if((usr.ElementalPoints >=5)&&(usr.Mnin >= 33))
						usr.LearnJutsu("Katon Chou Houka",100000,"ChouHouka","Katon Chou Houka is a powerful fireball and the new enhancement to Houka! This jutsu is typically the same as Houka except less concentrated and less powerful, shooting multiple fireballs in all directions once trained enough. 5 Elemental Points","Katon",5)
					//	usr.ElementalPointDeduction = 5
						usr.Frozen = 0
						return
					else
			...
			KaryuuEndan
				...
				Click()
					...
					if((usr.ElementalPoints >=5)&&(usr.Mnin >= 32))
						usr.LearnJutsu("Katon Karyuu Endan",100000,"KaryuuEndan","Katon Karyuu Endan shoots a large blast of fire in a straight direction that burns enemies as it flows. It has incredible destruction ratio. 5 Elemental Points","Katon",5)
					//	usr.ElementalPointDeduction = 5
						usr.Frozen = 0
						return
					else
			...
			Gouryuuka
				...
				Click()
					...
					if((usr.ElementalPoints >=6)&&(usr.Mnin >= 34))
						usr.LearnJutsu("Katon Gouryuuka",125000,"Gouryuuka","Gouryuuka is a powerful firey dragon head expelled from the mouth of the user. It incinerates all in its path. 6 Elemental Points",6)
				//		usr.ElementalPointDeduction = 6
						usr.Frozen = 0
						return
					else
			...
			Raikyuu
				...
				//		usr<<"You need 1 / 20 Lightning Mastery before you can learn this jutsu!";return
					if((usr.ElementalPoints >=3)&&(usr.Mnin >= 15))
						usr.LearnJutsu("Raiton; Raikyuu No Jutsu",10000,"Raikyuu","Raiton; Raikyuu No Jutsu is a technique where the user charges chakra within their hands and shoot it off. 3 Elemental Points","Raiton",3)
					//	usr.ElementalPointDeduction = 3
						usr.Frozen = 0
						return
					else
			...
			Ikazuchi
				...
				Click()
					...
					if((usr.ElementalPoints >=4)&&(usr.Mnin >= 22))
						usr.LearnJutsu("Raiton; Ikazuchi No Kiba",20000,"IkazuchiKiba","Raiton; Ikazuchi No Kiba is a powerful Raiton technique which shoots bolts of lightning on all side of the user! 4 Elemental Points","Raiton",4)
					//	usr.ElementalPointDeduction = 4
						usr.Frozen = 0
						return
					else
			...
			Touei
				...
				Click()
					...
					if((usr.ElementalPoints>=5)&&(usr.Mnin >= 15))
						usr.LearnJutsu("Raiton, Touei",60000,"Touei","Raiton Touei is a jutsu that allows you to temporarily focus your elemental chakra into quick bursts for instantaneous-like movement. 5 Elemental Points","Raiton",5)
					//	usr.ElementalPointDeduction = 5
						usr.Frozen=0
						return
					else
			...
			Garou
				...
				//		usr<<"You need 1 / 20 Lightning Mastery before you can learn this jutsu!";return
					if((usr.ElementalPoints >=3)&&(usr.Mnin >= 15))
						usr.LearnJutsu("Raiton; Rai no Garou",20000,"RairyuuGarou","Raiton; Rai no Garou is a technique where the user spins rapidly in a fierce tornado, attracting static energy which they can rip the opponent to shreds. 3 Elemental Points","Raiton",3)
					//	usr.ElementalPointDeduction = 3
						usr.Frozen = 0
						return
					else
			...
			OldGian
				...
				Click()
					...
					if((usr.ElementalPoints>=5)&&(usr.Mnin>=30))
						usr.LearnJutsu("Raiton: Impaling Electrocution",35000,"OldGian","Raiton: Impaling Electrocution is similar to the legendary Raiton: Gian Technique. The User creates a horizontal blast of Raiton Chakra that impales the enemy, causing continous damage and stunning the enemy. 5 Elemental Points","Raiton",5)
				//		usr.ElementalPointDeduction=5
						usr.Frozen=0
						return
					else
			...
			Tatsumaki
				...
				Click()
					...
					if((usr.ElementalPoints >=5)&&(usr.Mnin >= 37))
						usr.LearnJutsu("Raiton; Rairyuu no Tatsumaki",50000,"Tatsumaki","Raiton; Rairyuu No Tatsumaki is a jutsu where the user creates a strong dragon that is controllable in movement and damage. 5 Elemental Points","Raiton",5)
					//	usr.ElementalPointDeduction = 5
						usr.Frozen = 0
						return
					else
			...
			RaiYoroi
				...
				Click()
					...
					if(usr.RaitonKnowledge<50)
						usr<<"You need at least 50 Raiton Knowledge first!";return
					if((usr.ElementalPoints >=4)&&(usr.Mnin >= 22))
						usr.LearnJutsu("Raiton; Raigeki No Yoroi",30000,"Yoroi","Raiton; Raigeki No Yoroi is a powerful technique that absorbs all Raiton techniques shot at the user! 4 Elemental Points","Raiton",4)
					//	usr.ElementalPointDeduction = 4
						usr.Frozen = 0
						return
					else
			...
			Gian
				...
				Click()
					...
					if((usr.ElementalPoints >=7)&&(usr.Mnin >= 37))
						usr.LearnJutsu("Raiton; Gian",150000,"Gian","Raiton; Gian is an incredibly powerful technique which shoots a line of lightning from the user which zaps the target powerfully! 7 Elemental Points","Raiton",7)
			...
			Jibashi
				...
				Click()
					...
					if((usr.ElementalPoints >=4)&&(usr.Mnin >= 30))
						usr.LearnJutsu("Raiton; Jibashi",30000,"Jibashi","Raiton; Jibashi is a jutsu where eletrical pulses are shot at the designated target, shocking and stunning them! 4 Elemental Points","Raiton",4)
				//		usr.ElementalPointDeduction = 4
						usr.Frozen = 0
						return
					else
			...
			Hinoko
				...
				Click()
					...
					if((usr.ElementalPoints >=4)&&(usr.Mnin >= 30))
						usr.LearnJutsu("Raiton; Hinoko",45000,"Hinoko","Raiton; Hinoko is a fast electrical jutsu that utilizes shape and nature manipulation to form a vibrating blade of lightning that travels quickly and deals heavy damage to its targets, as well as temporarily numbing their bodies! 4 Elemental Points","Raiton",4)
				//		usr.ElementalPointDeduction = 4
						usr.Frozen = 0
						return
					else
			...
			Chidori
				...
				Click()
					...
					if((usr.ElementalPoints >=3)&&(usr.Mnin >= 15))
						usr.LearnJutsu("Chidori",20000,"Chidori","Chidori is a technique which generates so much chakra that it becomes visible as eletricity. A powerful move that can damage the opponent entirely! 3 Elemental Points","Ninjutsu",3)
				//		usr.ElementalPointDeduction = 3
						usr.Frozen = 0
						return
					else
			...
			Raikiri
				...
				Click()
					...
					if(usr.RaitonKnowledge<150)
						usr<<"You need at least 150 Raiton Knowledge first!";return
					if((usr.ElementalPoints >=5)&&(usr.Mnin >= 30))
						usr.LearnJutsu("Raikiri",50000,"Raikiri","Raikiri is a stronger alteration of Chidori, allowing more mobility for the user. 5 Elemental Points","Ninjutsu",5)
					//	usr.ElementalPointDeduction = 5
						usr.Frozen = 0
						return
					else
			...
			RaikiriWolf
				...
				Click()
					...
					if((usr.ElementalPoints >=5)&&(usr.Mnin >= 36))
						usr.LearnJutsu("Raikiri Wolf",50000,"RaikiriWolf","Raikiri Wolf is a strong technique utilizing Raikiri at the fullest chakra manipulation, manifesting a lightning wolf. 5 Elemental Points","Ninjutsu",5)
				//		usr.ElementalPointDeduction = 5
						usr.Frozen = 0
						return
					else
			...
			ChidoriSenbon
				...
				Click()
					...
					if((usr.ElementalPoints >=4)&&(usr.Mnin >= 20))
						usr.LearnJutsu("Chidori Senbon",20000,"ChidoriSenbon","Chidori Senbon is an alteration of Chidori. It uses shape manipulation to make a senbon using Chidori's natural properties. 4 Elemental Points","Ninjutsu",4)
					//	usr.ElementalPointDeduction = 4
						usr.Frozen = 0
						return
					else
			...
			ChidoriNagashi
				...
				Click()
					...
					if((usr.ElementalPoints >=5)&&(usr.Mnin >= 25))
						usr.LearnJutsu("Chidori Current",30000,"ChidoriNagashi","Chidori Current is an alteration of Chidori. It is a paralyzing current that is transferred through the body and outward. 5 Elemental Points","Ninjutsu",5)
				//		usr.ElementalPointDeduction = 5
						usr.Frozen = 0
						return
					else
			...
			ChidoriEisou
				...
				Click()
					...
					if((usr.ElementalPoints >=6)&&(usr.Mnin >= 30))
						usr.LearnJutsu("Chidori Spear",50000,"ChidoriEisou","Chidori Spear is an alteration of Chidori. It is an extending spear of lightning able to cut through anything and paralyze a body it peirces through the sheer electrical charge. 6 Elemental Points","Ninjutsu",6)
				//		usr.ElementalPointDeduction = 6
						usr.Frozen = 0
						return
					else
	...
	//					usr<<"You need 1 / 20 Earth Mastery first!";return
					if((usr.ElementalPoints>=3)&&(usr.Mnin >= 15))
						usr.LearnJutsu("Doton Doryo Dango",10000,"DoryoDango","Doton Doryo Dango is a technique in which the user uses their strength to pull a boulder from the ground and throw it with great force. 3 Elemental Points","Doton",3)
					//	usr.ElementalPointDeduction = 3
						usr.Frozen = 0
						return
					else
			...
			RockFist
				...
				Click()
					if((usr.ElementalPoints>=3))
						usr.LearnJutsu("Earth Style: Rock Fist",25000,"DotonKengan","Doton Rock Fist is a technique in which the user uses their strength to create a fist of pure Earth; truly damaging the opponent. 3 Elemental Points","Doton",3)
					//	usr.ElementalPointDeduction = 3
						usr.Frozen = 0
						return
					else
			...
			DotonSpear
				...
				Click()
					...
					if((usr.ElementalPoints>=4)&&(usr.Mnin >= 15))
						usr.LearnJutsu("Earth Style: Spear",25000,"DotonSpear","Doton Doryo Dango is a technique in which the user uses their strength to pull a boulder from the ground and throw it with great force. 4 Elemental Points","Doton",4)
					//	usr.ElementalPointDeduction = 4
						usr.Frozen = 0
						return
					else
			...
			DotonArmor
				...
				Click()
					if((usr.ElementalPoints>=4)&&(usr.Mnin >= 15))
						usr.LearnJutsu("Earth Style: Rock Armor",15000,"DotonArmor","Rock Armor is a technique in which the user manipulates their chakra to form a Rock Hard armor ontop of their body, protecting them from damage. 4 Elemental Points","Doton",4)
					//	usr.ElementalPointDeduction = 4
						usr.Frozen=0
						return
					else
			...
			KoukaArmor
				...
				Click()
					...
					if((usr.ElementalPoints>=5))
						usr.LearnJutsu("Earth Style: Rock Armor Advanced",40000,"KoukaArmor","Kouka Armor is a much more advanced and stronger version of Rock Armor.","Doton");return
			Doryuudan
			...
			//			usr<<"You need to know Taiga first!";return
					...
					if((usr.ElementalPoints>=4)&&(usr.Mnin >= 30))
						usr.LearnJutsu("Doton; Doryuudan No Jutsu",35000,"Doryuudan","Doton; Doryuudan No Jutsu is a strong jutsu that summons a dragon that shoots spikes from its mouth. 4 Elemental Points","Doton",4)
				//		usr.ElementalPointDeduction = 4
						usr.Frozen = 0
						return
					else
			...
			DotonDoseikiryuu
				...
				Click()
					...
					if((usr.ElementalPoints>=5)&&(usr.Mnin >= 34))
						usr.LearnJutsu("Doton; Earth and Stone Dragon",35000,"DotonDoseikiryuu","Doton; Earth and Stone Dragon is a strong jutsu that summons a dragon that will demolish the opponent, much Stronger of an impact than Doryuudan. 5 Elemental Points","Doton",5)
					//	usr.ElementalPointDeduction = 5
						usr.Frozen = 0
						return
					else
			...
			Doryuuheki
				...
				Click()
					...
					if(usr.DotonKnowledge<100)
						usr<<"You need at least 100 Doton Knowledge first!";return
					if((usr.ElementalPoints >=4)&&(usr.Mnin >= 20))
						usr.LearnJutsu("Doton; Doryuuheki No Jutsu",20000,"Doryuuheki","Doton; Doryuuheki is a jutsu that creates a strong wall of defence in front of you to block techniques. 4 Elemental Points","Doton",4)
				//		usr.ElementalPointDeduction = 4
						usr.Frozen = 0
						return
					else
			...
			//				return
					...
					if((usr.ElementalPoints>=5)&&(usr.Mnin >= 37))
						usr.LearnJutsu("Doton; Yomi Numa No Jutsu",50000,"YomiNuma","Doton; Yomi Numa produces a strong swamp that captures all who are within it. 5 Elemental Points","Doton",5)
				//		usr.ElementalPointDeduction = 5
						usr.Frozen = 0
						return
					else
			...
			DotonArijigoku
				...
				Click()
					...
					if((usr.ElementalPoints>=4)&&(usr.Mnin >= 35))
						usr.LearnJutsu("Antlion",50000,"DotonArijigoku","A jutsu which changes the properties of the earth to spiral inwards on a central point,forcing the opponent to struggle out, or face severe damage. 4 Elemental Points","Doton",4)
					//	usr.ElementalPointDeduction = 4
						usr.Frozen = 0
						return
					else
			...
			//				return
					...
					if((usr.ElementalPoints>=4)&&(usr.Mnin >= 32))
						usr.LearnJutsu("Rushing Swamp Waves",50000,"SwampBrambles","This jutsu is a more weaker version of its parent jutsu, Swamp of the Underworld. It's capable of capturing anyone who steps foot in it and slowing them down while dealing damage. 4 Elemental Points","Doton",4)
				//		usr.ElementalPointDeduction = 4
						usr.Frozen = 0
						return
					else
		...
		//				usr<<"You need 1 / 20 Earth Mastery first!";return
					if((usr.ElementalPoints >=3)&&(usr.Mnin >= 15))
						usr.LearnJutsu("Doton; Doruki Gaeshi",10000,"DorukiGaeshi","Doton; Doruki Gaeshi is a jutsu which places a earth wall of defence in front of you to protect you. 3 Elemental Points","Doton",3)
				//		usr.ElementalPointDeduction = 3
						usr.Frozen = 0
						return
					else
			...
			MoveStopper
				...
				Click()
					...
					if((usr.ElementalPoints > 4)&&(usr.Mnin >= 22))
						usr.LearnJutsu("Move Stopper",20000,"MoveStopper","Move Stopper is a earth technique that creates a barrier around the opponent trapping them from moving. A good catching technique. 4 Elemental Points","Doton",4)
					//	usr.ElementalPointDeduction = 4
						usr.Frozen = 0
						return
					else
			...
			Mole
				...
				Click()
					...
					if(usr.DotonKnowledge<50)
						usr<<"You need at least 50 Doton Knowledge first!";return
					if((usr.ElementalPoints>=4)&&(usr.Mnin >= 22))
						usr.LearnJutsu("Doton: Moguragakure No Jutsu",20000,"Moguragakure","Doton: Moguragakure No Jutsu is a jutsu where the user remains hidden deep underground which they can use to their advantage for a couple of other things they need. 4 Elemental Points","Doton",4)
					//	usr.ElementalPointDeduction = 4
						usr.Frozen = 0
						return
					else
			...
			TsuchiWana
				...
				Click()
					if((usr.ElementalPoints>=4)&&(usr.Mnin >= 22))
						usr.LearnJutsu("Doton; Tsuchi Wana",25000,"TsuchiWana","Doton; Tsuchi Wana is a technique that creates a field of rocks around the target, binding them. 4 Elemental Points","Doton",4)
					//	usr.ElementalPointDeduction = 4
						usr.Frozen = 0
						return
					else
			...
			Taiga
				...
				Click()
					if((usr.ElementalPoints >=2)&&(usr.Mnin >= 20))
						usr.LearnJutsu("Doton; Doryu Taiga",30000,"Taiga","Doton; Doryuu Taiga is a technique which creates a flow of river made out of mud, pushing the target away. 2 Elemental Points","Doton",2)
			...
			RisingEarthSpikes
				...
				Click()
					if((usr.ElementalPoints >=4)&&(usr.Mnin >= 25))
						usr.LearnJutsu("Doton;Rising Earth Spikes",15000,"RisingEarthSpikes","The user stomps the ground creating a series of spikes leading in a straight line. 4 Elemental Points","Doton",4)
					//	usr.ElementalPointDeduction = 4
						usr.Frozen = 0
						return
					else
			...
			RisingEarthPillars
				...
				Click()
					...
					if(usr.DotonKnowledge<100)
						usr<<"You need at least 100 Doton Knowledge first!";return
					if((usr.ElementalPoints >=5)&&(usr.Mnin >= 30))
						usr.LearnJutsu("Doton;Earth Pillars",30000,"RisingEarthPillars","Doton;Earth Pillars slowly creates a mass amount of Pillars around the User..that will easily penetrate enemies around them if caught....5 Elemental Points","Doton",5)
					//	usr.ElementalPointDeduction = 5
						usr.Frozen = 0
						return
					else
			...
			ColumnSpikes
				...
				Click()
					...
					if((usr.ElementalPoints >=5)&&(usr.Mnin >= 30))
						usr.LearnJutsu("Doton;Earth Column Spikes",30000,"ColumnSpikes","Doton;Earth Column Spikes creates four medium sized pillers which sourround the target and impale him/her repetetively..5 Elemental Points","Doton",5)
					//	usr.ElementalPointDeduction = 5
						usr.Frozen = 0
						return
					else
				...
				Click()
					...
					for(var/obj/SkillCards/TsuchiWana/P in usr.LearnedJutsus)
						if(P:Copied==1)
							usr<<"You used Sharingan Copy to learn this Jutsu! You need to know how to preform it originally!"
							return
					if(usr.DotonKnowledge<75)
						usr<<"You need at least 75 Doton Knowledge first!";return
					if((usr.ElementalPoints >=3)&&(usr.Mnin >= 36))
						usr.LearnJutsu("Doton: Dorou Domu",44000,"DorouDomu","Doton: Dorou Domu is a defensive earth sphere. It surrounds you and prevents almost any attack from penetrating. Unfortunately it is difficult to master and while in use you are unable to see outside. 3 Elemental Points","Doton",3)
					//	usr.ElementalPointDeduction = 5
						usr.Frozen = 0
						return
					else
			...
			DotonKakouSekkan
				...
				Click()
					...
					if(usr.DotonKnowledge<150)
						usr<<"You need at least 150 Doton Knowledge first!";return
					if((usr.ElementalPoints >=5)&&(usr.Mnin >= 30))
						usr.LearnJutsu("Doton: Descending Stone Coffin",44000,"DotonKakouSekkan","Doton: Descending Stone Coffin is a jutsu utilizing what you learned from Darou Domu. It surrounds the opponent and prevents almost any attack from penetrating. 5 Elemental Points","Doton",5)
					//	usr.ElementalPointDeduction = 5
						usr.Frozen = 0
						return
					else
			...
			DomuCrush
				...
				Click()
					...
					if((usr.ElementalPoints>=6)&&(usr.Mnin >= 36))
						usr.LearnJutsu("Doton; Earth Crush Jutsu",75000,"DorouDomuCrush","Doton; Earth Crush Jutsu is a strong jutsu that covers the enemy with a barrier of Earth that can crush over them for a killing blow. 6 Elemental Points","Doton",6)
					//	usr.ElementalPointDeduction = 6
						usr.Frozen = 0
						return
					else
			...
			WindTrick
				...
				Click()
					if((usr.ElementalPoints>4)&&(usr.Mnin >= 22))
						usr.LearnJutsu("Wind Trick",20000,"WindTrick","Wind Trick is a jutsu utilizing wind currents. It produces a strong turrent of wind that pushes back nearby targets and possible attackers, the wind current increase in size the more mastery of it. 4 Elemental Points","Fuuton",4)
					//	usr.ElementalPointDeduction = 4
						usr.Frozen = 0
						return
					else
			...
			SpinningWind
				...
				Click()
					if((usr.ElementalPoints >=3)&&(usr.Mnin >= 22))
						usr.LearnJutsu("Spinning Wind",10000,"SpinningWind","Spinning Wind is a technique that emits a small gust that leaves the target within a spinning wind for a small amount of time. 3 Elemental Points","Fuuton",3)
					//	usr.ElementalPointDeduction = 3
						usr.Frozen = 0
						return
					else
			...
			KazeDangan
				...
				//		usr<<"You need 1 / 20 Wind Mastery before you can learn this jutsu.";return
					if((usr.ElementalPoints >=3)&&(usr.Mnin >= 15))
						usr.LearnJutsu("Kaze Dangan No Jutsu",10000,"KazeDangan","Fuuton Kaze Dangan No Jutsu is a technique that shoots a fast paced bullet that hits the opponent with incredible force and pressure. 3 Elemental Points","Fuuton",3)
					//	usr.ElementalPointDeduction = 3
						usr.Frozen = 0
						return
					else
			...
			Reppushou
				...
				//		usr<<"You need 1 / 20 Wind Mastery before you can learn this jutsu.";return
					if((usr.ElementalPoints > 3)&&(usr.Mnin >= 15))
						usr.LearnJutsu("Fuuton Reppushou No Jutsu",10000,"Reppushou","Fuuton Reppushou No Jutsu is a jutsu that shoots a wave of wind that can shoot back the enemy or speed up things. 3 Elemental Points","Fuuton",3)
					//	usr.ElementalPointDeduction = 3
						usr.Frozen = 0
						return
					else
			...
			Daitoppa
				...
				Click()
					...
					if(usr.FuutonKnowledge<100)
						usr<<"You need at least 100 Fuuton Knowledge first!";return
					if((usr.ElementalPoints>=4)&&(usr.Mnin >= 16))
						usr.LearnJutsu("Fuuton Daitoppa No Jutsu",30000,"Daitoppa","Fuuton Daitoppa No jutsu is a technique that fires a gust of wind that shoots targets back. It can be used for combustion. 4 Elemental Points","Fuuton",4)
					//	usr.ElementalPointDeduction = 4
						usr.Frozen = 0
						return
					else
			...
			WhirlingTornado
				...
				Click()
					if(!(locate(/obj/SkillCards/DaiKamaitachi) in usr.LearnedJutsus))
						usr<<"You need to know Daikamaitchi first!";return
					...
					if((usr.ElementalPoints >=4)&&(usr.Mnin >= 30))
						usr.LearnJutsu("WhirlingTornado",25000,"WhirlingTornado","Summon a Whirling Vortex of violent wind to destroy objects in the surrounding area as well as buffet those nearby! 4 Elemental Points","Fuuton",4)
					//	usr.ElementalPointDeduction = 4
						usr.Frozen = 0
						return
					else
			...
			KazeKiri
				...
				Click()
					...
					if((usr.ElementalPoints >=4)&&(usr.Mnin >= 45))
						usr.LearnJutsu("Kaze Kiri No Jutsu",30000,"KazeKiri","Kaze Kiri No Jutsu is a technique that's very easily dodgable but incredibly powerful. It shoots a blade of wind that literally goes through the target, which can deal a lot of damage and go to each target. 4 Elemental Points","Fuuton",4)
					//	usr.ElementalPointDeduction = 4
						usr.Frozen = 0
						return
					else
			...
			KazeGai
				...
				Click()
					if(usr.FuutonKnowledge<400)
						usr<<"You need at least 400 Fuuton Knowledge first!";return
			...
			Renkuudan
				...
				Click()
					if(usr.FuutonKnowledge<300)
						usr<<"You need at least 300 Fuuton Knowledge first!";return
					if((usr.ElementalPoints >=7)&&(usr.Mnin >= 45))
						...
						if(!(locate(/obj/SkillCards/Daitoppa) in usr.LearnedJutsus))
							...
							usr.LearnJutsu("Fuuton Renkuudan No Jutsu",85000,"Renkuudan","Fuuton Renkuudan No Jutsu is a strong, powerful blast of wind that explodes on contact with the enemy! 7 Elemental Points","Fuuton",7)
					//		usr.ElementalPointDeduction = 7
							usr.Frozen = 0
							return
					else
		...
		//				usr<<"You need 1 / 20 Wind Mastery before you can learn this jutsu.";return
					if((usr.ElementalPoints > 3)&&(usr.Mnin >= 15))
						usr.LearnJutsu("VacuumWave",15000,"VacuumWave","A jutsu which causes the user to take a deep breath and spin around while exhaling, compressing the air that they release into a solitary blade of wind that can surround the user. 3 Elemental Points","Fuuton",3)
					//	usr.ElementalPointDeduction = 3
						usr.Frozen = 0
						return
					else
				...
				Click()
					...
					for(var/obj/SkillCards/VacuumWave/P in usr.LearnedJutsus)
						if(P:Copied==1)
							...
							return
					if((usr.ElementalPoints >=5)&&(usr.Mnin >= 30))
						usr.LearnJutsu("Vacuum Fuuton: Serial Waves",50000,"VacuumSerialWaves","A jutsu which creates a massive amount of blades of wind that are shot from the user at varying angles. 5 Elemental Points","Ninjutsu",5)
				//		usr.ElementalPointDeduction = 5
						usr.Frozen = 0
						return
					else
			...
			Yaiba
				...
				Click()
					if((usr.ElementalPoints>=4)&&(usr.Mnin >= 22))
						usr.LearnJutsu("Fuuton Kaze No Yaiba",20000,"Yaiba","Fuuton Kaze No Yaiba is a powerful technique that gives the user a wind sword that slices its victim. 4 Elemental Points","Fuuton",4)
				//		usr.ElementalPointDeduction = 4
						usr.Frozen = 0
						return
					else
			...
			DaiKamaitachi
				...
				Click()
					...
					if(usr.FuutonKnowledge<150)
						usr<<"You need at least 150 Fuuton Knowledge first!";return
					if((usr.ElementalPoints >=4)&&(usr.Mnin >= 30))
						usr.LearnJutsu("Giant WindCutter Jutsu",30000,"DaiKamaitachi","A jutsu that slices up the target’s body and is able to push away projectiles and people with mere ease. 4 Elemental Points","Fuuton",4)
				//		usr.ElementalPointDeduction = 4
						usr.Frozen = 0
						return
					else
			...
			Kakeami
				...
				Click()
					...
					if((usr.ElementalPoints >=5)&&(usr.Mnin >= 34))
						usr.LearnJutsu("Casting Wind Net",30000,"Kakeami","The user creates multiple narrow currents of wind that form a large net of sorts. These winds are sharp enough to slice through an opponent, inflicting severe injuries to them in a criss-cross pattern. 5 Elemental Points","Fuuton",5)
				//		usr.ElementalPointDeduction = 5
						usr.Frozen = 0
						return
					else
			...
			VacuumCannon
				...
				Click()
					if((usr.ElementalPoints >=3)&&(usr.Mnin >= 30))
						usr.LearnJutsu("Vacuum Cannon",30000,"VacuumCannon","The user shoots small, powerful and near-invisible blasts of wind. 3 Elemental Points","Fuuton",3)
				//		usr.ElementalPointDeduction = 3
						usr.Frozen = 0
						return
					else
			...
			VacuumSphere
				...
				Click()
					...
					if(usr.FuutonKnowledge<150)
						usr<<"You need at least 150 Fuuton Knowledge first!";return
					if((usr.ElementalPoints >=5)&&(usr.Mnin >= 30))
						usr.LearnJutsu("Vacuum Sphere",30000,"VacuumSphere","Wind Release: Vacuum Sphere is a technique that shoots many bullet size wind projectile that have the capabilities of piercing an enemy. It shoots incredibly fast making it very lethal. 5 Elemental Points","Fuuton",5)
			//			usr.ElementalPointDeduction = 5
						usr.Frozen = 0
						return
					else
			...
			VacuumGreatSphere
				...
				Click()
					...
					if(usr.FuutonKnowledge<250)
						usr<<"You need at least 250 Fuuton Knowledge first!";return
					if((usr.ElementalPoints >=5)&&(usr.Mnin >= 30))
						usr.LearnJutsu("Vacuum Great Sphere",30000,"VacuumGreatSphere","Wind Release: Vacuum Great Sphere is a technique that shoots many bullet size wind projectile that have the capabilities of piercing an enemy. It shoots incredibly fast making it very lethal; much stronger then the Average Vacuum Sphere. 5 Elemental Points","Fuuton",5)
				//		usr.ElementalPointDeduction = 5
						usr.Frozen = 0
						return
					else
			...
			//			usr<<"You need 1 / 20 Water Mastery first!";return
					if((usr.ElementalPoints >=3)&&(usr.Mnin >= 15))
						usr.LearnJutsu("Suiton Mizurappa No Jutsu",10000,"Mizurappa","Suiton Mizurappa No Jutsu is a beam-water technique that shoots water out from the user's mouth with great force and pressure. 3 Elemental Points","Suiton",3)
					//	usr.ElementalPointDeduction = 3
						usr.Frozen = 0
						return
					else
		...
		//				usr<<"You need 1 / 20 Water Mastery first!";return
					if((usr.ElementalPoints >=4)&&(usr.Mnin >= 15))
						usr.LearnJutsu("Suiton Bubble Capture",15000,"BubbleCapture","Suiton BubbleCapture is a Jutsu that allows the user to create a bubble that can entrap an opponent and slowly suffocate them within it when the air runs out. 4 Elemental Points","Suiton",4)
				//		usr.ElementalPointDeduction = 4
						usr.Frozen = 0
						return
					else
			...
			BubbleBeam
				...
				Click()
					...
					if((usr.ElementalPoints >=5)&&(usr.Mnin >= 15))
						usr.LearnJutsu("Suiton Bubble Beam",15000,"BubbleBeam","Suiton BubbleBeam is a Jutsu that allows the user to create a large amount of Bubbles in the shape of a V to rush at the opponent, slamming them and pushing them back severly when hit. 5 Elemental Points","Suiton",5)
					//	usr.ElementalPointDeduction = 5
						usr.Frozen = 0
						return
					else
			...
			Syrup
				...
				Click()
					...
					if((usr.ElementalPoints >=4)&&(usr.Mnin >= 22))
						usr.LearnJutsu("Mizuame Nabara No Jutsu",30000,"MizuameNabara","Mizuame Nabara No Jutsu is a jutsu that creates sticky syrup around the user making those stuck in it immobile. 4 Elemental Points","Suiton",4)
				//		usr.ElementalPointDeduction = 4
						usr.Frozen = 0
						return
					else
			...
			Suigadan
				...
				Click()
					...
					if((usr.ElementalPoints >=4)&&(usr.Mnin >= 30))
						usr.LearnJutsu("Suigadan",30000,"Suigadan","Suigdan is a powerful water jutsu that sends four spiraling tendrills of water to strike an opponent from underneath. This attack must be well timed because it cannot be preformed if the target is not standing on water. 4 Elemental Points","Suiton",4)
					//	usr.ElementalPointDeduction = 4
						usr.Frozen = 0
						return
					else
			...
			Suiryuudan
				...
				Click()
					...
					if((usr.ElementalPoints >=5)&&(usr.Mnin >= 30))
						usr.LearnJutsu("Suiton Suiryuudan No Jutsu",35000,"Suiryuudan","Suiton Suiryuudan No Jutsu is a jutsu which summons a dragon made out of water from water to attack a targetted player. 5 Elemental Points","Suiton",5)
					//	usr.ElementalPointDeduction = 5
						usr.Frozen = 0
						return
					else
			...
			Suikoudan
				...
				Click()
					...
					if((usr.ElementalPoints>=5)&&(usr.Mnin >= 30))
						usr.LearnJutsu("Suiton Suikoudan No Jutsu",40000,"Suikoudan","Suiton Suikoudan No Jutsu is a jutsu whcih summons a shark made out of water from water to attack a targetted player. 5 Elemental Points","Suiton",5)
					//	usr.ElementalPointDeduction = 5
						usr.Frozen = 0
						return
					else
			...
			Daikoudan
				...
				Click()
					...
					if(usr.SuitonKnowledge<250)
						usr<<"You need at least 250 Suiton Knowledge first!";return
					if((usr.ElementalPoints>=4)&&(usr.Mnin >= 30))
						usr.LearnJutsu("Suiton Daikoudan No Jutsu",40000,"Daikoudan","Suiton Daikoudan No Jutsu is a jutsu whcih summons an even larger shark made out of water from water to attack any victim in sight. 4 Elemental Points","Suiton",4)
					//	usr.ElementalPointDeduction = 4
						usr.Frozen = 0
						return
					else
			...
			Senjikizame
				...
				Click()
					...
					if(usr.SuitonKnowledge<250)
						usr<<"You need at least 250 Suiton Knowledge first!";return
					if((usr.ElementalPoints>=4)&&(usr.Mnin >= 30))
						usr.LearnJutsu("Suiton Thousand Man Eating Sharks",40000,"Senjikizame","Suiton Thousand man eating sharks creates a large amount of sharks to devour any enemies in their path. 4 Elemental Points","Suiton",4)
					//	usr.ElementalPointDeduction = 4
						usr.Frozen = 0
						return
					else
			...
			Daibakufu
				...
				Click()
					...
					if((usr.ElementalPoints >=4)&&(usr.Mnin >= 30))
						usr.LearnJutsu("Suiton Daibakufu No Jutsu",25000,"Daibakufu","Suiton Daibakufu No Jutsu is a jutsu which shoots a large torrent which keeps the enemy in repetive damage. 4 Elemental Points","Suiton",4)
					//	usr.ElementalPointDeduction = 4
						usr.Frozen = 0
						return
					else
			...
			Daibakuryuu
				...
				Click()
					...
					if(usr.SuitonKnowledge<150)
						usr<<"You need at least 150 Suiton Knowledge first!";return
					if((usr.ElementalPoints >=5)&&(usr.Mnin >= 30))
						usr.LearnJutsu("Suiton Daibakuryuu",25000,"Daibakuryuu","Suiton Daibakuryuu is a jutsu that allows the opponent to create a strong tide of water, sucking in the opponent and damaging them. 5 Elemental Points","Suiton",5)
				//		usr.ElementalPointDeduction = 5
						usr.Frozen = 0
						return
					else
			...
			BakuSuishouha
				...
				Click()
					...
					if((usr.ElementalPoints >=5)&&(usr.Mnin >= 35))
						usr.LearnJutsu("Suiton Baku Suishouha No Jutsu",50000,"BakuSuishouha","Suiton Baku Suishou No jutsu is a technique that shoots a large torrent wave that covers the field with water that can be used to the user's advantage. 5 Elemental Points","Suiton",5)
				//		usr.ElementalPointDeduction = 5
						usr.Frozen = 0
						return
					else
			...
			//			usr<<"You need 1 / 20 Water Mastery first!";return
					if((usr.ElementalPoints>=2)&&(usr.Mnin >= 22))
						usr.LearnJutsu("Water Creation Jutsu",20000,"WaterCreation","Water Creation Jutsu is a jutsu that creates a supply of water allowing players to use. 2 Elemental Points","Suiton",2)
				//		usr.ElementalPointDeduction = 2
						usr.Frozen = 0
						return
					else
			...
			Suijinheki
				...
				Click()
					...
					if(usr.SuitonKnowledge<150)
						usr<<"You need at least 150 Suiton Knowledge first!";return
					if((usr.ElementalPoints>=3)&&(usr.Mnin >= 22))
						usr.LearnJutsu("Suiton Suijinheki No Jutsu",5000,"Suijinheki","Suiton Suijinheki No Jutsu is a jutsu that creates a quick block infront of the user to block incoming techniques. 3 Elemental Points","Suiton",3)
				//		usr.ElementalPointDeduction = 3
						usr.Frozen = 0
						return
					else
			...
			Teppoudama
				...
				Click()
					if((usr.ElementalPoints>=4)&&(usr.Mnin >= 30))
						usr.LearnJutsu("Suiton Teppoudama",25000,"Teppoudama","Suiton Teppoudama is a strong and fast water bullet projectile that when it colldies with something bursts with a large shockwave knocking anyone nearby away with great force. 4 Elemental Points","Suiton",4)
				//		usr.ElementalPointDeduction = 4
						usr.Frozen = 0
						return
					else
				...
				Click()
					...
					for(var/obj/SkillCards/Teppoudama/P in usr.LearnedJutsus)
						if(P:Copied==1)
							...
							return
					if((usr.ElementalPoints>=4)&&(usr.Mnin>=30))
						usr.LearnJutsu("Suiton Water Gun",20000,"MizuTeppou","Suiton Water Gun is an advanced jutsu that shoots a powerful blast of water at the opponent, damaging them fiercely. 4 Elemental Points","Suiton",4)
				//		usr.ElementalPointDeduction=4
						usr.Frozen=0
						return
					else
				...
				Click()
					...
					for(var/obj/SkillCards/Suikoudan/P in usr.LearnedJutsus)
						if(P:Copied==1)
							...
							return
					if((usr.ElementalPoints>=4)&&(usr.Mnin>=30))
						usr.LearnJutsu("Suiton: Water Clones",35000,"MizuBunshin","Suiton Water Clones is a jutsu that enables the ninja to create clones from water. 5 Elemental Points","Suiton",5)
				//		usr.ElementalPointDeduction=5
						usr.Frozen=0
						return
					else
			...
			WaterPrison
				...
				Click()
					...
					if((usr.ElementalPoints >=4)&&(usr.Mnin >= 45))
						usr.LearnJutsu("Suiton Suirou",50000,"WaterPrison","Is a suiton technique that engulfs an opponent with a barrier freezing both user and victim. 4 Elemental Points","Suiton",4)
					//	usr.ElementalPointDeduction = 4
						usr.Frozen = 0
						return
					else
					...
					if(!(locate(/obj/SkillCards/Shousen) in usr.LearnedJutsus))
						if(usr.GenSkill+(usr.ChakraC/25)>=14)
							if(usr.NinjutsuKnowledge>150)
								usr.LearnJutsu("Heal",50000,"Shousen","Shousen is a medical Jutsu that enables one to heal someone by utilizing their chakra.","Ninjutsu")
							else
								usr<<"You need more Ninjutsu knowledge(150)!";return
						...
						if(!(locate(/obj/SkillCards/Chikatsu) in usr.LearnedJutsus))
							if(usr.GenSkill+(usr.ChakraC/25)>=17)
								if(usr.NinjutsuKnowledge>250)
									...
									usr<<"You need more Ninjutsu knowledge(250)!";return
						...
						if(!(locate(/obj/SkillCards/SelfHeal) in usr.LearnedJutsus))
							if(usr.GenSkill+(usr.ChakraC/25)>=20)
								if(usr.NinjutsuKnowledge>500)
									...
									usr<<"You need more Ninjutsu knowledge(500)!";return
					...
					if(!(locate(/obj/SkillCards/Oukashou) in usr.LearnedJutsus))
						if(usr.GenSkill+(usr.ChakraC/25)>=18)
							if(usr.NinjutsuKnowledge>300)
								...
								usr<<"You need more Ninjutsu knowledge(300)!";return
						else if(usr.GenSkill<15)
							usr<<"You don't have enough Control with your chakra to learn this Jutsu..";return
					else
						usr<<"You have already learned [src]!";return
					...
					if(!(locate(/obj/SkillCards/PoisonFog) in usr.LearnedJutsus))
						if(usr.GenSkill+(usr.ChakraC/25)>=15)
							if(usr.NinjutsuKnowledge>100)
								...
								usr<<"You need more Ninjutsu knowledge(100)!";return
						else if(usr.GenSkill<12)
							usr<<"You don't have enough Control with your chakra to learn this Jutsu..";return
					else
						usr<<"You have already learned [src]!";return
						...
						if(usr.GenSkill+(usr.ChakraC/25)>=16)
							if(usr.NinjutsuKnowledge>300)
								if(usr.PoisonKnowledge>50)
									...
									usr<<"You need more Poison Knowledge to learn this technique(50)!";return
							else
								usr<<"You need more ninjutsu knowledge(300)!";return
					...
					if(locate(/obj/SkillCards/PoisonMist) in usr.LearnedJutsus)
						if(usr.GenSkill+(usr.ChakraC/25)>18)
							if(usr.NinjutsuKnowledge>350)
								...
								usr<<"You need more Ninjutsu Knowledge(350)!";return
						...
						if(!(locate(/obj/SkillCards/ChakraMode) in usr.LearnedJutsus))
							if(usr.GenSkill+(usr.ChakraC/25)>=28)
								if(usr.NinjutsuKnowledge>750)
									...
									usr<<"You need more Ninjutsu knowledge(750)!";return
...
//
		...
		KekkaiDrain
			...
			Click()
				usr<<sound('SFX/click1.wav',0)
				usr.LearnJutsu("Kikkaichu Drain",15000,"KikkaichuDrain","Kikkaichu Drain is a technique utilizing the Female bugs and the Kikkaichu's ability to drain chakra, letting them drain the chakra of people with female bugs on them.","Ninjutsu")
		KekkaiTracking
			name="Kekkai Tracking"
			icon_state="KekkaiTracking"
			Click()
				usr<<sound('SFX/click1.wav',0)
				usr.LearnJutsu("Kikkaichu Tracking",10000,"KonchuuTracking","Kikkaichu Tracking is a technique utilizing an Aburame's Female bugs, allowing theAburame to track down these females and find the location of an enemy.","Ninjutsu")
///////////////////////////////////////////////Sarutobi Clan///////////////////////////////////////////////////////////////////
		WillOfFire
			name="Will Of Fire"
		...
		SharinganOneTwoThree
			...
			Click()
				...
				if(usr.SharinganMastery>=91)
					alert(usr, "Your Sharingan has reach maturity. You are now a fully fledged Uchiha, but are there even further heights to reach?","[src.name]")
				else
					alert(usr,"Sharingans that reach maturity of three tomoe have the ability to cast powerful genjutsu, see through and counter jutsus, and even copy them to an extent.","[src.name]")
		SharCopy
			name="Sharingan Copy"
			icon_state="SharCopy"
			Click()
				usr<<sound('SFX/click1.wav',0)
				if(usr.SharinganMastery<91)
					usr<<"You need more Sharingan Mastery(91).";return
		...
		GenjutsuCounter
			...
			Click()
				...
				if(usr.GenjutsuKnowledge<100)
					usr<<"You need more Genjutsu Knowledge(100).";return
...
/////////////////////////////////////
		ShikyakuControl
			...
			icon_state="Canine"
			var/price="Through moving around while in Shikyaku(Four legged technique)"
			var/maxprice=10
			Click()
				usr<<sound('SFX/click1.wav',0)
				alert(usr, "[src] increases mastery over your ability to properly use Shikyaku and eventually being able to move near light speed with enough practice and mastery. How to Train it: [price] [usr.ShikyakuControl]/[maxprice]","[src.name]")//,,,,0,"400x150",,style)
		Companionship
			name="Companionship"
			icon_state="Training"
			var/price="Through training, helping and spending time with your dog."
			var/maxprice="No Limit"
			Click()
				usr<<sound('SFX/click1.wav',0)
			//	var/style = "<style>BODY {margin:0;font:arial;background:black;\
			//		color:white;}</style>"
				alert(usr, "[src] is something members of the Inuzuka Clan take seriousy, rather than just claiming dogs are tools for war and battle. Befriending your dog will allow a much more cooperative canine and better tactics and decisions in battle.  How to Train it: [price] [usr.DogCompanionship]/[maxprice]","[src.name]")//,,,,0,"400x150",,style)
		Training
			name="Training"
			icon_state="Training"
...
//////////////////////
		JintonIntensity
			name="Jinton: Intensity"
			icon_state="JintonIntensity"
			var/price="By using your jinton based techniques."
			var/maxprice=5
			Click()
				usr<<sound('SFX/click1.wav',0)
				alert(usr, "[src] is a passive that allows you to increase your damage done by Jinton based techniques. How to Train it: [price] [usr.JintonIntensity]/[maxprice]","[src.name]")//,,,,0,"400x150",,style)
		JintonControl
			name="Jinton: Control"
			icon_state="JintonControl"
			var/price="By using your jinton based techniques."
			var/maxprice=5
			Click()
				usr<<sound('SFX/click1.wav',0)
				alert(usr, "[src] is a passive that allows you to lower the chakra costs of Jinton based techniques. How to Train it: [price] [usr.JintonControl]/[maxprice]","[src.name]")//,,,,0,"400x150",,style)
		...
		Erosion
			...
			Click()
				...
				if(usr.SandMastery<50)
					usr<<"You need more Sand Mastery(50).";return
...
//////////////////Hoshigaki
		...
		SamehadeReturn
			...
			Click()
				usr<<sound('SFX/click1.wav',0)
				usr.LearnJutsu("Samehade Return",15000,"SamehadeReturn","A technique which allows the Hoshigaki to return the Samehade \
...
turf
	Skilltree/Passives/TaijutsuPassives
		...
		Buff
			name="Buff"
			icon_state="Buff"
			var/price="Every time hit by a critical attack, blocking a devastating attack, or being dealt a devastating attack all give it a chance of increasing."
			var/maxprice="No Limit"
			Click()
				usr<<sound('SFX/click1.wav',0)
		...
		Focus
			name="Focus"
			icon_state="Focus"
			var/price="Every time you perform a critical attack it has a chance of increasing. When you get hit it also has a chance of increasing. Keeping your eyes on your opponent when they attempt to make you lose target has a chance of increasing this passive."
			var/maxprice="No Limit"
			Click()
				usr<<sound('SFX/click1.wav',0)
				if(usr.TaijutsuMastery<1)
					...
					return
				alert(usr, "[src] increases the chance of getting a critical hit. It also increases the user's Focus against other attacks, helping in other things, such as increasing your ability to focus on your target. How to Train it: [price] [usr.Focus]/[maxprice]","[src.name]")//,,,,0,"400x150",,style)
				if(usr.StartingPoints>=1&&!usr.clicking)
					usr.clicking=1
					switch(input(usr,"Would you like to raise this passive using your left over Stat Points?") in list ("No","Yes"))
						if("Yes")
					...
					if("Get")
						...
						//		color:white;}</style>"
							...
							if(usr.TaijutsuKnowledge<15)
								usr<<"You need more Taijutsu Knowledge(15).";return
					...
					if("Get")
						...
						//	var/style = "<style>BODY {margin:0;font:arial;background:black;\
							...
							if(usr.TaijutsuKnowledge<40)
								usr<<"You need more Taijutsu Knowledge(40).";return
					...
					if("Get")
						...
						//	var/style = "<style>BODY {margin:0;font:arial;background:black;\
							...
							if(usr.TaijutsuKnowledge<100)
								usr<<"You need more Taijutsu Knowledge(100).";return
					...
					if("Get")
						...
						//		color:white;}</style>"
							...
							if(usr.TaijutsuKnowledge<165)
								usr<<"You need more Taijutsu Knowledge(165).";return
	...
	//					alert("[src] .","[src]")
					...
					//			color:white;}</style>"
							...
							if(usr.TaijutsuKnowledge<200)
								usr<<"You need more Taijutsu Knowledge(200).";return
	...
	//					alert("[src] .","[src]")
					...
					//			color:white;}</style>"
							...
							if(usr.TaijutsuKnowledge<280)
								usr<<"You need more Taijutsu Knowledge(280).";return
...
*/
		...
		HandsealAgility
			...
			var/price="By reaching 100 Ninjutsu Knowledge"//raising it with your leftover Starting Points."
			var/maxprice=1
			var/Reached100=0
			Click()
				usr<<sound('SFX/click1.wav',0)
				if(usr.NinjutsuKnowledge<100)
					usr.HandsealAgilityActive=0;
					alert(usr,"This passive is unavailable to you at this time. You need 3 Handseal Mastery before you can view this passive.")
					return
				if(usr.HandsealsMastery<3)
					...
					return
				if(usr.NinjutsuKnowledge>=100)
					usr.HandsealAgilityActive=1
				alert(usr, "[src] allows the user to run while performing handseals. How to Train it: [price] [usr.HandsealAgilityActive]/[maxprice]")
...
turf
	Skilltree/Passives/GenjutsuPassives
		...
		GenjutsuKnowledge
			...
			var/price="By performing genjutsu techniques. Succumbing to the effects of other Genjutsu Techniques also gives a slight chance to increase this."
			var/maxprice="No Limit"
			Click()
				usr<<sound('SFX/click1.wav',0)
			//	var/style = "<style>BODY {margin:0;font:arial;background:black;\
