...
obj/Effects
	...
	ZMash
		icon_state="Z"
	LeftArrowMash
		icon_state="Left Arrow"
	RightArrowMash
		icon_state="Right Arrow"
	AvailableQuest
		icon_state="AvailableQuest"
//Blood Effects:
proc/Blood(dx,dy,dz)
	spawn()
...
mob/proc/Bloody()
	...
	//	o.Owner=src
		...
		spawn(9)
			...
			x.icon='Blood.dmi'
			x.Owner=src
//			world<<"The blood belongs to [x.Owner]"
			var/v=rand(1,7)
			x.pixel_x=rand(-8,8)
			x.pixel_y=rand(-8,8)
			x.icon_state="l[v]"
...
mob/proc/RemoveUchihaAura()
	...
	for(var/client/CC in world)
		for(var/image/x in CC.images)
			spawn()
				if(x&&(x.layer==FLY_LAYER+1)&&x.loc==src)
					del(x)
			...
			if(M.client&&(M in OnlinePlayers)&&M!=src&&M.z==src.z&&M.Auraoff!=1 || M.HasAura)
				spawn()
					...
					if(M.ChakraColorR==100&&M.ChakraColorG==100&&M.ChakraColorB==255)
						...
						I=image('Icons/Jutsus/ChakraAuraPink.dmi',M,"",FLY_LAYER+1)
					else
						if(M.Clan=="Kyomou")
	...
	if(src&&src.client&&OnOff==0)//was just onoff remove the src if needed
		if(src in OnlinePlayers)
			for(var/image/x in src.client.images)
				spawn(4)
					if(x&&(x.layer==FLY_LAYER+9||x.icon=='Icons/Jutsus/ChakraAuraBlue.dmi'||x.icon=='Icons/Jutsus/ChakraAuraRed.dmi'||x.icon=='Icons/Jutsus/ChakraAuraPink.dmi'))
						del(x)
					if(!src)
						return
...
obj/Jutsu
	Explosion
		...
		dir=NORTH
		New()
			spawn() src.SoundEngine('SFX/Bang.wav')
			..()
				...
				for(var/mob/M in oview(1,src))
					spawn()
						if(M.SensoryRange>=5&&M.ByakuganMastery>1000&&!M.aflagged&&!M.Frozen&&M.FrozenBind==""&&prob(40))
							...
							var/mob/O=src.Owner
							var/damage=M.maxstamina*(0.015*(O.ExplosiveMastery/15))
							var/damage2=M.maxhealth*(0.014*(O.ExplosiveMastery/15))
							if(M.Guarding)
								damage/=10
								damage2/=3
							if(M.Mogu)
...
mob/proc/SoundEngine(Sound,Range)//BaseRange=10
	set background=1
	Range=2
	for(var/mob/player/X in range(Range,src))
		if(X.ckey!=null)//&&src.client)
			var/sound/A=sound(Sound)//,0,0,0)
			A.x=src.x-X.x;A.y=src.y-X.y
			A.falloff=2
			spawn()
				X.client.sound_system.PlaySound(A)
		//	X<<A
...
obj/proc/SoundEngine2(Sound)//Range,volume2)//BaseRange=10
	...
	for(var/mob/player/X in range(Range,src))
		if(X.ckey!=null)
			...
			A.falloff=2
			spawn()
				X.client.sound_system.PlaySound(A,src)
//		X<<A
	sleep(1)
...
//	var/list/L = list()
	for(var/mob/player/X in range(Range,src))
		if(X.ckey!=null)
//		var/sound/A=sound(Sound,0,0)
//		A.x=src.x-X.x;A.y=src.y-X.y
//		A.falloff=2
			var/sound/A=sound(Sound)//,0,0)//,volume=40)
			spawn()
				X.client.sound_system.PlaySound(A)
//		X<<A
//	sleep(1)
...
//	while(countb<10)//Original as of 9/22
...
mob/proc/NewDamage(Damage,WhereItHits,WhoDidIt,JutsuName,MessageColor="gray",JutsuType)//DirectArmorHit) JutsuType="Ninjutsu" or Tai/Gen/Ken/Katon/etc.
	...
	//if(src.deathcount>0.5)
		//src.DALastAttacker=WhoDidIt
		//if(src.DATimer< 15)//if your Da timer is below 30
			//src.DATimer = 15//it sets your DA timer to 30
			//if(!DAList.Find(src.key))//Looks for the key in the list
				//DAList.Add(src.key)//and adds if it its not there
	if(WhoDidIt)
		if(WhoDidIt:client)
			if(WhoDidIt:inMei)
//	if(WhoDidIt:client&&WhoDidIt:inMei)
				view(10,WhoDidIt)<<"[WhoDidIt]'s cloaking jutsu has been undone!"
		...
		if(src.ConversusinEye==1)
			if(src.ConversusinMastery<=100)
				if(prob(15))
					src<<"You dodge the attack."
					return
			else if(src.ConversusinMastery>100&&src.ConversusinMastery<=250)
				if(prob(18))
					src<<"You dodge the attack."
					return
			else if(src.ConversusinMastery>250)
				if(prob(20))
					src<<"The strength of your eyes cause you to dodge the attack."
					return
		if(src.ConversusinThirdStage==1)
			if(prob(30))
				src<<"The Attack phases right through you...."
				return
	if(src.SatouDrunk)
			...
			if(src.stamina>src.maxstamina*0.45)
				if(WhereItHits=="Health")
					...
					if(prob(7))
						...
						if(src.Drunk>=10)
							src.Drunk=10
					src<<"You're so drunk you don't feel any pain done to your organs!"
...
//		return//Just added 12/11/2012
	...
	if(src.IndigoBlock==1&&src.Guarding||src.PaperStyleDance==1&&src.Guarding||src.PaperStyleDance==2)
		if(src.IndigoBlock)
			...
			src.chakra-=Damage
		src<<"You turn yourself to paper to avoid damage!";
		flick("zan",src);return
	if(src.SusanooBones)
		src.SusanooHealth -= Damage
		src.chakra-=(Damage/2)
		Damage=(Damage/2)
		src<<"Your susanoo absorbs some of the damage"
		SusanooDeleteCheck()
	if(src.SusanooSkeleton)
		src.SusanooHealth -= Damage
		src.chakra-=(Damage/1.5)
		Damage=(Damage/4)
		src<<"Your susanoo absorbs some of the damage"
		SusanooDeleteCheck()
//	if(src.SusanooIn)
	if(src.SusanooPerfect)
		src.SusanooHealth -= Damage
...
//	if(src in WhoDidIt:Allies){Damage*=0.50}
	...
	if(WhoDidIt:Trait3=="Perseverance"){Damage*=0.70}
	if(src.ARank=="Combatant"){Damage*=0.85}
	if(WhoDidIt:ARank=="Combatant"){Damage*=1.07}
	else if(WhoDidIt:ARank=="Leader"||WhoDidIt:ARank=="Co-Leader"){Damage*=1.13}
	if(src.Trait=="Tough"){Damage*=0.85}
	if(src.SageMode=="Toad"){Damage*=0.90}
	if(src.Trait3=="Perseverance"){Damage*=0.70}
	if(WhoDidIt:SecondName=="Uzumaki"){Damage*=0.95}
//	if(WhoDidIt:PowerScrollBoost==1){Damage*=1.30}.
	if(WhereItHits=="Health")
		if(src.BoneCorpseRank=="CoLeader"){Damage*=0.95}
		if(src.BoneClubLeader){Damage*=0.85}
		if(src.GateIn=="Wound"){Damage*=0.75}
		else if(src.GateIn=="Limit"){Damage*=0.5}
		else if(src.GateIn=="View"){Damage*=0.3}
		else if(src.Trait=="Hard-Worker"){Damage*=1.1}
		else if(src.SecondName=="Momochi"){Damage*=1.1}
		else if(src.SecondName=="Sarutobi"){Damage*=1.15}
		...
		if(JutsuType=="Heat")//To help out Katon a bit
			ArmorAddUpForM/=2
		else if(JutsuType=="atomic")
			ArmorAddUpForM=0
		Damage-=(Damage*((ArmorAddUpForM)*0.01))  //Armor decreases damage done to the body.

		if(B&&src.BloodLink==1&&B in view(10,src))
			B.health-=Damage
			src<<"The Damage has been transferred to your BloodPartner [B] instead!"
			B<<"You've taken the Damage for [src]!"
			B.Death(WhoDidIt)
		else
			if(!src.knockedout)
				src.health-=Damage
			if(src.JashinMode)
//				if(C in oview(20,src)
				C.health-=Damage*rand(0.70,0.90)
				C.Death(WhoDidIt)

			if(JutsuName!=null)
				src<<output("<font color=[MessageColor]>You were hit by the [JutsuName] for [Damage] vitality damage! (Protected from [100-(round(Damage/StartDamage)*100)]%)</font>","Attack")
				oview(10,src)<<output("<font color=[MessageColor]>[src] was hit by the [JutsuName] for [Damage] vitality damage!</font>","Attack")
			else
				if(MessageColor!="gray")
					src<<output("<font color=[MessageColor]>You were hit for [Damage] vitality damage! (Protected from [100-(round(Damage/StartDamage)*100)]%)</font>","Attack")
					oview(10,src)<<output("<font color=[MessageColor]>[src] was hit for [Damage] vitality damage!</font>","Attack")
				else
		...
		if(Damage>1400-round(((A.Focus/2)-src.Buff))&&!src.knockedout&&!src.Guarding)
			if(src.Trait2=="Iron Will")
				...
				src.deathcount+=0.75
		//
	else if(WhereItHits=="Stamina")
		if(istype(src,/mob/IndigoRingBarrier))
			src.health-=Damage
			src.Death(WhoDidIt)
			return
		if(istype(src,/mob/Bugs/Kekkai)||istype(src,/mob/Bugs/Swarm)||istype(src,/mob/Bugs/BugShield)||istype(src,/mob/Bugs/SpecialSwarm)||istype(src,/mob/Sand/SunaNoTate)||istype(src,/mob/pet))
			src.health-=Damage
			src.Death(WhoDidIt)
			return
		...
		if(src.GateIn=="Wound")
			Damage*=0.8
		else if(src.GateIn=="Limit")
			Damage*=0.6
		else if(src.GateIn=="View")
			Damage*=0.5
		...
		if(B&&src.BloodLink==1&&(locate(B in view(10,src))))
			...
			src.stamina-=Damage
			if(src.JashinMode)
		//		Damage*=rand(0.10,0.40)
				C.stamina-=Damage*rand(0.70,0.90)
				spawn()
					C.Death(WhoDidIt)
			...
			if(JutsuName!=null)
				src<<output("<font color=[MessageColor]>You were hit by the [JutsuName] for [Damage] stamina damage! (Protected from [(ArmorAddupForM*2)] damage)</font>","Attack")
				oview(10,src)<<output("<font color=[MessageColor]>[src] was hit by the [JutsuName] for [Damage] stamina damage!</font>","Attack")
			else
				if(MessageColor!="gray")
					src<<output("<font color=[MessageColor]>You were hit for [Damage] stamina damage! (Protected from [(ArmorAddupForM*2)] damage)</font>","Attack")
					oview(10,src)<<output("<font color=[MessageColor]>[src] was hit for [Damage] stamina damage!</font>","Attack")
				else
					..() //No output, so we can redo the Damage Procs as we please and still put the game up.
		if(Damage>900-round((A.Focus/2)-src.Buff)&&!src.knockedout&&src.Guarding)
			if(src.Buff<1000)
				if(prob(5))
					src.Buff+=pick(0.1,0.2,0.3,0.4)
			if(Damage>1500)
				if(src.Buff<1000)
					if(prob(10))
						src.Buff+=pick(0.1,0.2,0.3,0.4)
...
atom/movable/proc/DamageProc(Damage,WhereItHits,WhoDidIt,JutsuName,MessageColor="gray",JutsuType)
	if(ismob(src))
		var/mob/m = src
		m.DA_AVOIDING = world.time + 1000
	if(isobj(src))
		src:DamageIt(Damage)
	if(ismob(src))
		if((src&&WhoDidIt)&&(WhoDidIt:NonExist||src:NonExist))
			src<<"...A spirit can't be harmed...."
			WhoDidIt<<"Your attack slipped right through [src] causing you no harm!"
			return
		if((src&&WhoDidIt)&&(WhoDidIt:Phasing||src:Phasing))//&&(!src:knockedout))
			src<<"[WhoDidIt]'s attack slipped right through you causing you no harm!"
			WhoDidIt<<"Your attack slipped right through [src] causing them no harm!"
			return
//		if(!src:knockedout)//Just added as of 8/4/2013
