...
mob/proc/BubunBaika()
	...
	if(src.BubunBaika)
		var/obj/SmokeCloud/S=new();S.loc=src.loc;src.BubunBaika=0;src<<output("You reduce the size of your arms!","Attacks");src.overlays-=A;return
	if(src.BaikaCharged>=1|src.ChargingForBaikaNoJutsu)
		return
	else
		src.icon_state="handseal";sleep(11);src.icon_state=""
		var/obj/SmokeCloud/SS=new();SS.loc=src.loc
		src<<output("You increase the size of your arms!","Attacks")
		src.overlays-=A
		src.overlays+=A
		src.BubunBaika=1
		src.Endurance+=200
...
mob/proc/Nikudan()
	if(!src.BaikaCharged>=1)
		src<<output("You need to be in Baika No Jutsu first!","Attack");return
	else
		var/StaminaLoss=100
...
mob/proc/AkimachiSlam()
	if(!src.BubunBaika)
		src<<"You need to be in Partial Body Expansion to use this jutsu";
		return
	if(src.calories<2000)
		src<<"You don't have enough calories to use this jutsu.";
		return
	src<<"You slam the ground causing an earthquake"
	...
	Green
		...
		verb/Eat()
			...
			if(usr.calories<1000)
				usr<<"You can't eat this right now, you don't have enough calories!";return
			usr.calories-=1000
			usr.chakra+=500
			usr<<"You eat the first pill!"
			usr.Frozen=1
			sleep(11)
			usr.Frozen=0
			usr<<"You feel something starting!"
			usr.hunger=0
			usr.Pill="Green"
			var/count=0
	...
	Red
		...
		verb/Eat()
			...
			if(usr.calories<3000)
				usr<<"You can't eat this right now, you don't have enough calories!";return
			usr<<"You eat the Chili Pepper Pill!"
			usr.calories-=3000
			usr.chakra+=1500
			usr.overlays+='Icons/Jutsus/AkimichiWings.dmi'
			usr.Frozen=1
			sleep(11)
			usr.Frozen=0
			usr<<"You feel something starting!"
