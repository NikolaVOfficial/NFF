turf
	...
	//				walk_towards(C,src)
		if(usr.UsingAmaterasu)
			for(var/obj/Jutsu/Uchiha/AMA/T in oview(20))
				if(T.Owner==usr)
					walk_towards(T,src)
		...
		if(usr.usemurderofcrows)
			for(var/mob/Jutsu/MurderCrows/A in oview(20))
				if(A.Owner==usr)
					walk_towards(A,src)
		if(usr.Kawarimi)
			for(var/mob/Klog/A in oview(20))
				if(A.Controllable&&A.Owner==usr)
					walk_towards(A,src)
			for(var/mob/Klog2/A in oview(20))
				if(A.Controllable&&A.Owner==usr)
					walk_towards(A,src)
...
mob/proc/BlackBladeSpread()
	...
	for(var/turf/P in view(1,Z))
		if(prob(60))
			var/obj/Jutsu/Uchiha/AMA/EE=new();
			EE.Owner=src
		//	EE.nin=usr.nin
		//	E.nin=src.nin
			EE.loc=P;
