		...
		New()
			...
			spawn()
			 ...
			 //		O.icon_state="handseal"
					...
					for(var/mob/M in view(1,src))
						if(M!=src.Owner)
							M<<"You're sliced up by the Shurikens!"
...
mob/proc
	ShurikenSoujuu()
		if(src.ShurikenMode)
			src.ShurikenMode=0;src<<"You release the Shurikens."
			for(var/obj/A in view(20,src))
				if(A.icon=='Icons/shuriken.dmi'&&A.icon_state=="throw")
					walk(A,A.dir)
					return
		else
			src<<"You enter the Shuriken controlling mode. All Shurikens thrown can be controlled through clicking!";src.ShurikenMode=1
	...
	ChakraShuriken()
		...
		src.ChakraSight(1)
		src<<"For a little time you can see chakra!"
		spawn(100)
			src.see_invisible=1
			src.ChakraSight(0)
	...
	ChakraShurikenShield()
		if(src.knockedout)
			...
			src<<output("You shroud yourself in a Torrent of Shurikens!","Attack")
		//	if(prob(75))
	...
	ShurikenSmithyJutsu()
		...
		spawn(100)
			...
			src.see_invisible=1
		src.icon_state="handseal";
		src<<"You generate a torrent of Chakra Shurikens and constantly evict them from your body!"
		while(src.icon_state=="handseal")
			src.see_invisible=99;
