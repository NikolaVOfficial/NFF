						...
						spawn(1)
							if(M&&M!=src.Owner)
								...
								if(!M.BeingHitBack)
									if(distance>3)
										distance = 3
									M.HitBack(distance+1,get_dir(src,M));
									M.Normal()
...
mob/proc
	RangeExtension(Goggles)
		if(src.ScreenX!=src.OriginalX&&src.ScreenY!=src.OriginalY)
			...
			if(src.ScreenX==40&&src.ScreenY==30)
...
mob/proc		//One Eyed Byakugan
	...
	Kaiten()
		...
		if(!src.Kaiten&&!src.firing)
			...
			if(src.TaijutsuMastery<5)
				...
				src.KaitenCharge=round(src.TaijutsuKnowledge/400)
			...
			while(ZA==1)
				if(src.Kaiten)
					src.dir=NORTH;sleep(1);src.dir=EAST;sleep(1);src.dir=SOUTH;sleep(1);src.dir=WEST;src.stamina-=round(src.maxstamina*0.005);src.ChakraDrain(300)
				else
	...
	HakkeRokujonyonshou(mob/M,Power)
		if(M.knockedout||M.Dead)
			world<<"[src]([src.oname],Key:[src.key]) tried to abuse palms!"
			return
		...
		if(src.TaijutsuStyle=="Jyuken")
			...
			while(i>0&&get_dist(M,src)<=1&&src.bya&&!M.knockedout)
				...
				if(M&&(M.knockedout!=1)&&(M.sphere!=1)&&(M.Kaiten!=1))
					...
					src.JC++
					flick("jyuken",src)
					if(prob(25+src.StanceMastery+Power))
						M.StunAdd(5)
					if(M.JC>0&&M)
						viewers()<<sound('SFX/JyukenClash.wav',0);
						M.dir=get_dir(M,src);
			...
			if(M)
				if((M.JC<1)&&(ClosedTenketsu==1))
					...
					spawn(rand(300,500))
						M.UnableToChargeChakra=0;M<<"<font color = #00ffff size = 1>Your tenketsu points open up and your chakra begins flowing again!</font>"
			src.JC=0
			src.AttackDelay=0
			if(M)
				M.FrozenBind=""
		...
		if(src.DoingPalms!=2)
			src<<"Start up 64 palms then use this!";return
		if(src.TaijutsuStyle=="Jyuken")
			src<<"You change your pose quickly.";src.DoingPalms=3
mob/proc
	Kaiten2()
		if(src.firing)
			return
