...
mob/proc
	...
	FourthSeal()
		...
		if(src.KetsuekiDrain) return
			...
			src.Handseals(40-src.HandsealSpeed);if(src.HandsSlipped) return
			spawn(600)
	...
	SeventhSeal() //Actually is the Eighth Seal....
		...
		while(EighthSeal==1&&Time>0)
			...
			src.chakra-=5
			src.health+=7
			if(src.stamina>src.maxstamina*0.75) src.stamina=src.maxstamina*0.75
			if(src.health>src.maxhealth*0.8) src.health=src.maxhealth*0.8
			if(TT==4)
				src.thirst+=1
				if(src.thirst>=100)
					src.thirst=100
