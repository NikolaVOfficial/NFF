...
mob/proc/KubiAttack(damage,mob/attacker)
	if(src.inKubi)
		...
		while(src&&src.inKubi&&attacker.chakra>0&&src.health>0&&src.Struggle<src.StruggleAgainstKubi&&src.client)
...
mob/proc
	...
	ForrbiddenShadowArtMassiveNuiOnslaught()
		...
		if(src.Shibari)
			...
			var/SDir = src.dir
			while(Shadows>0)
		//		world<<"First line of Nui loop."
				var/i=3;//for the while loops;The while loop does is make the needles shot far out then swarm to the target
				//this gives the target time to run aswell as make it so the needles don't all fall together on one tile.
				src.icon_state="handseal";
				src.Frozen=1;
				src.UsingNui=1;
				src.ChakraDrain(5000);//Drain is this low since its called 10 times for total of 80k
				var/obj/Nara/KageNui/A=new();
				src.JutsuList+=A;
				A.Owner=src;
				A.shadowreach=10+src.ShadowReach;
				A.loc=src.loc;
				A.Move_Delay=0;
				src.shadowList.Add(A);
				...
				if(Shadows==6 || Shadows==10)
					...
					while(i>0)
						i--
				//		world<<"Value of Shadows is [Shadows]."
						step(A,A.dir);
						sleep(pick(1,2));
				var/s = get_dist(A,M);
				s-=3
			//	i-=1//I Sasuna just added this 8/4/2013
				if(s<=0)
					s=1
				spawn(1)
			//		world<<"Kage Nui Strike called."
					if(A)
						A.KageNuiStrike(usr,s,M)
				Shadows--
				sleep(2)
			//	world<<"Forbidden Nui while loop called."
			Shadows=0;
			src.Frozen=0;
			src.icon_state="";
			src.UsingNui=0
