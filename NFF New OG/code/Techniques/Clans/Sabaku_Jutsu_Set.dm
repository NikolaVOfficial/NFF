...
obj/proc/Sousou()
	...
	if(O.Bijuu=="Shukaku"&&O.TailState>0) damage*=1.75
...
obj/Jutsu/Sand
	...
	SandWhip
		...
		Bump(A)
			...
			if(ismob(A))
				...
				if(!Hit)
					...
					if(prob(O.NinjutsuKnowledge/25)||(O.Bijuu=="Shukaku"&&O.TailState>0&&prob(70))) //Was originally /200
						M.StunAdd(10)
						M.Running=0
		...
		Move()
			var/PrevLoc=src.loc
			var/obj/Jutsu/Sand/SandTrail/P=new()
			P.loc=PrevLoc
			..()

	SandTrail
		icon_state="Sand Trail"
		density=0
		layer=TURF_LAYER
		New()
			..()
	...
	SandArm2
		...
		Bump(A)
			...
			if(ismob(A))
				...
				if(M.Kaiten||M.sphere)
					return
				var/damage=round(rand(O.NinSkill*5,O.NinSkill*7.5));if(O.Bijuu=="Shukaku"&&O.TailState>0) damage*=2.5;M.DamageProc(damage,"Stamina",O);view(M)<<"[M] was hit by the Sand Arm!!";M.HitBack(3,src.dir)
			if(istype(A,/turf/))
				var/turf/T = A
				if(T.density)
					del(src)
...
//		src.SandCheck(30)
		if(src.sphere)
			src.FrozenBind="";src.firing=0
			for(var/obj/Jutsu/Sand/Sphere/Zzz in world)
				if(Zzz.Owner==src)
					del(Zzz)
			src.sphere=0;src<<"You take down the sand sphere.";src.usingS=0
			return
		if(!src.sphere)
			...
			while(src.sphere&&A)
				...
				if(src.chakra<ChakraDrain)
					...
					sleep(4)
			src.FrozenBind="";src.firing=0
			src.sphere=0;src<<"The sand sphere was broken!";src.usingS=0
			if(A)
				A.loc=null
