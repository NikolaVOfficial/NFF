			...
			spawn(15)
				while(src)
					for(var/mob/M in oview(1,src))
						if(M.name!=src.name)
							if(CSet=="C1"&&prob(15)&&M.Running)
								src.Explode()
							if(CSet=="C2"&&prob(24)&&M.Running)
								src.Explode()
					//		if(prob(50)&&M.Running)
					//			src.Explode()
					//		else if(prob(25)&&!M.Running)
					//			src.Explode()
					sleep(5);
		Click()
			if(src.Owner==usr)
...
mob/proc
	...
	SetRightHand()
		if(src.WeaponInRightHand!=""&&src.WeaponInRightHand!="Bird"&&src.WeaponInRightHand!="Mine"&&src.WeaponInRightHand!="\
		Spider"&&src.WeaponInRightHand!="Flower"&&src.WeaponInRightHand!="Wall"&&src.WeaponInRightHand!="Spear"&&src.WeaponInRightHand!="ClaySenbon"&&src.WeaponInRightHand!="Grasshopper")
			...
			var/A = input("What do you wish to equip to your Right hand?") in src.KibakuAbilities + list("Grasshopper","Mine","Cancel")
			src.SetRightHand=1
			...
			if(A=="Cancel")
				...
				src.SetRightHand=0
	SetLeftHand()
		if(src.WeaponInLeftHand!=""&&src.WeaponInLeftHand!="Bird"&&src.WeaponInLeftHand!="Mine"&&src.WeaponInLeftHand!="\
		Spider"&&src.WeaponInLeftHand!="Flower"&&src.WeaponInLeftHand!="Wall"&&src.WeaponInLeftHand!="Spear"&&src.WeaponInLeftHand!="ClaySenbon"&&src.WeaponInLeftHand!="Grasshopper")
			...
			var/A = input("What do you wish to equip to your left hand?") in src.KibakuAbilities + list("Grasshopper","Mine","Cancel")
			src.SetLeftHand=1
			if(A!="Cancel")
	...
	CreateC3()
		...
		if(!src.knockedout)
			...
			for(var/obj/RandomEquipment/Clay/SE)
				if(SE.Ammount>1)
					HaveClay=1
			//	HaveClay=1;
				SE.Ammount-=50;
...
//			HaveClay=1;
	...
	CreateFlyingBird()
		...
		if(!HaveClay)
			src<<"You've ran out of Clay!";return
		src.chakra-=2000;src<<"You begin to generate a Giant bird of Clay!"
...
//		src.chakra-=4000;
		...
		src<<"Click to have explosive spike rise! You have 20 seconds and 3 Spikes."
		Spikesleft=3
		spawn(200)
			src<<"Time for jutsu has expired."
			Spikesleft=0;

mob/proc/MakeSpike(atom/Target)
	if(get_dist(src,Target)>7)
		src<<"Your opponent is too far away for you to create an Iwazuka clay shard around them!"
		return
	src.Frozen=1;
	src.firing=1;
	src.icon_state="handseal"
