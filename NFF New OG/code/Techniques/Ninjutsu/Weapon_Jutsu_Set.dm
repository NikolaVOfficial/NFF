...
//	var/Mastery=0
	...
	src<<"You begin to concentrate on the summoning. Don't move! This will take a few seconds."
	while(src.icon_state=="handseal"&&!src.knockedout&&Check1==0)
		...
		if(T>15&&Uses>1000)
			...
			Check1=2
		else if(T>15&&Uses<150)
			Check1=1
	T=0
	if(Check1==1)
		src<<"You've summoned a Kunai!"
	...
	Bump(A)
		if(ismob(A))
			...
			if(M.icon_state=="Block"&&prob(8*M.DeflectionZ))//Chance to block the incoming senbon
				...
				return;
			if(M.Kaiten)
				view(M)<<output("<font>You deflect the Kunai!</font>","output")
				del(src);
				return;
			if(src.StunChance==1&&prob(30))
				M.Stun=10;
			M.DamageProc(damage,"Health",O)
			view(M)<<output("<font>[M] was by Kunai for [damage] damage!</font>","Attack")
...
mob/proc/ControllingWindmill(Uses)
	...
	src.Handseals(1-src.HandsealSpeed)
	if(src.HandsSlipped) return
	src<<"You begin to create a Windmill that you can move with the Mouse!"
	src.icon_state="handseal"
