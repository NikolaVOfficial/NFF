...
/*mob/proc/Mystical_Hand_Technique(Uses)
	...
	for(var/mob/M in get_step(src,src.dir))
		if(istype(M,/mob/pet))
			var/mob/pet/P=M
			if(P.Owner==src)
				var/healpet=(10+(10*Uses)+1)/rand(5,10)
				if(healpet>100+(100*src.ShousenMastery))
					healpet=(100-rand(-25,25))*(src.ShousenMastery+1)
				src<<"You begin to heal [P.name]"
				P.health+=healpet
...
mob/proc/Mystical_Hand_Technique(Uses)
	if(src.firing||src.knockedout||src.ShousenInUse)
		return
	if(src.target)
		src.dir=get_dir(src,src.target)
	flick("throw",src)
	...
	for(var/mob/M in get_step(src,src.dir))
		if(istype(M,/mob/pet))
			...
			if(P.health>=P.maxhealth)
				P.health=P.maxhealth
			return
		if(src.GenSkill>=30&&src.HoldingR&&!src.knockedout)
			M<<"[src] uses his chakra to heal you."
			src<<"You begin to heal [M]."
			src.ChakraDrain(50000)//around 900 chakra i think? Control effects this, all I can tell you is it's costly, it used around 20-25% chakra per use on a new char with 4k chakra
			var/Z=30+Uses//I didnt do much for this shousen, i left it be seeing as it'd be for a rank, i figured it should have power reguardless.
			if(Z>100)
				Z=100
			if(prob(Z))
				if(M.StruggleAgainstDeath>0)
					if(prob(Uses/10))
						M.Struggle+=rand((Uses/8),(Uses/10))//Again left this the same, as is ingame it's op theres no limit set.
				var/heal=(10+(30*Uses)+1)/rand(5,10)
				if(heal>100+(100*src.ShousenMastery))
					heal=(100-rand(-25,25))*(src.ShousenMastery+1)
				if(src.ARank=="Healer")
					heal+=rand(50,100)
				if(src.SageMode=="Slug")
					heal*=2
				if(M.health<M.maxhealth||M.stamina<M.maxstamina)//I slightly changed this part, basically it uses the old burst shousen numbers, heals both stats, stam twice as much.
					M.health+=heal
					M.stamina+=(heal*2)
					src<<output("You healed [heal] health and [heal*2] stamina.","Attack")
					if(heal>350)//Lowered this number so it's more constant.
						M.deathcount-=0.2//I didnt want this high at all.
						if(M.deathcount<0)
							M.deathcount=0
					if(M.health>M.maxhealth)
						M.health=M.maxhealth
					if(M.stamina>=M.maxstamina)
						M.stamina=M.maxstamina
					if(M.client&&M.Dead&&M.health>=M.maxhealth/2&&!PermDeath&&!M.CBleeding&&!M.ImmuneToDeath)
						M.Dead=0
						M.icon_state=""
						M.FrozenBind=""
						M.sight&=~(SEE_SELF|BLIND)
						orange(12,M)<<"<font size = 2>[M] was revived from the brink of death by [src]!</font>"
				if(M.health>=M.maxhealth)
					src<<output("They're fully healed in Health.","Attack")
				if(M.stamina>=M.maxstamina)
					src<<output("They're fully healed in Stamina.","Attack")
				if(M.screwed&&src.Cure)
					M.screwed=0;M<<"Your nerves were healed.";src<<"You healed their nerves."
				if(M.CBleeding&&src.Cure)
					M.CBleeding=0;M<<"Your no longer bleeding.";src<<"You healed their internal bleeding."
				if(M.Poisoned&&src.Cure)
					M.Poisoned=0
					M.PoisonTime=0
					M<<"[src] has cured your Poison."
			...
			M<<"[src] begins to use his chakra to heal you, don't move!"
			src<<"You begin to heal [M], aslong as no one moves you will continuously heal them!"
			while(src.loc==Location&&!src.knockedout&&(M in get_step(src,src.dir)))
				sleep(10)
				src.ShousenInUse=1
				src.UnableToChargeChakra=1
				TI+=1//A var used to display healing monitoring message to the user, saying whats getting healed.
				src.ChakraDrain(2000)//~20 chakra a tick
				src.ShousenInUse=1//I used this on the skillcards to prevent multi-healing/healing stack by user.
				if(M.PetOut==0&&M.PetsHealth<101)
					M.PetsHealth+=5
				if(M.client&&M.Dead&&M.health>=M.maxhealth/2&&!PermDeath&&!M.CBleeding&&!M.ImmuneToDeath)
					M.Dead=0
					M.icon_state=""
					...
					orange(12,M)<<"<font size = 2>[M] was revived from the brink of death by [src]!</font>"
				if(M.StruggleAgainstDeath>0)
					M.Struggle+=1//A low static amount, to ASSIST, not be a life saver.
				if(M.Status!=""&&src.Cure)
					M.StatusEffected=0//Yea i didnt redo this, this is still instant.
					M.Status=""
					M<<"[src] has cured your Ailment."
				if(M.Poisoned&&src.Cure)
					M.PoisonTime-=2//I think i made the poisontime var IDR, it's exactly that though.
				if(M.health<M.maxhealth||M.stamina<M.maxstamina)//Heals both stats if either one is below full
					M.health+=5+(src.ShousenMastery*(src.GenSkill/4))
					M.stamina+=25+((src.ShousenMastery*(src.GenSkill/5))*2)
				if(M.screwed)
					M.screwed=0;M<<"Your nerves were healed.";src<<"You healed their nerves."//again, instant cured
				if(M.LegSliced>=1&&src.Cure)
					M.LegSliced=0//Slow cure
				if(M.health>=M.maxhealth&&M.stamina>=M.maxstamina*0.99)//Makes sure vit is at 100% and stamina is at or abive 99%
					M.deathcount-=0.1//More effective than my new iryo's heal, aswell as cost friendly, but takes time.
				if(TI==10)//The healing messages
					if(M.health<M.maxhealth*0.99)
						src<<"You're Healing Vitality!"
					if(M.stamina<M.maxstamina*0.99)
						src<<"You're Healing Stamina!"
					if(M.health>=M.maxhealth&&M.stamina>=M.maxstamina*0.99)
						if(M.deathcount>0)
							src<<"You're Restoring Willpower!"
					TI=0
				if(M.deathcount<0)//stat overcap checks
					M.deathcount=0
				if(M.health>M.maxhealth)
					M.health=M.maxhealth
				if(M.stamina>M.maxstamina)
					M.stamina=M.maxstamina
				if(M.health>=M.maxhealth)//Auto ending
					if(M.stamina>=M.maxstamina*0.99)
						if(M.deathcount<=0)
							if(M.PoisonTime<0.1&&src.Cure)//If the user has cure & if target is effected
								if(M.LegSliced<1&&src.Cure)
									M<<"You're completely Healed!"
									src<<"They're completely Healed!"
									src.ShousenInUse=0
									src.UnableToChargeChakra=0
									return
							else
								M<<"You're completely Healed!"//If the user doesnt have cure
								src<<"They're completely Healed!"
								src.ShousenInUse=0
								src.UnableToChargeChakra=0
								return
			src.ShousenInUse=0//If user or target moves.
			src.UnableToChargeChakra=0
...
mob/proc/InternalBleed()
	var/InternalBleeding=rand(25,350)
	...
	while(InternalBleeding)
		...
		if(src.knockedout||InternalBleeding<=0)
			InternalBleeding=0
		sleep(11)
mob/proc/InternalClawsBleed()
	var/InternalBleeding=rand(1,2)
	if(!(locate(/obj/ReleaseThingyAilment/Bleed) in src.client.screen))
		var/obj/ReleaseThingyAilment/Bleed/P = new()
		P.HowLong=InternalBleeding
		if(src.client)
		...
		for(var/mob/M in get_step(src,src.dir))
			...
			//	src.Frozen=1
				...
				if(CCC>80)
					CCC=80
				var/ChakraLeech=1
				while(!src.knockedout&&!M.knockedout&&ChakraLeech)//&&(M.Struggle<(src.GenSkill*30)*(src.ChakraLeech+1)*pick(1,1.5,2)))
					src.icon_state="throw"
					if(M.Struggle>=CCC)//((src.GenSkill*30)*(src.ChakraLeech+1)*pick(1,1.5,2)))
...
mob/proc/ChakraMode()
	...
	while(src.ChakraMode)
		...
		src.ChakraDrain(25000)
		src.ChakraPool-=rand(250,300)
		sleep(65/(src.ChakraModeMastery+1))
		if(src.knockedout)
			src.ChakraMode=0
			src<<"You release the Forbidden Ninjutsu...."
			src<<"The jutsu takes it's toll upon you.."
...
mob/proc
	ChakraPunch()
		if(src.UserChargingForChakraPunch)
			...
			src.OukashouHits=1+src.OukashouMastery
	...
	HitTheGround()
		...
		for(var/mob/M in oview(Range,src))
			var/DamageCheck=get_dist(M,src)
			if(DamageCheck>2)
				Damage*=0.70
			if(DamageCheck>4)
				Damage*=0.50
			if(DamageCheck>=6)
				Damage*=0.50
		//	var/DamageCheck2=Damage/(0.17*DamageCheck)
		//	Damage*=(Damage*(0.17*DamageCheck))
			spawn()
				Quake_Effect(M,10,2)
...
mob/proc
	...
	HitTheGroundKick()
		...
		if(Damage>60*src.GenSkill)
			Damage=60*src.GenSkill
		src.ChargedChakraForKick=0
		var/Range=round(Damage/250)
		if(Damage>2000)  //Chakra Kick has greater range but lower damage cap. This way its different from Chakra Punch.
			Damage=2000;
		if(Range>10)
			Range=10
		view(Range)<<output("<font>[src] slams the ground dealing [Damage]!</font>","Attack")
		src.CreateCrator()
		//for(var/turf/T in oview(Range,src))
		//	spawn()
		//		if(prob(25))
...
mob/proc/Chakra_Sense()
	...
	sleep(10)
	src.ChakraSense=1
	src<<"You're now able to sense chakra! Press C to see if there is any noticable Chakra around you!"
...
mob/proc/EyeoftheKagura()
	if(!usr.watching)
		var/EoK=list()
		for(var/mob/player/TTT in world)
			EoK+=TTT
		var/mob/M = input(usr,"Who do you want to see?","Sense") in EoK + list("Cancel")
		if(M == "Cancel"){return}
		usr<<"You watch [M]!"
