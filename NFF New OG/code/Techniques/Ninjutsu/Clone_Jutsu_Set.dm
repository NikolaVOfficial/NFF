	...
	Clone
		New()
			...
			spawn()
				...
				//	if(State!="running")
				//		world<<"Current Icon State isn't running"
				//	else
				//		world<<"Current Icon State=[State]"
			//		src.dir=WEST
		//			if(State=="")
		//				world<<"Current Icon State=Blank"
		//			else
		//				world<<"Current Icon State=[State]"
				//	world<<"Current Icon State=[State]"
					sleep(2)
		//		if(src.CloneType=="Rasenshuriken")
				//	world<<"Wow testing the Clone New() :^{). Clones current direction is [src.dir]"
				src.dir=WEST
				src.dir=8
				if(CloneType=="SageCharge")
					spawn (300)
						del(src)
						return
			spawn(900)
		...
		Bump(atom/M)
			if(istype(M,/mob/)) // If they run into the player
				if(CloneType=="ToadSage"&&!Grappled)
					Grappled=1
					var/mob/O=src.Owner
					var/mob/MA=M
			//		src.icon_state="Attack1"
					...
					if(MA.client)
						MA<<"[O]'s Bunshin successfully grapples you from a distance due to it's use of Senjutsu! Locking both your arm and leg movements!"
						MA.Frozen=1
					O<<"Your Clone has placed [MA] in a strong grapple!"
					spawn(25)
						O.ToadSageBunshinAttack(MA)
		...
		Del()
			var/mob/M=src.Owner
			if(src.CloneType=="SageCharge") //sagecharge clone

				M.NatureChakra+=src.NatureChakra
				if (M.NatureChakra >=600)
					M.NatureChakra=600
					M << "Your Clone has been destroyed and you gained [src.NatureChakra] Sage Chakra!"
			if(SleepingInTheSky)
...
//Kikkaichu
	...
	for(var/mob/Clones/Clone/K in world)
		if(K.Owner==usr&&!K.Tackling&&K.CloneType!="KBunshin"&&K.CloneType!="Rasenshuriken"&&K.CloneType!="SageCharge")
			if(K.Frozen)
				K.Frozen=0
				walk_towards(K,src)
				K.moving=1
...
turf/Click()//this will make it so if you click the turf your pet will walk to it			//like in MJ
	if(usr.shunshin&&usr.Trait=="Speedy"&&!usr.CastingGenjutsu&&!usr.LegSliced&&!usr.Guarding&&usr.Status!="Asleep")
		...
		if(!usr.Running)
			usr<<"You aren't able to use Shunpo because you aren't running."
			return
		if(usr.z!=src.z)
			usr<<"I like how I told you multiple times not to do this..and you keep trying to do it.."
			usr.health-=1500
			usr.deathcount+=10
			return
	...
	//			del(R)
			...
			usr.loc=src
	if(usr.FTG_Mode)
		if(!src.density&&usr.cantele&&usr.FrozenBind==""&&!usr.Frozen&&usr.Stun<1&&src.z==usr.z)
			if(!usr.Running)
				usr<<"You aren't able to use Flying Thunder God because you aren't running."
				return
	...
	for(var/mob/Clones/Clone/K in world)
		if(usr&&K.Owner==usr&&!K.Tackling&&K.CloneType!="KBunshin"&&K.CloneType!="Rasenshuriken"&&K.CloneType!="SageCharge")
...
obj/Click()//this will make it so if you click the turf your pet will walk to it			//like in MJ
	for(var/mob/Clones/Clone/K in world)
		if(K&&K.Owner==usr&&usr&&!K.Tackling&&K.CloneType!="KBunshin"&&K.CloneType!="Rasenshuriken"&&K.CloneType!="SageCharge")
			if(K.Frozen)
				K.Frozen=0
				walk_to(K,src)
				K.moving=1
...
//	if(A)//Lag fix 6/23/2013
	...
	if(src.SageMode=="Toad")
		A.CloneType="ToadSage"
		A.density = 1
		A.Clone = 1
		A.health=10
		A.RunningSpeed=5;
		A.Running=1;
		src << " This clone utilizes senjutsu!"
...
//				Focus=ASDD.Current_Number
	...
	while(Control>0&&src.icon_state=="handseal")
		...
		if(src.chakra<=500)
			...
			CurrentControl=0
			return
		var/list/BAD = list("locs","ManBeastClaws","ZankuuhaOn","Kengan","BubunBaika","scalpel","KagDance","cansave","icon_state","RasenganD","ChidoriD","Rasenganon","Chidorion","WeaponInRightHand","WeaponInLeftHand","LeftHandSheath","RightHandSheath","GateIn","key","ckey","client","type","parent_type","verbs","vars","group","x","y","z","loc","contents","alterV","alterC","alterR","OriginalVillage","Orank","OClan","ImmuneToDeath","Owner","CopyMode","ChargedChakraForPunch")
		if(CurrentControl==1)
...
//				Focus=ASDD.Current_Number
	...
	if(K)
		for(var/V in src.vars)
			var/list/BAD = list("locs","Kengan","ZankuuhaOn","ManBeastClaws","BubunBaika","shunshin","scalpel","WeaponInLeftHand","WeaponInRightHand","cansave","key","ckey","client","type","parent_type","verbs","vars","group","x","y","z","loc","contents","controlled","alterV","alterC","alterR","OriginalVillage","Orank","OClan","ImmuneToDeath")
			if(!BAD.Find(V))
				K.vars[V] = src.vars[V]
