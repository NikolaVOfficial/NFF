...
obj/Puppet_Summoning_Scrolls
	...
	verb
		Rename()
			NamingPuppet
			var/characterfirstname=input(usr,"What would you like to rename your Puppet?","Name")
			if(length(characterfirstname)<1)
				alert(usr,"It must be longer than 1 letter!")
				goto NamingPuppet
			if(length(characterfirstname)>15)
				alert(usr,"It must be shorter than 15 letters!")
				goto NamingPuppet
	...
	verb
		Use()
			...
			for(var/obj/Puppet_Summoning_Scrolls/A in usr.contents)
				PuppetsList+=A
			var/obj/Puppet_Summoning_Scrolls/AC = input("Which Puppet would you like to use this on?") in PuppetsList + list("Cancel")
			if(AC=="Cancel")
				return
			if(AC.Summoned)
				usr<<"You have to unsummon it to work on it.";return
			if(src.name=="Structure Enhancer"||src.name=="Structure Enhancer Lite"||src.name=="Structure Enhancer Gold")
				if(AC.BonusArmor>800)
					usr<<"You've already reached the cap for enhancing the Structure of your Puppet. Perhaps there's something else you can do.";return
				AC.BonusHealth+=Amount
			if(src.name=="Armor Enhancer"||src.name=="Armor Enhancer Lite"||src.name=="Armor Enhancer Gold")
				if(AC.BonusArmor>800)
					usr<<"You've already reached the cap for enhancing the Armor of your Puppet. Perhaps there's something else you can do.";return
			...
			if(src.name=="Power Enhancer"||src.name=="Power Enhancer Lite"||src.name=="Power Enhancer Gold")
				if(AC.BonusStrength>1000)
					usr<<"You've already reached the cap for enhancing the Strength of your Puppet. Perhaps there's something else you can do.";return
			...
			if(src.name=="Spiked Arms")
				if(istype(AC,/obj/Puppet_Summoning_Scrolls/Hawk/))
					...
					usr<<"This Puppet does not support that feature.";return
			if(src.name=="Adjustable Room")
				src<<"Currently disabled, sorry.";return
			...
			if(src.name=="Fast Return")
				usr<<"You will now be able to summon your [AC] puppet faster!"
				AC.Fast_Return=1
				del(src)
				return
			if(src.name=="Exploding Return")
				usr<<"Your [AC] puppet now has the ability to explode once it's destroyed!"
				AC.ExplodingReturn=1
				del(src)//just started adding the return and del(src) statements 12/1/2012
				return
			if(src.name=="Stun Resister")
				AC.StunResister=1
				usr<<"You have placed a Stun Resister on your [AC] puppet.!"
				del(src)
				return
			if(src.name=="Adjustable Capture Room")
				if(istype(AC,/obj/Puppet_Summoning_Scrolls/Dragon/))
					...
					usr<<"This Puppet does not support that feature.";return
			//	AC.AdjustableRoom=1
				usr<<"You have placed the Adjustable Capture Room attachment onto your Dragon! When your Dragon approaches an enemy it will place them inside it!"
				del(src)
				return
				...
				for(var/obj/Puppet_Summoning_Scrolls/Dragon/P in src.contents)
					if(istype(P,/obj/Puppet_Summoning_Scrolls/Dragon/))
						if(P.AdjustableRoom)
							...
							usr<<"You need to apply the Adjustable Capture Room Feature before you can apply this!";return
				usr<<"You have placed the Adjustable Capturing Room Implosion attachment onto your Dragon! When your Dragon stores someone inside of them press X to release an explosion harming the person inside at the cost of destroying your puppet!"
				del(src)
				return
			if(src.name=="Kunai Projectile Attachment - Light")
				if(istype(AC,/obj/Puppet_Summoning_Scrolls/Talon/))
					...
					usr<<"This Puppet does not support that feature.";return
			//	AC.AdjustableRoom=1
				usr<<"You have placed the Light Kunai Projectile attachment onto your Talon! Press A while using your puppet to have it release Kunais at a slow rate!"
				del(src)
				return
			if(src.name=="Kunai Projectile Attachment - Medium")
				if(istype(AC,/obj/Puppet_Summoning_Scrolls/Talon/))
					...
					if(ACC.KunaiProjectile==1&&ACC.KunaiAutoFire==0)
						...
						usr<<"You need to have the Kunai Projectile Attachment on first before you can apply this one!";return
					else if(ACC.KunaiAutoFire!=0)
						usr<<"You're not able to use the Kunai Projectile Attachment with the Rapid Fire Attachment!";return
				//	ACC.KunaiProjectile=1;ACC.KunaiAutoFire=0
				else
					usr<<"This Puppet does not support that feature.";return
				usr<<"You have placed the Medium Kunai Projectile attachment onto your Talon! Press A while using your puppet to have it release slightly stronger Kunais at a slightly faster rate!"
				del(src)
				return
			if(src.name=="Kunai Projectile Attachment - Strong")
				...
				//	ACC.KunaiProjectile=1;ACC.KunaiAutoFire=0
					...
					usr<<"This Puppet does not support that feature.";return
				usr<<"You have placed the Strong Kunai Projectile attachment onto your Talon! Press A while using your puppet to have it release much stronger Kunais at a much faster rate!"
				del(src)
				return
			if(src.name=="Kunai Rapid Projectile Attachment - Light")
				if(istype(AC,/obj/Puppet_Summoning_Scrolls/Talon/))
					...
					usr<<"This Puppet does not support that feature.";return
				usr<<"You have placed the Light Kunai Rapid Projectile attachment onto your Talon! Press A while using your puppet to have it release Rapid Kunais!"
				del(src)
				return
			if(src.name=="Kunai Rapid Projectile Attachment - Medium")
				if(istype(AC,/obj/Puppet_Summoning_Scrolls/Talon/))
					...
					if(ACC.KunaiAutoFire==1&&ACC.KunaiProjectile==0)
						...
						usr<<"You need to have the Light Rapid Fire Kunai Projectile Attachment on first before you can apply this one!";return
					else if(ACC.KunaiProjectile!=0)
						usr<<"You're not able to use the Kunai Rapid Fire Projectile Attachment with the normal Kunai Attachment!";return
				else
					usr<<"This Puppet does not support that feature.";return
				usr<<"You have placed the Medium Rapid Kunai Projectile attachment onto your Talon! Press A while using your puppet to have it release much more Kunais at a faster rate!"
				del(src)
				return
			if(src.name=="Kunai Rapid Projectile Attachment - Strong")
				if(istype(AC,/obj/Puppet_Summoning_Scrolls/Talon/))
					...
					if(ACC.KunaiAutoFire==2&&ACC.KunaiProjectile==0)
						...
						usr<<"You need to have the Medium Rapid Fire Kunai Projectile Attachment on first before you can apply this one!";return
					else if(ACC.KunaiProjectile!=0)
						usr<<"You're not able to use the Kunai Rapid Fire Projectile Attachment with the normal Kunai Attachment!";return
				else
					usr<<"This Puppet does not support that feature.";return
				usr<<"You have placed the Strong Rapid Kunai Projectile attachment onto your Talon! Press A while using your puppet to have it release much more Kunais at a much faster rate than before!"
				del(src)
				return
			if(src.name=="Poison Tipped Blades Attachment")
				if(istype(AC,/obj/Puppet_Summoning_Scrolls/Talon/))
					var/obj/Puppet_Summoning_Scrolls/Talon/ACC=AC;ACC.PoisonBlades=1
				else
					usr<<"This Puppet does not support that feature.";return
				usr<<"You have placed the Poison Tipped Blades onto your Talon! When your Talon Puppets Attacks someone it has a high chance to poision them with a very harmful poison!"
				del(src)
				return
			if(src.name=="Spreading SmokeScreen Attachment - Light")
				if(istype(AC,/obj/Puppet_Summoning_Scrolls/Hawk/))
					...
					usr<<"This Puppet does not support that feature.";return
				usr<<"You have placed the SmokeScreen Spread Attachment onto your Hawk! By pressing this skillcard you are able to release a small field of Smoke Screen around your puppet to partially blind enemy Ninja!"
				del(src)
				return
			if(src.name=="Spreading SmokeScreen Attachment - Medium")
				if(istype(AC,/obj/Puppet_Summoning_Scrolls/Hawk/))
					...
					if(ACC.SpreadingSmokeScreen==1)
						...
						usr<<"You need to have the Light SmokeScreen Attachment on first before you can apply this one!";return
				else
					usr<<"This Puppet does not support that feature.";return
				usr<<"You have placed a much stronger version on the SmokeScreen Spread Attachment onto your Hawk! By pressing this newer skillcard you are able to release a larger field of Smoke Screen around your puppet to partially blind enemy Ninja!"
				del(src)
				return
			if(src.name=="Spreading SmokeScreen Attachment - Strong")
				if(istype(AC,/obj/Puppet_Summoning_Scrolls/Hawk/))
					...
					if(ACC.SpreadingSmokeScreen==2)
						...
						usr<<"You need to have the Medium SmokeScreen Attachment on first before you can apply this one!";return
				else
					usr<<"This Puppet does not support that feature.";return
				usr<<"You have placed an even stronger version on the SmokeScreen Spread Attachment onto your Hawk! By pressing this newer skillcard you are able to release an even larger field of Smoke Screen around your puppet to partially blind enemy Ninja!"
				del(src)
				return
			if(src.name=="Spreading Poison Attachment - Light")
				if(istype(AC,/obj/Puppet_Summoning_Scrolls/Hawk/))
					...
					usr<<"This Puppet does not support that feature.";return
				usr<<"You have placed the Poison Spread Attachment onto your Hawk! By pressing this skillcard you are able to release a small field of poisinous gas around your puppet to harm enemy Ninja!"
				del(src)
				return
			if(src.name=="Spreading Poison Attachment - Medium")
				if(istype(AC,/obj/Puppet_Summoning_Scrolls/Hawk/))
					...
					if(ACC.SpreadingPoison==1)
						...
						usr<<"You need to have the Light Poison Attachment on first before you can apply this one!";return
				usr<<"You have placed a stronger version of the Poison Spread Attachment onto your Hawk! By pressing this newer skillcard you are able to release a larger field of poisinous gas around your puppet to harm enemy Ninja!"
				del(src)
				return
			if(src.name=="Spreading Poison Attachment - Strong")
				if(istype(AC,/obj/Puppet_Summoning_Scrolls/Hawk/))
					...
					if(ACC.SpreadingPoison==2)
						...
						usr<<"You need to have the Medium Poison Attachment on first before you can apply this one!";return
				usr<<"You have placed an even stronger version of the Poison Spread Attachment onto your Hawk! By pressing this newer skillcard you are able to release an even larger field of poisinous gas around your puppet to harm enemy Ninja!"
				del(src)
				return
			if(src.name=="Adjustable Fire Attachment")
				if(istype(AC,/obj/Puppet_Summoning_Scrolls/Dragon/))
					...
					usr<<"This Puppet does not support that feature.";return
				usr<<"You have placed the Fire Attachment onto the mouth of your Dragon! If you press Z while controlling your Dragon you will release fire!"
				del(src)
				return
			if(src.name=="Spreading Ice Attachment - Light")
				if(istype(AC,/obj/Puppet_Summoning_Scrolls/Dragon/))
					...
					usr<<"This Puppet does not support that feature.";return
				usr<<"You have placed the Light Ice Attachment onto the head of your Dragon! If you press X while controlling your Dragon you will release a small stream of Liquid Nitrogen!"
				del(src)
				return
			if(src.name=="Spreading Ice Attachment - Medium")
				if(istype(AC,/obj/Puppet_Summoning_Scrolls/Dragon/))
					...
					if(ACC.IceBreathing==1)
						...
						usr<<"You need to have the Light Ice Attachment Equipped to equip this attachment!";return

				else
					usr<<"This Puppet does not support that feature.";return
				usr<<"You have placed the Medium Ice Attachment onto the head of your Dragon! If you press X while controlling your Dragon you will release a larger stream of Liquid Nitrogen!"
				del(src)
				return
			if(src.name=="Spreading Ice Attachment - Strong")
				if(istype(AC,/obj/Puppet_Summoning_Scrolls/Dragon/))
					...
					if(ACC.IceBreathing==2)
						...
						usr<<"You need to have the Medium Ice Attachment Equipped to equip this attachment!";return

				else
					usr<<"This Puppet does not support that feature.";return
				usr<<"You have placed the Strong Ice Attachment onto the head of your Dragon! If you press X while controlling your Dragon you will release a large stream of Liquid Nitrogen!"
				del(src)
				return
			usr<<"[AC.BonusHealth] Bonus Health."
			usr<<"[AC.BonusArmor] Bonus Armor."
			usr<<"[AC.BonusStrength] Bonus Strength."
			del(src)
	Structure_Enhancer
...
//	var/mob/Puppet/Dragon/User=ByWhat
	view()<<"The Puppet opens it's body revealing a large box like area and shoves [src] into it!"
	src<<"Press Spacebar to Struggle Out of the sharp jagged Box!"
	if(ByWhat.AdjustableRoomCollapsation)
		M<<"Press X to implode the Dragon puppet!"
...
//			BackUp-=src
		sleep(1)
	view()<<"[src] has broken out from the puppet!"
//	if(ByWhat)
//		ByWhat.CaughtInRoom-=src
//	else
//		BackUp-=src
...
mob/proc/PuppetMasterJutsu()
	...
	for(var/obj/Puppet_Summoning_Scrolls/A in src.contents)
		PuppetsList+=A
	var/obj/Puppet_Summoning_Scrolls/AC = input("Which Puppet would you like to Summon?") in PuppetsList + list("Cancel")
	if(AC=="Cancel")
		return
	if(AC.BonusArmor>1000)
	...
	if(AC.BonusHealth>1000)
		AC.BonusHealth=1000
	if(AC.Delay)
		src<<"This Puppet has a delay before being summoned again.";return
	if(AC.Summoned)
		src<<"You must unsummon this puppet for it to be used.";return

	if(AC.PuppetControlType==1&&src.ControllingPuppet1)
		src<<"You're already controlling a Puppet utilizing the Keyboard.";return

	if(AC.PuppetControlType==2&&src.ControllingPuppet2)
		src<<"You're already controlling a Puppet utilizing the Mouse.";return
...
//		src.ControllingPuppet2=0;src.ControllingPuppet1=0
		src<<"That puppet is not a Puppet Summoning type? You've encoutered an error, please post this on the forums with the Puppet specifically attempted to be summoned.";return
mob/var/tmp/PuppetSwap=0
mob/proc/PositionSwap()
	src<<"Click the Puppet you'd like to swap with."
	src.PuppetSwap=1
mob/proc/PuppetView()
	...
	for(var/mob/Puppet/A in world)
		if(A.Owner==src)
			PuppetsList+=A
	var/mob/Puppet/AC = input("Which Puppet would you like to view?") in PuppetsList + list("Cancel")
	if(AC=="Cancel")
		return
	if(!AC)
		return
...
mob/proc/PuppetReturn()
	...
	for(var/mob/Puppet/A in world)
		if(A.Owner==src)
			PuppetsList+=A
	var/mob/Puppet/AC = input("Which Puppet would you like to return?") in PuppetsList + list("Cancel")
	if(AC=="Cancel")
		return
	if(!AC)
		return
...
mob/Puppet/Dragon/proc/PuppetFireBall()
	...
	view()<<"[src] begins to release a large steel pole from their mouth and a scent of Gas resonates..."
	sleep(15)
