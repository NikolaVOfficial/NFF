...
mob/proc
	...
	XiveresSelfishPunishment()
		...
		for(var/mob/M in world)
			if(M.XiveresMember)
				...
				var/obj/Mshar/A=new();A.screen_loc = "1,1 to 32,23"
	...
	XiveresToBase()
		...
		spawn(100)
			src.KiriOpen=0
		K.openlocation=K2.loc;
		K2.openlocation=K.loc
		K.Owner=src;
		K2.Owner=src
		src.KiriOpen=1
	...
	ReachForTheSky2()
		if(src.ChakraPool<=8000)
			src<<"You don't have enough chakra to reach The Hideout!"
			return
		src.Handseals(90)
		if(src.HandsSlipped) return
		src.ChakraDrain(40000)
		...
		spawn(30)
			if(src.knockedout)
				...
				src.loc=locate(31,57,40)
				src.FrozenBind=""
				src<<"<i><font color = yellow>You arrive at the Hideout...</font></i>"
	BoneTeleport()
		if(src.ChakraPool<=8000)
			src<<"You don't have enough chakra to reach The Hideout!"
			return
		...
		src.ChakraDrain(40000)
		src.ChakraPool-=4500
		src<<"<i><font color = purple>You begin to teleport to base..wait a second..</font></i>"
		src.FrozenBind="Reaching For the Sky"
		var/image/BoneImage=icon('BoneClub/BoneTeleport.dmi',src.loc,"BoneTeleport")

		spawn(60)
	//		BoneImage.loc=src.loc
			src<<BoneImage
			if(src.knockedout)
				src.FrozenBind=""
				return
			else
				src.loc=locate(5,1,44)
				src.FrozenBind=""
				src<<"<i><font color = purple>You arrive at the base...</font></i>"
	IndigoWarp()
		if(src.ChakraPool<=5000)
