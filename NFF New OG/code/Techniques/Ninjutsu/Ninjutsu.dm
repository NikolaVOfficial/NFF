...
mob/proc/ToggleSageAura()
	if(src.SageMode!="Rikudou")
		...
		src.AuraOn=0;src.overlays-='New Icons(Con4)/NewSageModeAura.dmi';src.overlays-='New Icons(Con4)/AshuraSageMode.dmi';src.overlays-='New Icons(Con4)/AshuraSageJacket.dmi'
		src.overlays-='New Icons(Con4)/NewSageModeAura.dmi';src.overlays-='New Icons(Con4)/Rikudou Truth Seeking Orbs.dmi';src.overlays-='New Icons(Con4)/RikudouAura.dmi';src.overlays-='New Icons(Con4)/PureYellowShoe.dmi'
		FTG_Mode=0
		src.nin=src.Mnin*2
		...
		src.AuraOn=1
		src.overlays+='New Icons(Con4)/AshuraSageMode.dmi'
		src.overlays+='New Icons(Con4)/AshuraSageJacket.dmi'
	//	src.overlays+='New Icons(Con4)/NewSageModeAura.dmi'
		src.overlays+='New Icons(Con4)/RikudouAura.dmi'
		src.overlays+='New Icons(Con4)/PureYellowShoe.dmi'
...
mob/proc/RikudouSageMode(TimeLimit=10,Length=src.NatureChakra)
	if(src.knockedout)
		return
	var/UsedBijuu=0
...
//	if(src.TailState>0)
	...
	src<<"Toogle sage mode aura to activate your full sage mode powers"
...
//	if(!UsedBijuu)
	...
	src.tai=src.Mtai*2
	src.ToggleSageAura()
	if(!(locate(/obj/SkillCards/RikudouSageAura) in src.LearnedJutsus))
...
mob/Klog2
	...
	Death(mob/M)
		spawn(1)
			src.overlays=null;
			src.icon="NarutoStuff.dmi";
			src.icon_state="Exploding Log";
			flick("tag",src)
...
mob/proc/SilentKilling()
	...
	for(var/mob/M in get_step(src,src.dir))
		if(M.dir==src.dir)
			if((X>90&&(src.Focus>M.Focus*1.2)||src.key==""||(src.BloodLust&&prob(50))))
				M.CBleeding=1;
					...
					if(M!=O)
						...
						if(M&&M.client)
							...
							if(M.target==src&&src.ButterflyDance)
								...
								if(M.CursedSealType=="Fire"&&!M.CursedSealActivated)
				//	return
									M.DeleteTarget()
							sleep(3)
				step_to(src,O)
			...
			while(src)
				...
				if(src.icon_state=="fist")
					for(var/mob/M in src.loc)
						...
						if(!CapturePerson)
							if(M.FrozenBind=="")
								M<<"You've been binded by the Leaf Bind!";M.FrozenBind="Haruno";CapturePerson=M
						if(M.FrozenBind=="Haruno"&&O.Guarding)
							M<<"The grip tightens on you!"
							M.DamageProc(O.nin*4,"Stamina",O)
						sleep(11)
				sleep(11)
		spawn(120)
			del(src)
	Del()
		var/mob/M=CapturePerson
		var/mob/O=src.Owner
		if(M)
...
mob/proc/BoneSageMode(TimeLimit,Length)
	if(src.knockedout)
		return
	src.icon_state="rest"
	src<<"You begin to concentrate and store up natural energy in your bones! Don't move at all!"
	if(TimeLimit>0)
		src<<"You have [TimeLimit] seconds to remain meditating."
	var/A=TimeLimit
	while(A>1)
	...
	src<<"Success! Your bones are now enhanced with chakra forged from your surroundings!"
	src.SageMode="Bone"
	src.overlays+='Icons/New Base/BoneSage.dmi'
	src.icon_state="power"
	sleep(11)
	src.icon_state=""
	...
	while(SageModeTime&&src.SageMode!=""&&src.chakra>1000&&!src.knockedout)
		src.nin=src.Mnin*1.25
		src.gen=src.Mgen*1.4
		src.tai=src.Mtai*1.2
		src.stamina+=src.maxstamina*0.01
		src.chakra+=src.Mchakra*0.01
		src.SageModeTime--
		sleep(10)
	for(var/obj/SkillCards/Yanagi/B in src.LearnedJutsus)
		B.DelayIt(450,src,"Clan")
	for(var/obj/SkillCards/Tsubaki/C in src.LearnedJutsus)
		C.DelayIt(450,src,"Clan")
	for(var/obj/SkillCards/Karamatsu/D in src.LearnedJutsus)
		D.DelayIt(450,src,"Clan")
	for(var/obj/SkillCards/Tessenka/E in src.LearnedJutsus)
		E.DelayIt(450,src,"Clan")
	for(var/obj/SkillCards/BoneMembrane/F in src.LearnedJutsus)
		F.DelayIt(450,src,"Clan")
	for(var/obj/SkillCards/Sawarabi/G in src.LearnedJutsus)
		G.DelayIt(450,src,"Clan")
	for(var/obj/SkillCards/SawarabiAdvanced/H in src.LearnedJutsus)
		H.DelayIt(450,src,"Clan")
	for(var/obj/SkillCards/ArmBone/I in src.LearnedJutsus)
		I.DelayIt(450,src,"Clan")
	for(var/obj/SkillCards/Ibara/J in src.LearnedJutsus)
		J.DelayIt(450,src,"Clan")
	src.UpdateInv()
	src<<"You lose control of the natural energy stored up in your bones, and it slips away!"
	src.stamina=src.stamina*0.25
	src<<"You feel exhausted, and your bones feel weak and useless compared to normal..."
	src.overlays-='Icons/New Base/BoneSage.dmi'
...
mob/proc/SageMode(TimeLimit,Length,WhichAnimal)
	...
	if(src.SageMode=="Wood")//Ammmmmmmmm. Needs Icon
	...
	if(src.SageMode=="Toad")
		view()<<"[src]'s eyes resemble that of a toad's..."
		src.LearnedJutsus+= new/obj/SkillCards/SpirallingSpheres
		src.LearnedJutsus+= new/obj/SkillCards/Immediate_Giant_Rasengan
		src.LearnedJutsus+= new/obj/SkillCards/ToadSageBunshinTrap
		src.stamina+=rand(2500,4500)
		if(src.stamina>src.maxstamina*1.3)
			src.stamina=src.maxstamina*1.3
	//	src.icon='Icons/New Base/SageMonkey.dmi'
	...
	while(SageModeTime&&src.SageMode!="")
		if(src.SageMode=="Wood")
			src.nin=src.Mnin*1.65
			src.gen=src.Mgen*1.45
		...
		sleep(10)
	if(src.SageMode=="Toad")
		for(var/obj/SkillCards/SpirallingSpheres/P in src.LearnedJutsus)
			del(P)
		for(var/obj/SkillCards/Immediate_Giant_Rasengan/C in src.LearnedJutsus)
			del(C)
		for(var/obj/SkillCards/ToadSageBunshinTrap/E in src.LearnedJutsus)
			del(E)
	src.UpdateInv()
	src<<"Your Sage Mode deactivates."
	src.deathcount++
...
mob/proc/CursedSealActivation()
	if(src.CursedSealActivated||src.CursedSealActivatedS)
...
//		src.Focus=src.OldFocus
	...
	//	src.OldFocus=src.Focus
		if(src.FeralRage||src.Majest)
			src<<"Remove your boost first!";return
		src<<"You activate the Cursed Seal!"
		if(src.CursedSealPool<100)
			src.deathcount++
		else
			src.deathcount+=0.5
		if(src.StruggleAgainstDeath>0&&src.knockedout)
			src.Struggle=src.StruggleAgainstDeath+1
		else if(src.CursedSealPool>1000)
		...
		//	src.overlays+=src.CSLays
			while(src.CursedSealActivatedS)
				if(src.ChakraPool<src.MaxChakraPool*0.25)
					src.ChakraPool=0
					src.CursedSealActivatedS=0;//Ends when chakrapool is gone.
				if(src.health<100)
					src.health=100//Prevents them from dying till CS2 ends.
				var/C=src.MaxChakraPool
				if(src.chakra<src.Mchakra)
					src.ChakraPool-=(C*0.01)
					src.chakra+=(C*0.02);//Drains chakra pool and adds it to chakra at a 1:2 ratio.
				if(src.health<=1000)
					src.health+=5
					src.chakra-=5;//Recovers health from chakra if health is low.
				src.ChakraPool-=25
				if(src.nin<150)
					src.nin+=2
				if(src.gen<150)
		...
		//	src<<"You deactivate the Cursed Seal!"
		//	src.icon=usr.Oicon
			src.deathcount+=2;//So it kills you when it ends.
			src.nin=src.Mnin
			src.tai=src.Mtai
			src.gen=src.Mgen
			src.Endurance=src.MEndurance
			...
			if(src.CursedSealType=="Hell")
				...
				src.chakra+=(src.Mchakra*0.5);
				if(src.chakra>src.Mchakra*1.25)
					src.chakra=src.Mchakra*1.25
				src.stamina+=(src.maxstamina*0.5)
				if(src.stamina>src.maxstamina*1.25)
	...
	//			src.gen=src.Mgen*2;
				src.tai=src.Mtai*2
			//	src.stamina+=(src.maxstamina*0.5)
				src.stamina=src.maxstamina*2.50
		...
		//		src.chakra+=src.Mchakra*0.25
			...
			if(src.CursedSealType=="Air")//Speed type CS
				src.runningspeed=11
				src.stamina+=src.maxstamina*0.20
				if(src.stamina>src.maxstamina*1.25)
					src.stamina=src.maxstamina*1.25
				src.chakra+=src.Mchakra*0.20
			...
			while(src.CursedSealActivated)
				...
				if(prob(2)&&src.CursedSealPool<=50)
					src.deathcount++
					src.CursedSealPool++
				else if(prob(0.5))
					src.CursedSealPool++
...
mob/proc/ZeroVoidBarrier()
	...
	while(src.UsingZeroVoid)
		src.chakra-=round(src.Mchakra*0.01)
...
mob/var/tmp/UsingSpaceTimeRasengan=0
mob/proc
	Flying_Thunder_God_Mode()
		var/ED='Icons/New Base/Clothing/Member Shop Clothing/SageCloak2.dmi'
		if(src.FTG_Mode)
			src.FTG_Mode=0

			var/list/Techniques=list("FlyingThunderGod1","FlyingThunderGod2","FlyingThunderGod3","MarkerPlaceQuick")
			...
			src.LearnedJutsus+=new /obj/SkillCards/FlyingThunderGod3
	...
	Flying_Thunder_God_3()
		...
		//if (A in src.SealPlacedOn)
		if(src.Rasenganon==1)
			src<< "You already have rasengan on"
			return

		src.Rasenganon=1;src.RasenganType=0; src.RasenganCharge=1
		src.RasenganD=0
		var/B='Icons/Jutsus/Rasengan2.dmi';src.overlays-=B;src.overlays+=B
		src << "<font color=#d0bd1a><b>Hold Z to charge your space time Rasengan, Press X to use it"
		oview(11) << "<font color=#d0bd1a><b>[src] begins to charge their space time rasengan"
		src.UsingSpaceTimeRasengan = 1
		...
		if(!src.RasenganCharge)
			...
			src.RasenganD-=20
				/*sleep(50)
				if(src.knockedout)
					src << "<font color=#d0bd1a><b>You were knocked out and lost your charge"
					return
				...
				for(var/image/I in src.client.images)
					if(I.loc == A.loc)
						del(I)
				/*if(rockarmor)
					src << "<font color=#d0bd1a><b> You use more space time chakra to breakthrough [A]'s rock armor"
					src.chakra-=(src.Mchakra*0.25)
					A.rockarmor = 0
					src.RasenganD-=1000
					*/
	...
	Place_Marker_Quick()
		...
		/*	spawn(600)
			...
			src<< "You need to be facing a person on NPC !"
