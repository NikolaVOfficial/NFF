...
obj/Jutsu/Elemental/Fuuton
	...
	ShingerTornadoParts
		Part1
			icon_state="Tornado 0,0"
			pixel_x=-32
		Part2
			icon_state="Tornado 1,0"
		Part3
			icon_state="Tornado 2,0"
			pixel_x=32
		Part4
			icon_state="Tornado 0,1"
			pixel_x=-32
			pixel_y=32
		Part5
			icon_state="Tornado 1,1"
			pixel_y=32
		Part6
			icon_state="Tornado 2,1"
			pixel_x=32
			pixel_y=32
		Part7
			icon_state="Tornado 0,2"
			pixel_x=-32
			pixel_y=64
		Part8
			icon_state="Tornado 1,2"
			pixel_y=64
		Part9
			icon_state="Tornado 2,2"
			pixel_x=32
			pixel_y=64
		Part10
			icon_state="Tornado 0,3"
			pixel_x=-32
			pixel_y=96
		Part11
			icon_state="Tornado 1,3"
			pixel_y=96
		Part12
			icon_state="Tornado 2,3"
			pixel_x=32
			pixel_y=96
		Part13
			icon_state="Tornado 0,4"
			pixel_x=-32
			pixel_y=128
		Part14
			icon_state="Tornado 1,4"
			pixel_y=128
		Part15
			icon_state="Tornado 2,4"
			pixel_x=32
			pixel_y=128
	ShingerTornado
		icon_state= "Tornado 1,0"
		Move()
			...
			for(var/mob/M in view(src,3))
				var/damage=(400-(M.Endurance))
				if(damage<=50)
					damage=50
				if(M!=O)
					step_towards(M,src)
					M.DamageProc(damage,"Stamina",O,"Tornado","grey")
			..()
			for(var/obj/Jutsu/Elemental/E in view(src,3))
				if((!(istype(E,/obj/Jutsu/Elemental/Doton)))&&(!(istype(E,/obj/Jutsu/Elemental/Fuuton/WindTrail)))&&(!(istype(E,/obj/Jutsu/Elemental/Fuuton/ShingerTornado))))
					E.loc=null
				if(O&&O.TorrentialWinds>0)
					for(var/turf/G in view(src,1))
						var/found=0
						for(var/obj/Jutsu/Elemental/Fuuton/WindTrail/F in G)
							if(F in O.JutsuList)
								found=1
						if(found==0)
							var/obj/Jutsu/Elemental/Fuuton/WindTrail/W=new(G)
							W.dir=src.dir;W.Owner=O;W.Life=(O.TorrentialWinds*30);O.JutsuList+=W
		New()
			..()
			spawn(150)
				del(src)
	...
	KazeDangan
		...
		New()
			...
			spawn()
				...
				while(src)
					...
					//	else
						src.SoundEngine('SFX/WindSFX.wav')
					sleep(1)
					for(var/obj/Jutsu/P in oview(1,src))//src.loc)//Changed to a much better for loop as of 11/16/12
		...
		Bump(A)
			...
			if(ismob(A))
				...
				spawn()
					M.SoundEngine('SFX/HitStrong.wav',7)
			...
			spawn()
				while(src)
					spawn(13)
						src.SoundEngine('SFX/WindSFX.wav')
					sleep(1)
					for(var/obj/Jutsu/P in view(1,src))//src.loc)//Changed to a much better for loop as of 11/16/12
						if(istype(P,/obj/Jutsu/Elemental/Raiton))
							if(P.JutsuLevel>(src.JutsuLevel*3))
								BaseChance=32.5
							else if(P.JutsuLevel>(src.JutsuLevel*2))
...
*/
		...
		Bump(A)
			...
			if(ismob(A))
				...
				spawn()
					M.SoundEngine('SFX/HitStrong.wav')
				if(M.RaiArmor&&M.RaiArmor!=2)
					M.overlays-='Icons/Jutsus/RaiArmor.dmi';M.tempmix='Icons/Jutsus/ChakraAura.dmi';M.tempmix+=rgb(224,197,2);var/icon/Q=icon(M.tempmix);M.overlays-=Q;M.RaiArmor=0;M.Touei=0
	...
	Kakeami
		...
		Bump(A)
			...
			//	viewers()<<sound('SFX/HitStrong.wav',0,0,1)
				for(var/mob/player/E in view())
					if(E.ckey!=null)
						spawn()
							E.client.sound_system.PlaySound('SFX/HitStrong.wav')
				M.HitBack(1,src.dir)
				M.Stun=10;
				del(src)
			if(istype(A,/obj/))
	...
	//		..()
			...
			//	view(M)<<output("<font color=white>[M] was hit by Fuuton VacuumSphere!([damage])</font>","Attack")
				if(M.RaiArmor&&M.RaiArmor!=2)
					M.overlays-='Icons/Jutsus/RaiArmor.dmi';M.tempmix='Icons/Jutsus/ChakraAura.dmi';M.tempmix+=rgb(224,197,2);var/icon/Q=icon(M.tempmix);M.overlays-=Q;M.RaiArmor=0;M.Touei=0
			//	viewers()<<sound('SFX/HitStrong.wav',0,0,1)
				for(var/mob/player/Q in view())
					if(Q.ckey!=null)
						spawn()
							Q.client.sound_system.PlaySound('SFX/HitStrong.wav')
				spawn()
		...
		//	..()
			...
			//	view(M)<<output("<font color=white>[M] was hit by Fuuton VacuumSerialWaves!([damage]Health-[damage]Stamina)</font>","Attack")
				if(M.RaiArmor&&M.RaiArmor!=2)
					M.overlays-='Icons/Jutsus/RaiArmor.dmi';M.tempmix='Icons/Jutsus/ChakraAura.dmi';M.tempmix+=rgb(224,197,2);var/icon/Q=icon(M.tempmix);M.overlays-=Q;M.RaiArmor=0;M.Touei=0
			//	viewers()<<sound('SFX/HitStrong.wav',0,0,1)
				for(var/mob/player/MM in view())
					if(MM.ckey!=null)
						spawn()
							MM.client.sound_system.PlaySound('SFX/HitStrong.wav')
				spawn()
					M.Bloody();M.Bloody();M.Bloody()
				M.HitBack(5,src.dir)
				del(src)
		...
		Bump(A)
			...
			//	viewers()<<sound('SFX/Slice.wav',0,0,1)
				for(var/mob/player/MM in view())
					if(MM.ckey!=null)
						spawn()
							MM.client.sound_system.PlaySound('SFX/Slice2.wav')
				spawn()
					M.Bloody();M.Bloody();M.Bloody()
				M.HitBack(5,src.dir)
				del(src)
...
mob/proc
	...
	WindTrick(Uses)
		...
		if(src.HandsSlipped) return
		var/A=Uses*0.01
		var/image/B=image('Icons/Jutsus/Gale Wind Palm.dmi',src)
		src<<B
		spawn(20)
			del(B)
		if(A>4) A=4
	...
	KazeNoYaibaJutsu()
		if(usr.firing||usr.Frozen||usr.FrozenBind!="")
			...
			flick("Attack1",src)
			spawn() src.SoundEngine('SFX/WindSFX.wav')
			sleep(5)
...
//			K.Move_Delay=0
	...
	ShingerTornado()
		src.ChakraDrain(100)
		src.Handseals(20-src.HandsealSpeed)
		var/obj/Jutsu/Elemental/Fuuton/ShingerTornado/K=new();K.loc=src.loc;K.dir=src.dir;K.Owner=src;walk(K,src.dir,5)
		K.overlays+=/obj/Jutsu/Elemental/Fuuton/ShingerTornadoParts/Part1;K.overlays+=/obj/Jutsu/Elemental/Fuuton/ShingerTornadoParts/Part3;K.overlays+=/obj/Jutsu/Elemental/Fuuton/ShingerTornadoParts/Part4;K.overlays+=/obj/Jutsu/Elemental/Fuuton/ShingerTornadoParts/Part5;
		K.overlays+=/obj/Jutsu/Elemental/Fuuton/ShingerTornadoParts/Part6;K.overlays+=/obj/Jutsu/Elemental/Fuuton/ShingerTornadoParts/Part7;K.overlays+=/obj/Jutsu/Elemental/Fuuton/ShingerTornadoParts/Part8;K.overlays+=/obj/Jutsu/Elemental/Fuuton/ShingerTornadoParts/Part9;
		K.overlays+=/obj/Jutsu/Elemental/Fuuton/ShingerTornadoParts/Part10;K.overlays+=/obj/Jutsu/Elemental/Fuuton/ShingerTornadoParts/Part11;K.overlays+=/obj/Jutsu/Elemental/Fuuton/ShingerTornadoParts/Part12;K.overlays+=/obj/Jutsu/Elemental/Fuuton/ShingerTornadoParts/Part13;
		K.overlays+=/obj/Jutsu/Elemental/Fuuton/ShingerTornadoParts/Part14;K.overlays+=/obj/Jutsu/Elemental/Fuuton/ShingerTornadoParts/Part15;
	Atsugai()
		src.ChakraDrain(100000)
		src.Handseals(25,"Fuuton")
		if(src.HandsSlipped) return
		...
		for(var/turf/T in range(5,src))
			if(src.dir==NORTH||src.dir==SOUTH)
				if(T.y==src.y)
					var/obj/Jutsu/Elemental/Fuuton/Gust/A=new();A.name="[src]";A.Owner=src;A.JutsuLevel=src.FuutonKnowledge;A.loc=T;A.dir=src.dir;walk(A,src.dir);A.Super=1
			if(src.dir==WEST||src.dir==EAST)
				if(T.x==src.x)
					var/obj/Jutsu/Elemental/Fuuton/Gust/A=new();A.name="[src]";A.Owner=src;A.JutsuLevel=src.FuutonKnowledge;A.loc=T;A.dir=src.dir;walk(A,src.dir);A.Super=1
