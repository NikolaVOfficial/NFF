...
mob/proc/SyrupCatcher()
	if(src.FrozenBind=="Syrup")
		...
		while(src&&src.FrozenBind=="Syrup"&&src.Struggle<src.StruggleAgainstSyrup)
			...
			sleep(3)
	src<<"You broke out of the Syrup!";
	src.StruggleAgainstSyrup=0;
	src.FrozenBind=""
	src.ImmuneToSyrup=1
	spawn(300)
		src.ImmuneToSyrup=0
...
obj/Jutsu/Elemental/Suiton
	...
	WaterfallOverlay
		...
		//	var/sound/Choice=pick('SFX/waterfall.wav','SFX/Waterfall2.wav','SFX/Waterfall3.wav')
//			spawn()
//				while(src)
//					spawn(15)

//						src.SoundEngine('SFX/waterfall.wav')
			spawn(30)
				del(src)
		A
				...
				while(src)
					...
					for(var/mob/M in src.loc)
						if(M in OnlinePlayers)
							...
							if(M==src.Owner)
								damage/=2
							M.DamageProc(damage,"Stamina",src.Owner,"pull from the current","blue")
							sleep(3)
							step_to(M,J)
					sleep(5)
			spawn(300)
					...
					for(var/mob/M in src.loc)
						...
						if(M!=O&&!M.knockedout)
							if(M.Mogu)
								...
								M.Running=0;
								M.FrozenBind="Syrup";
								M.StruggleAgainstSyrup=35;
								M<<"You've been captured by Syrup! Press Space rapidly to struggle out of it!"
								if(M.shika)
									M<<"Due to being on all fours, the syrup appears to be even stronger.."
									M.StruggleAgainstSyrup=60;
								src.Captured.Add(M)
								spawn()
									M.SyrupCatcher()
					sleep(11)
	...
	SharkAA
		icon='Icons/Jutsus/Suikoudanno.dmi';icon_state="1,0";pixel_x=-16;pixel_y = -96
	SharkBB
		icon='Icons/Jutsus/Suikoudanno.dmi';icon_state="2,0";pixel_x = 16;pixel_y = -96
	SharkCC
		icon='Icons/Jutsus/Suikoudanno.dmi';icon_state="1,1";pixel_x = -16;pixel_y = -64
	SharkDD
		icon='Icons/Jutsus/Suikoudanno.dmi';icon_state="2,1";pixel_x = 16;pixel_y = -64
	SharkEE
		icon='Icons/Jutsus/Suikoudanno.dmi';icon_state="1,2";pixel_x = -16;pixel_y = -32
	SharkFF
		icon='Icons/Jutsus/Suikoudanno.dmi';icon_state="2,2";pixel_x = 16;pixel_y = -32
	SharkGG
		icon='Icons/Jutsus/Suikoudanno.dmi';icon_state="1,3";pixel_x=-16//pixel_y = 32
	SharkHH
		icon='Icons/Jutsus/Suikoudanno.dmi';icon_state="2,3";pixel_x=16//pixel_x = 32;pixel_y = 32
	SuitonSuikoudanGiant
		icon = 'Icons/Jutsus/Suikoudanno.dmi'
		icon_state="0,1"
		density = 1
		layer = MOB_LAYER+1
		var
		...
		New()
			..()
			spawn()
				overlays+=/obj/Jutsu/Elemental/Suiton/SharkAA;overlays+=/obj/Jutsu/Elemental/Suiton/SharkBB;overlays+=/obj/Jutsu/Elemental/Suiton/SharkCC;overlays+=/obj/Jutsu/Elemental/Suiton/SharkDD;overlays+=/obj/Jutsu/Elemental/Suiton/SharkEE;overlays+=/obj/Jutsu/Elemental/Suiton/SharkFF;overlays+=/obj/Jutsu/Elemental/Suiton/SharkGG;overlays+=/obj/Jutsu/Elemental/Suiton/SharkHH;
				..()
			spawn(40)
				del(src)
	/*	Move()
		...
		Bump(A)
			...
			if(ismob(A))
				...
				if(M.PaperStyleDance)
					M.PaperStyleDance=0
			//	M.DamageProc(damage,"Stamina",O)
				M.DamageProc(damage,"Stamina",O,"impact of the giant Water Shark","blue")
			//	M.DamageProc(damage2,"Health",O)
				M.DamageProc(damage2,"Health",O,"bite of the Water Shark","blue")
				if(M.RaiArmor)
					M.StunAdd(10)
					M.DamageProc(500,"Health",O)
			//	view(M)<<output("[M] was hit by Suiton Suikoudan!([damage])", "Attack")
	...
	Suigadan
		...
		New()
			...
			spawn()
				while(src)
					spawn(5)
						src.SoundEngine('SFX/WaterSFX.wav')
//					sleep(2)
					for(var/mob/M in src.loc)
						if(M.InHydro)
							M.WaterMass += 300
		...
		//	spawn(1)
			spawn()
				...
				while(src)
					if(prob(5))
						spawn(25)
							src.SoundEngine('SFX/Waterfall2.wav')
	...
	UnderWater()
		if(src.layer==MOB_LAYER)
			...
			if(locate(/obj/Jutsu/Elemental/Suiton/Owater) in src.loc)
				...
				src<<"You feel as though an unknown force calls from the nearby water..."
				return
		else
			src<<"You can't use this as you're not at a normal Layer!";
			return
mob/proc/
	Resurface()
		if(src.UnderWater!="")
			if(!(locate(/obj/Jutsu/Elemental/Suiton/Owater) in src.loc))
				src<<"You need to be underwater to come back up!"
				return
		if(src.layer!=MOB_LAYER)
				...
				if(src.UnderWater=="Light")
					if(locate(/obj/Jutsu/Elemental/Suiton/Owater) in src.loc)
						...
						if(src.FrozenBind=="UnderWater")
							src.FrozenBind=""
						src.UnderWater=""
						src<<"You have resurfaced from the water!"
		else
			src<<"You're at the normal layer, what do you need to use this for?";
			return
mob/proc/
	UnderWaterPull()
		...
		//	var/obj/Jutsu/Elemental/Suiton/Suigadan/A=new();A.loc=M.loc;A.Owner=src;A.JutsuLevel=src.SuitonKnowledge
			...
			src<<"[M] need's to be on water!"
			return
		var/Time=50
		src.icon_state="handseal"
		view()<<"[src] forms a handseal, as [M] plunges straight down into the water!"
		...
		//	src.icon_state="Handseal"
			...
			sleep(5)
		src.icon_state=""
		src<<"You release the Jutsu. Allowing [P] to resurface!"
		src.FrozenBind=""
		src.Frozen=0
		P.Resurface()
mob/proc
	...
	SuiryuudanZ()
		...
		if(src.WaterCheck())
			...
			K.name="[src]";
			K.Move_Delay=1
			if(src.WaterDragonMaster>=1&&src.WaterCheck()!=0)
				K.Move_Delay=0
			...
			usr<<"You need to be on water!";return
	SuikoudanZ(Uses)
		src.ChakraDrain(27500)
		src.Handseals(40-src.HandsealSpeed)
		if(src.HandsSlipped) return
		usr.WaterSharksLeft=1+src.WaterSharksExpert
		usr<<"You've got [src.WaterSharksLeft] uses of Water Shark Jutsu. Click on a target to send them attacking!"
		spawn(200)
			if(usr.WaterSharksLeft>0)
				usr<<"You ran out of time to finish using the Water Shark Jutsu.";usr.WaterSharksLeft=0
	...
	Senjikizame(Uses)
		...
		if(!src.WaterCheck())
			src<<"You need a water source!"
		var/SS=9
		if(src.HoldingR&&Uses>300)
	//	if(src.key=="CobraT1337"&&Uses>300)
			while(SS>=0)
	//		while(SS>0)
	...
	DaibakufuZ()
		...
		if(src.WaterCheck())
			...
			spawn(21-src.WaterChakra)
				...
				src.firing=0;
				src.icon_state=""
			var/obj/Jutsu/Elemental/Suiton/WaterfallOverlay/AX=new();
			AX.loc=src.loc;
			sleep(30-src.WaterChakra);
			spawn()
				src.CreateBubbles("Strong");
			del(AX)
			var/obj/Jutsu/Elemental/Suiton/DaiWater/AA/A=new();
			var/obj/Jutsu/Elemental/Suiton/DaiWater/BB/B=new();
			var/obj/Jutsu/Elemental/Suiton/DaiWater/CC/C=new();
			var/obj/Jutsu/Elemental/Suiton/DaiWater/DD/D=new();
			var/obj/Jutsu/Elemental/Suiton/DaiWater/EE/E=new()
			if(src.dir==SOUTH)
	...
	Daibakuryuu(Uses)
		...
		src.icon_state="handseal"
		if(src.WaterSpawn||src.WaterCheck())
			var/obj/Jutsu/Elemental/Suiton/Dwater/A=new();A.Sucking=1;A.nin=src.nin*1.7;A.loc = src.loc;A.dir = src.dir;A.Owner = src
...
mob/proc
	WaterPrison()
		...
		M.Frozen=1
		src.Frozen=1
		M << output("<b><font color=blue>You've been caught in [usr]'s Water Prison Jutsu!</font></b>","Attack")
		M << "Quick, tap space to Struggle!"
		var/Damage=src.nin*1.5
		M.Struggle=0
		while(src.icon_state=="handseal")
			if(M.InHydro)
				M<<"You absorb the water which [src] forms around you!"
	...
	HydroForm()
		if(src.InHydro)
			src.InHydro=0
			src << "Your body solidifies!"
		else
			src.InHydro = 1
			src << "You use your chakra to break down your body into a liquid substance, becoming water!"
			while(src.InHydro)
				...
				if(src.WaterMass)
					src.ChakraDrain(5*src.WaterMass)
...
mob/proc/BubbleCreation()
	...
	flick("Power",src)//bends them over
...
mob/proc/WhirlPoolControlling()
	if(src.WhirlPoolMode)
		src.WhirlPoolMode=0
		src.WhirlPoolSetUp=0
		src<<"You stop releasing Torrents of Water."
		return
	if(src.WhirlPoolSetUp)
		src<<"You stop preparing for the Torrent Release."
		src.WhirlPoolSetUp=0
		return
	else
		src.ChakraDrain(25000)
		src<<"Double Click on any player made water you created to create a whirlpool there!"//and move your mouse along it to make dangerous torrents!"
		var/TimeToUse=5
