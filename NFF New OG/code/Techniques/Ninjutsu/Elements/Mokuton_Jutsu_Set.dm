mob/var/MokutonSageAgeCapable=0
mob/var/MokutonSageAge=0
mob/var/tmp
	KusaBy
	Tsutakei=0
		...
		Bump(A)
			...
			if(ismob(A))
				...
				if(M.Kaiten||M.sphere)
					return
				var/damage=round(rand(M.maxhealth*0.05,M.maxhealth*0.10));
				...
				while(src)
					...
					if(src.icon_state=="VineGuard")
						for(var/mob/M in src.loc)
							...
							if(!CapturePerson)
								if(M.FrozenBind=="")
									M<<"You've been binded by the Leaves!";M.FrozenBind="Kusa";CapturePerson=M
					sleep(5)
			spawn(100)
				del(src)
		Del()
		...
		New()
			...
			spawn()
				...
				for(var/mob/M in src.loc)
					...
					var/damage=rand(20,30); //Was Originally Earthmastery*15
					damage+=BonusDamage
		...
		New()
			...
			spawn()
				...
				for(var/mob/M in src.loc)
					if(M.Kaiten||M.sphere)
						return
					var/mob/O=src.Owner;var/damage=rand(20,30);////Was Originally Earthmastery*15
					view(M)<<output("<font color=#BB0000 size=2>[M] was hit by the Wood Spikes!([damage])</font>","Attack")
...
mob/proc
	...
	TreeMeld()
		...
		var/TreeCheck=0
		var/turf/TreeLoc
		for(var/obj/Trees2/P in oview(15,src))
			if(P)
				TreeLoc=P.loc
				TreeCheck=1
				break
			if(!P)
				src<<"There needs to be a Tree nearby to use this technique."
		//		return
				break
			TreeCheck=1
		if(!TreeCheck)
			src<<"There wasn't a tree nearby so the technique was useless."
			return
		else
			src.Frozen=1
			src.firing=1
			...
			spawn(20)
				if(src.FrozenBind!="")
					src.FrozenBind=""
				src<<"You use your control over your chakra and the wood on the tree to merge through it..."
				src.loc=TreeLoc
