	...
	DemonMirror
		...
		New()
			...
			spawn()
				...
				src.opacity=1
				while(src)
					for(var/obj/Jutsu/Elemental/Suiton/Dwater/P in view(4,src))
						if(P.Sucking)
							del(src)
					sleep(20)
...
mob/proc
	...
	SnowAura(Uses)
		...
		if(src.WaterCheck())
			...
			src<<"You must remain standing still on the water for at least 5 seconds to create the Aura!"
			var/turf/Location=src.loc
			var/time=0
			while(src.loc==Location&&src.WaterCheck()&&time<5&&src.icon_state=="handseal")
				if(src.knockedout)
					return
				time++
				sleep(10)
			if(time>=5)
				src.icon_state=""
				src<<"You have entered your Snow Aura! It will last for 30 seconds and will harm anyone that is near you due to it's extremely low temperatures."
				var/TimeAura=30
				while(TimeAura>0&&!src.knockedout)
					TimeAura--
					for(var/mob/M in oview(2,src))
						if(M in OnlinePlayers)
							spawn()
								M.DamageProc(rand(15+(src.nin/5),30+(src.nin/5)),"Health",src,"the unreal coldness..","#00FFFF")
					sleep(10)
				src<<"Your Chakra Aura has turned back to it's normal temperature.."
			else
				src<<"You weren't able to make your Chakra as cold as snow.."
				return
	...
	IceShard(Uses)
		...
		if(src.WaterSpawn||src.WaterCheck())
			...
			if(Uses>100)
				...
				var/obj/Jutsu/Elemental/Hyouton/IceShard/CC=new();CC.loc=src.loc;CC.Owner=src;CC.dir=SOUTHWEST;walk(CC,CC.dir)
		...
		while(src.hyoushou&&!src.knockedout&&distance(src,Center)<20)
			...
			if(src.chakra>=0)
				...
				for(var/obj/Jutsu/Elemental/Hyouton/DemonMirror/S in src.JutsuList)
					del(S)
				for(var/obj/SkillCards/MakyouHyoushou/Q in src.LearnedJutsus)
					Q.DelayIt(600,src,"Clan")
				src.hyoushou=0;
