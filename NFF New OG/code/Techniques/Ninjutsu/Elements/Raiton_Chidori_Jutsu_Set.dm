		...
		New()
			...
			spawn()
				while(src)
					var/mob/O=src.Owner
					sleep(4)
				//	src.SoundEngine('SFX/LightningSFX.ogg',3,40)
	...
	ChidoriSenbon
		...
		Bump(A)
			...
			if(ismob(A))
				...
				spawn()
					if(M.client)
						M.client.sound_system.PlaySound('SFX/Slice2.wav')
...
*/
	EisouTrail
		...
		New()
			...
			//		src.SoundEngine('SFX/LightningSFX.ogg',9,60)
					for(var/obj/Jutsu/Elemental/Doton/Z in src.loc)
			...
			Execute()
				...
				for(var/mob/M in src.loc)
					if(!M.Mogu)
						...
						spawn() M.Bloody()
						spawn() M.Bloody()
						spawn() src.SoundEngine('SFX/Slice2.wav')
...
mob/proc
	Chidoriz(Type)
		if(src.Raikirion)
			src<<"You're already using Raikiri!";return
		if(src.Chidorion)
			src<<"You're already using Chidori!";return
	...
	ChidoriSenbon()
		...
		if(src.firing)
			...
			spawn(3)
				src.overlays-='Icons/Jutsus/Chidori.dmi'
			flick("Attack2",src);
			var/I=(src.ChidoriSenbonExpert*2)+rand(1,2)
			if(I>6)
				I=6
...
mob/proc/KirinWeakProc()
	...
	if(src.HandsSlipped) return
	src.overlays-='Icons/Jutsus/Chidori.dmi'
	view()<<"[src] holds up his arm to the sky"
	GettingReadyKirin=1
	...
	if(KirinCheck==1)
		...
		src.overlays+='Icons/Jutsus/Chidori.dmi';
		src<<"Click a spot to smash a bolt of lightning on the area.";
		view()<<"<font size=1><font face=verdana><b><font color=#EBEB00>[src]<font color=white> Says: This is Kirin!</font>"
		src.UsingKirin=1;
mob/proc/KirinStrongProc()
	...
	if(src.HandsSlipped)
		return
	src.overlays-='Icons/Jutsus/Chidori.dmi';
	view()<<"<font size=1><font face=verdana><b><font color=red>[src]<font color=white> Says: This is Kirin!"
