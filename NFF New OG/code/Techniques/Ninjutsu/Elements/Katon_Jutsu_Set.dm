mob/var/tmp
	...
	//Magma hits times 24 with Ghitdrain having a spawn
	//Magma hits times 16 with Ghitdrain having it's spawn removed
mob/proc/GHitDrain()
//	spawn()
	while(src.Ghit>0)
		src.Ghit--;
		...
		New()
			...
			spawn()
				...
				if(O.BlueFire)
					...
					overlays+=/obj/Jutsu/Elemental/Katon/Goukakyuu/AB;
					overlays+=/obj/Jutsu/Elemental/Katon/Goukakyuu/BB;
					overlays+=/obj/Jutsu/Elemental/Katon/Goukakyuu/CB;
					overlays+=/obj/Jutsu/Elemental/Katon/Goukakyuu/DB;
					overlays+=/obj/Jutsu/Elemental/Katon/Goukakyuu/EB;
					overlays+=/obj/Jutsu/Elemental/Katon/Goukakyuu/FB;
					overlays+=/obj/Jutsu/Elemental/Katon/Goukakyuu/GB;
					overlays+=/obj/Jutsu/Elemental/Katon/Goukakyuu/HB
				else
			...
			//				src.SoundEngine('FireSFX.wav')
					/*for(var/obj/Jutsu/P in range(1,src))
						if(istype(P,/obj/Jutsu/Elemental/Suiton/))
							...
							if(prob(BaseChance))
								del(src)
						else if(istype(P,/obj/Jutsu/Kiriame))
							del(src)
						else if(istype(P,/obj/Jutsu/Paper))
							del(P)
						sleep(1)*/
						//You need to find a more efficient way to do this.
						//Suiton should be under the Elemental loop already, since its being done anyways
						//rather than wasting CPU on a second loop.
						//Paper and Kiriame can be done differently as well.
						//Paper can be made an exception in the Elemental loop.
						//Kiriame can be done via that jutsu since its called far less than each Goukakyuu object.
						//It might be CPU taxing when it is called but overall it saves a lot of stress.
						...
						spawn()
							...
							if(M.Ghit<=0)
								if(M.Kaiten||M.sphere||M.UsingDomu||M.Mogu||M.InvulnerableToFire)
									return
								if(M.DotonArmor||M.PaperStyleDance)
									if(M.DotonArmor)
										if(prob(5)&&!M.knockedout)
											M.DotonArmor=0
											M.overlays-='Earth Armor.dmi'
									else if(M.PaperStyleDance)
										M.PaperStyleDance=0
								var/ChanceOfBurn=(O.Broil*3)/(M.FireImmunity+1)
...
//This is your lag issue right here ^
//That's extremely taxing on the CPU, checking the z map every 0.8 seconds or so
//for every SINGLE goukakyuu object.
//Gotta find a better way to do this as well if you're dead set on it.
//Perhaps making a mob tmp variable that changes when they walk into a z map. Mapping Area datums into the map that specify this can help.
//If you need more info just ask me, Sasuna.
...
//				src.nin=null
	...
	KRyuuka
		...
		Move(Pwner)
			var/mob/P = Pwner
			var/obj/Jutsu/Elemental/Katon/KRyuukaTrail/L=new();
			L.icon = src.icon;
			L.loc = src.loc;
			L.dir = src.dir;
		//	L.Owner=src.Owner
		...
		Bump(A)
			...
			if(ismob(A))
				var/mob/M = A
				var/mob/O=src.Owner;
				var/damage=round((O.nin*rand(1.6,2)));
				if(M.InHydro)
					damage /= 15
					if(prob(40))
						src.loc=null
				...
				if(M.NibiResistance==3)
					damage /=10
				M.DamageProc((damage*(1/5)),"Stamina",O,"Ryuuka","red")
				if(M&&!M.BoneMembrane)
					M.DamageProc((damage*(4/5)),"Health",O,"Ryuuka","red")
						...
						for(var/mob/M in view(2,src))
							spawn()
								if(M!=O&&(M&&O))
									...
									spawn()
										step(M,src.dir);
										M.DamageProc(O.nin*0.5,"Health",O);
									Pushes+=1
						//			sleep(3)
									if(Pushes>=PushCap)
										M.Frozen=0
...
mob/proc/Hikibou(Uses)
	...
	sleep(11)
	var/obj/Jutsu/Elemental/Katon/Goukakyuu/K=new();
	K.Move_Delay=0;
	K.loc=locate(FireLocationX,FireLocationY,FireLocationZ);
	K.nin=src.nin;
	K.Owner=src;
	K.icon_state="5";
	K.JutsuLevel=src.KatonKnowledge/4
	if(Uses>100)
		var/obj/Jutsu/Elemental/Katon/Goukakyuu/KK=new();
		KK.loc=locate(FireLocationX+1,FireLocationY,FireLocationZ);
		KK.nin=src.nin*0.5;
		KK.Owner=src;
		KK.icon_state="5";
		KK.JutsuLevel=src.KatonKnowledge/4
...
//	src.Frozen=1;
	src.FrozenBind="Goukakyuu"
	src.doingG=1
	spawn(34)
		src.doingG=0;src.FrozenBind=""//src.Frozen=0;
		src.overlays-='Icons/Jutsus/KatonTechniques.dmi';src.overlays-='Icons/Jutsus/BKatonTechniques.dmi'
...
//			M<<"[src] is using Fire Spit Barrage."
	var/random=rand(5,9)
	while(random>0)
		spawn()
			var/obj/Jutsu/Elemental/Katon/FireSpit/F=new(Pwner=src);
			F.loc=src.loc;
			F.Owner=src;
			F.nin=src.nin;;
			F.dir=src.dir
