...
//	var/turf/CurrentLoc=new(Loc)
	for(var/Length = 1 to Len) // Makes it go from 1 to Len which is the length of the bolt
		...
		sleep(pick(0.5,1))//Was just 1 but changed it to 0,1 9/20/2013 // Small delay for the bolt to take its time :)
	var/obj/TmpObj = Bolt[Bolt.len] // This is to indicate the last bolt of the chain!
	CurrentLoc=TmpObj.loc
	spawn(2)
		for(var/obj/CheckObj in Bolt)
			del(CheckObj) // Then delete the bolts.
mob/var/tmp/
	RaiArmor=0
	TatsumakiOn=0
	InGarou=0
	...
	Raikyuu
		...
		//	spawn()
				while(src)
					sleep(2)
					spawn(15)
						src.SoundEngine('SFX/LightningSFX.ogg')
		Bump(A)
			..()
	...
	TatsumakiTrail
		...
		New()
			...
			spawn(1)
				...
				while(src)
					sleep(2)
		//			src.SoundEngine('SFX/LightningSFX.ogg',5,3)
					spawn(1)
				...
				while(src)
					...
					spawn(1)
						...
						for(var/mob/M in src.loc)
							...
							if(M.Chidorion||M.Raikirion)
								flick("Attack1",M)
								view(M)<<output("<font color=yellow size=2>[M] cancels out the Raiton technique with Chidori!</font>","Attack")
	...
	Cloud
		...
		New()
			...
			spawn()
				...
				//	src.SoundEngine('SFX/LightningSFX.ogg',10,3)
					spawn(1)
...
mob/proc
	...
	Raikyuuz()
		if(src.firing||src.Kaiten||src.FrozenBind=="Shibari")
			...
			src.Handseals(30-src.HandsealSpeed)
			if(src.HandsSlipped) return
			src<<"You are charging up Raikyuu, if you've bought the passive, hold R while using this to increase the size!";src.icon_state="Power"
...
//New Garou
	RairyuuGarou()
		if(src.runningspeed<=10)
			src<<"You aren't running fast enough to use this!"
			return
		if(src.Clan=="Yotsuki")
			src.ChakraDrain(15000)
		else
		...
		src<<"You form into a tornado and dash forward"
		src.Constant()
		src.overlays-='Icons/Jutsus/Garou.dmi';src.overlays+='Icons/Jutsus/Garou.dmi'
		spawn(40)
			src.InGarou=0;src.firing=0;src.overlays-='Icons/Jutsus/Garou.dmi'
	...
	IkazuchiKiba(Uses)
		...
		if(src.HoldingR)
			...
			src<<"Hold down R while using Ikazuchi to use the Alternative attack!"
