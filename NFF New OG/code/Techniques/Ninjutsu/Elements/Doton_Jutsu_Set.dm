			...
			spawn()
				while(src)
					...
					for(var/mob/M in view(1,src))
						M.Stun+=3;
						del(src)
						view(M)<<output("<font size=2>You have fallen into a crator.</font>","Attack")
						src.CreateCrator()
	doryuuhekiT
		icon_state = "top"
		Element="Doton"
...
//				sleep(1)
	...
	//	icon_state = "5C"
			...
			if(!src.hit)
				if(src.ChuuninBoulder==1)
					...
					//			return
								...
								M<<"The Boulders lays an impacting blow upon your body, You won't be able to handle much more of those..."
								M.HitBack(9,src.dir)
						...
						//		return
								...
								M.Guarding=0;
								M.icon_state="dead"
								M<<"The Boulder Lays your body onto the ground as you struggle to get back up..Your body is weak to the point where Guarding won't even help you anymore!"
						...
						//		M.Quake_Effect_Self(M,10,4)
							...
							//		M.GotoVillageHospital()
								M<<"The final impact from the Boulder was too much for you to handle even with your guard and you fall unconcious..."
								spawn()
									M.GotoVillageHospital()
								M.TakingPartInChuunins=0
								M.BoulderStrike=0
								world<<"[M] was thrown off the Cliff trying to reach the Forest Of Death.."
								del(src)
						...
						//M<<"Nop.";
					//	world<<"Test bump not in chuunin."
					//	M.GotoVillageHospital()
					//	return
			//	else
		...
		New()
			..()
			overlays+=/obj/Jutsu/Elemental/Doton/BigDoton/A;overlays+=/obj/Jutsu/Elemental/Doton/BigDoton/B;overlays+=/obj/Jutsu/Elemental/Doton/BigDoton/C;overlays+=/obj/Jutsu/Elemental/Doton/BigDoton/D;overlays+=/obj/Jutsu/Elemental/Doton/BigDoton/E;overlays+=/obj/Jutsu/Elemental/Doton/BigDoton/F;overlays+=/obj/Jutsu/Elemental/Doton/BigDoton/G;overlays+=/obj/Jutsu/Elemental/Doton/BigDoton/H
			...
			spawn()
				...
				while(src)
					for(var/mob/M in get_step(src,src.dir))
						...
						view(M)<<output("<font color=red>[M] was hit by doseikiryuu!([damage])</font>","Attack")
						M.SoundEngine('HitStrong.wav')
	...
	Doryuudan5
		...
		Bump(A)
			...
			if(ismob(A))
				var/mob/M = A
				if(M.intank)
					M<<"The Spikes bounce off your boulder!"
					del(src)
				if(M.Kaiten||M.sphere)
					del(src)
				if(M.InHydro)
				...
				while(src)
					...
					if(src.icon_state=="DoBind")
						for(var/mob/M in src.loc)
							...
							if(M.shika)
								if(prob(10))
									M<<"Due to your quick reflexes you were able to break out of the rock bind easily!"
									M.FrozenBind="";M.EarthBindedBy=""
									del(src)
							if(M.ImmuneToDeath)
								del(src)
							if(M.SusanooIn)
								M<<"Your Susanoo causes the Rock Bind to shatter around you."
								del(src)
							if(M.sphere|M.intsu|M.ingat|M.Kaiten|M.RaiArmor|M.knockedout|M.intank)
								del(src)
							if(M.FrozenBind=="")
								M<<"You've been binded by the Rock Bind! Tap Spacebar to break out!";M.FrozenBind="DoBind";M.EarthBindedBy="[O]"
							if(M.InHydro)
								M<< "You slip out of the Rock Bind!"
								M.FrozenBind="";M.EarthBindedBy=""
								return
							sleep(11)
							if(M.Struggle>src.StruggleOutBind)
								M<<"You've forcefully broken out of Rock Bind!"
								M.FrozenBind=""
								M.EarthBindedBy=""
								del(src)
					sleep(11)
						...
						if(!M.RaiArmor)
							if(M.name!="Crows"&&M.icon!='Crows.dmi'&&M.client||M.name!="Crows"&&M.icon!='Crows2.dmi'&&M.client)
								if(!M.knockedout&&!M.ImmuneToDeath&&M.FrozenBind=="")
									...
									M.Frozen=1;
									M<<"You feel yourself being pulled in! Quickly tap the space bar to struggle out!"
									M.Struggle=0
								...
								if(M in src.CaptureList)
									if(TimeUp<=0)
										...
										M<<"You failed to struggle out and have been buried underground."
				...
				//		sleep(2)
					for(var/mob/M in src.loc)
						if(!(M in OnlinePlayers))
							continue
						if(M in OnlinePlayers)
							if(!M.RaiArmor&&!M.InGarou&&!M.knockedout&&M!=O)
								...
								if(prob(40))
									M.Running=0
								spawn()
									M.DamageProc(damage,"Health",O,"Yomi","gray")//Hdamage,"Health",O,"Earth Pillar","#BB0000")
									M.DamageProc(damage,"Stamina",O,"Yomi","gray")
							else if(M==O)
								if(O.DotonKnowledge>750)
									O.DamageProc(damage/2,"Health",O,"Yomi","gray")
								else if(O.DotonKnowledge>1000)
									O.DamageProc(damage/3,"Health",O,"Yomi","gray")
								else
									O.DamageProc(damage,"Health",O,"Yomi","gray")
					sleep(7)
			//		spawn(12)
...
mob/proc
	...
	DotonDorukiGaeshi(Uses)
		...
		//	if(Uses<=650)
				...
				src.StaminaDrain(50);src<<"You slam the ground!";flick("beastman",src)
	...
	Doton_Spear()
		...
		view()<<"[src] manipulates their earth chakra and forms a earth like Spear around their arm!"
		var/Time=75
		...
		//	src.ChakraDrain(5000)
			...
			if(src.knockedout)
				src<<"As you fall unconcious your arm turns back to normal."
				src.overlays-='Earth Spear.dmi'
				src.DotonSpear=0
				src.WeaponInLeftHand=PreviousWeaponInHand
				return
			Time--
		src<<"Your arm goes back to normal.";src.DotonSpear=0;
		src.WeaponInLeftHand=PreviousWeaponInHand
		src.overlays-='Earth Spear.dmi'
		src.overlays-='Earth Spear.dmi'
		return
	DotonKengan()
		...
		if(src.sphere||src.Kaiten||src.firing)
			return
		src.Kengan=1
		view()<<"[src] manipulates their earth chakra and forms a earth like fist around their arm!"
		var/Time=60
		while(src.Kengan&&Time>0)
			...
			if(src.chakra<=0)
				...
				src<<"Your lack of Chakra turns your fist back to normal.";return
			if(src.knockedout)
				src<<"As you fall unconcious your fist turns back to normal.";src.Kengan=0;return
			Time--
		src.Kengan=0
	...
	//	src.overlays+='Earth Armor.dmi'
		var/I='Icons/Jutsus/Earth Armor.dmi'
	//	I += rgb(120,0,0)
		src.Oicon=src.icon
		src.icon=I
//		usr.overlays+='Icons/Jutsus/BEyes.dmi'
		src.DotonArmor=1
		view()<<"[src] manipulates their earth chakra and forms a armor of rock around their body."
		src.Endurance+=100
	...
	//	src.overlays-='Earth Armor.dmi'
		src<<"Your rock armor begins to crumble!"
	...
	DotonWall()
		...
		for(var/turf/LA in getline(src,Location))
			if(istype(LA,/turf/Buildings/zDensity))
				src<<"This Jutsu isn't safe to use in this location!"
				return
	...
	MoveStopper()
		...
		if(M in oview(6))
			...
			if(src.HandsSlipped) return
			if(M.intank)
				src<<"[M] is moving at too fast of a speed! There's no way you can catch him!"
				M<<"[src] tried to place rocks around you in order to slow your movements but your boulder form stopped this!";return
	//		if(prob(70))
	//			M.Running=0;
			...
			var/turf/T2 = get_step(M,turn(M.dir,45));
			var/turf/T3 = get_step(M,turn(M.dir,-45))
			var/turf/T4 = get_step(M,turn(M.dir,90));
			var/turf/T5 = get_step(M,turn(M.dir,-90))
			for(var/N=5, N>0, N--)
	...
	DotonDoryuudanTechnique()
		...
		src.icon_state="handseal";
		src.FrozenBind="Using Doryuudan"//was src.Frozen=1
		spawn(15)
			src.FrozenBind=""
	...
	DotonDorouDomu()
		if(src.UsingDomu)
			for(var/obj/Jutsu/Elemental/Doton/DorouDomu/Objects in view(1,src))
				if(Objects.Owner==src)
					del(Objects)
			src<<"You stop using DorouDomu and release the Earth."
			src.UsingDomu=0
			return
		src.ChakraDrain(15000)
		...
		if(!src.UsingDomu)
			...
			while(src.UsingDomu)
				...
				sleep(11)
				if(prob(2))
					src<<"The chakra seeps from your hands as you hold up the earth.."
				if(usr.chakra<1)
					for(var/obj/Jutsu/Elemental/Doton/DorouDomu/Objects in view(1,src))
						if(Objects.Owner==src)
							del(Objects)
					for(var/obj/SkillCards/DorouDomu/P in src.LearnedJutsus)
						if(P.TurnedOn)
							P.TurnedOn=0
					src<<"You stop using DorouDomu and release the Earth.";
		...
		if(src.UsingDomuCrush)
			if(src.DotonCrushTarget!=null)
				...
				for(var/obj/Jutsu/Elemental/Doton/DorouDomu/Objects in range(10,src))
					if(Objects.Owner==src)
						del(Objects)
			src<<"You stop using Iwakazushi and release the Earth.";src.UsingDomuCrush=0;src.DotonCrushTarget=null;return
		src.Target()
		if(src.ntarget)
			return
		var/mob/M=src.target
		if(M.Mogu)
			src<<"[M] is underground so this jutsu will have no effect.";return
		if(M.FrozenBind=="DoBind")
			src<<"[M] is already binded by your rocks.";return
		src.ChakraDrain(20000);src.Handseals(45-src.HandsealSpeed)
		if(src.HandsSlipped) return
		if(!src.UsingDomuCrush)
			src.DotonCrushNumber=0
			src<<"Press X to crush the target or press the skillcard again to release them. The longer you hold them in the bind, the stronger the crush will be.";var/a=M.loc;sleep(35-src.EarthChakra)
			if(M.loc==a)
				...
				while(src.UsingDomuCrush)
					...
					if(usr.chakra<1)
						...
						for(var/obj/Jutsu/Elemental/Doton/DorouDomu/Objects in view(1,M))
							if(Objects.Owner==src)
								del(Objects)
						src<<"You stop using Iwakazushi and release the Earth.";src.icon_state="";src.UsingDomuCrush=0;src.DotonCrushTarget=null;return
			else
				src<<"[M] is moving too rapidly to catch them in the earth..";src.DotonCrushTarget=null;src.UsingDomuCrush=0;return
	DotonKakouSekkan()
		if(src.firing||src.Kaiten)
			return
		if(src.UsingKouka)
			if(src.KoukaTarget!=null)
				...
				for(var/obj/Jutsu/Elemental/Doton/KakouSekkan/Objects in range(10,src))
					if(Objects.Owner==src)
						del(Objects)
			src<<"You release the target from your jutsu.";src.UsingKouka=0;src.KoukaTarget=null;return
		else
			src.ChakraDrain(30000)
			src.Target()
			SekkanList=new();
			...
			M<<"You've been sealed within a stone! Quickly tap the space bar to struggle out!"
...
//			HelpfulUse+=2
		...
		if(src.HandsSlipped) return
		src.icon_state="handseal"
		src<<"Press and release Z to cancel the jutsu early."
...
mob/proc/CratorSpawn()
		...
		if(src.HandsSlipped) return
		usr<<"Click to cause a crator at that location!"
		MakeCrato=1
		spawn(200)
...
//	src.firing=1
	...
	M<<"You're being pulled into the swamp! Tab Spacebar to struggle out of it!"
//	src.firing=1;
	src.FrozenBind="Using Arijigoku"
...
//	M.firing=1
	...
	while(src.icon_state=="handseal"&&M.Struggle<MudStruggle)
		if(Time<=0)
			M.health-=2000
			M<<"You have been buried into the mud!"
		src.chakra-=9
		sleep(6)
		if(src.chakra<=0)
			src.chakra=0;
...
mob/proc/ColumnSpikes()
	...
	src.ChakraDrain(15000)
	if(src.ntarget)
		src<<"You need a target to use this jutsu."
		return
	var/mob/M = src.target
	src.Frozen=0
	src.icon_state="beastman";
	...
	src.firing=1
	var/Hitloc=M.loc
	M<<"You feel your surroundings beginning to reform."
	spawn(12)
		if(M.loc != Hitloc)
			usr<<"They were to fast to capture!"
