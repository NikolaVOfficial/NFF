...
mob/proc
	...
	FreezeTime(WhichFreezeTime)
		...
		if(FreezeTimeEffect=="Screenwide")
			...
			src<<"You're forced to stop altering the space time continuum!"
			src.Frozen=0
		if(FreezeTimeEffect=="Mapwide")
			var/TimeFrozen=10000
			var/tmp/list/Effected2 = list()
			...
			while(TimeFrozen>0)
				src.Frozen=1
				for(var/mob/M in Effected2)
					M.FrozenBind="SpaceTimeFrozen"
					TimeFrozen-=M.Struggle+5
					M.Struggle=0
				sleep(6)
			for(var/mob/M in Effected2)
				M<<"The space time continuum goes back to it's normal state and you're able to move again!"
