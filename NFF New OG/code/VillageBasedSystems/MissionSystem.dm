...
mob/proc/MissionComplete()
	src.OnMission=0
	if(src.MissionAbuseCheck)
		for(var/mob/M in OnlinePlayers)
			if(M.key==""||M.key=="DemonicK"||M.key=="Ishuri")
				M<<"[src] [src.key] [src.client.address] [src.client.computer_id] is abusing missions."
				break
	src.MissionAbuseCheck=1
	spawn(200)
		src.MissionAbuseCheck=0
...
//		src<<"You're already doing a mission!";return
	if(Rank=="D")
		...
		if(PickMission==7) // disabled
			goto Randomizer
		//	if(Village!="Leaf")
		//		goto Randomizer
		//	else
		//		src.GiveMission("Find Stray Dog",1000,6000,"The Inuzuka Clan wants some Genin to help gather stray dogs to deliver to Infants. Help them out, Will you? Bring me the Dog when you finish.")
		if(PickMission==8)
			if(Village!="Sound")
				goto Randomizer
			else
...
mob/NPC/RandomNPCs/LeafVillagers/LeafMissionJounin
	...
	verb
		Talk()
			...
			var/MissionCheck1=0
			if(usr.OnMission&&usr.CurrentMission!="Negotiating Peace With Leaf")
...
mob/NPC/RandomNPCs/SoundVillage/SoundMissionJounin
	...
	verb
		Talk()
			...
			set src in oview(3)
			set category ="Mission"
			if(usr.OnMission&&usr.CurrentMission!="Negotiating Peace With Sound")
				usr<<"You're already doing a mission!";return
...
mob/NPC/RandomNPCs/RainVillage/RainMissionJounin
	...
	verb
		Talk()
			...
			set category ="Mission"
			var/MissionCheck1=0
			if(usr.OnMission&&usr.CurrentMission!="Negotiating Peace With Rain")
				usr<<"You're already doing a mission!";return
...
proc
	MissionPayout(mob/P in world,Mission,MissionRank,Money,Village)
		...
		else if(usr.key==""||usr.key=="DemonicK")
	...
	//	else
		P.Yen+=Money
		if(MissionRank=="S")
			P.NinjaCash+=100;P.kills+=5;P.smission+=1;P.VMorale+=16;P.JutsuDelay-=30000;if(P.Subscriber) P.JutsuDelay-=1000;if(P.JutsuDelay<0) P.JutsuDelay=0;P<<"Your Village Morale went up by 16!";P<<"Your Class Level increased by 5";P<<"Your Ninja Cash went up by 100"
		else if(MissionRank=="A")
			P.NinjaCash+=50;P.kills+=4;P.amission+=1;P.VMorale+=8;P.JutsuDelay-=15000;if(P.Subscriber) P.JutsuDelay-=800;if(P.JutsuDelay<0) P.JutsuDelay=0;P<<"Your Village Morale went up by 8!";P<<"Your Class Level increased by 4";P<<"Your Ninja Cash went up by 50"
		else if(MissionRank=="B")
			P.NinjaCash+=30;P.kills+=2;P.bmission+=1;P.VMorale+=4;P.JutsuDelay-=9000;if(P.Subscriber) P.JutsuDelay-=400;if(P.JutsuDelay<0) P.JutsuDelay=0;P<<"Your Village Morale went up by 4!";P<<"Your Class Level increased by 2";P<<"Your Ninja Cash went up by 30"
		else if(MissionRank=="C")
			P.NinjaCash+=15;P.kills+=1.5;P.cmission+=1;P.VMorale+=2;P.JutsuDelay-=6000;if(P.Subscriber) P.JutsuDelay-=200;if(P.JutsuDelay<0) P.JutsuDelay=0;P<<"Your Village Morale went up by 2!";P<<"Your Class Level increased by 1.5";P<<"Your Ninja Cash went up by 15"
		else if(MissionRank=="D")
			P.NinjaCash+=5;P.kills+=1;P.dmission+=1;P.VMorale+=1;P.JutsuDelay-=2000;if(P.Subscriber) P.JutsuDelay-=100;if(P.JutsuDelay<0) P.JutsuDelay=0;P<<"Your Village Morale went up by 1!";P<<"Your Class Level increased by 1";P<<"Your Ninja Cash went up by 5"
		P.picked=0
		P.CurrentMission=""
		for(var/obj/SkillCards/S in P.LearnedJutsus)
			if(S.Uses<1000)
				S.Uses+=1
