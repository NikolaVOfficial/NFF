...
mob/proc/GiveOrganizationVerbs()
	if(!src.client)
		return
	if(src.OrganizationRank=="Leader"||src.OrganizationRank=="Co-Leader")
		src.verbs+=new/mob/Organization/Leader/verb/Announce()
		src.verbs+=new/mob/Organization/Leader/verb/SetRank()
	if(src.OrganizationRank=="Leader"||src.OrganizationRank=="Co_Leader"||src.OrganizationRank=="Recruiter")
		src.verbs+=new/mob/Organization/Leader/verb/Invite()
		src.verbs+=new/mob/Organization/Leader/verb/Boot()
	if(src.OrganizationRank=="Leader"||src.OrganizationRank=="Co-Leader"||src.OrganizationRank=="Recruiter"||src.OrganizationRank=="Member")
	...
	Leader
		verb
			Announce(msg as text)
				set name="Organization Announce"
				var/list/L = list("<")
				if(usr.CantAlert&&usr.key!=""&&usr.key!="Ishuri")
				...
				for(var/mob/M in OnlinePlayers)
					if(M.Organization==src.Organization&&M.OrganizationID==src.OrganizationID)
						M<<"<font face=verdana><font size=2><b><center>Organization Announcement:<center><font color=gray size = 2>[msg]</font>"
			SetRank()
				var/list/Menu = list()
				for(var/mob/M in OnlinePlayers)
					...
					if(M.Organization==src.Organization&&M.OrganizationID==src.OrganizationID)
						Menu.Add(M)
				var/mob/M = input("Whose Rank do you wish to alter?","Organization Ranking") as null | anything in Menu
				if(!M)return
				if(usr!=M&&M.OrganizationRank=="Leader")
					alert("You cannot change the leader's rank!");return
				var/NewRank=input(usr,"What should [M]'s new Organization Rank be?") in list("Leader","Co-Leader","Recruiter","Member","Cancel")
				if(NewRank=="Leader"&&usr.OrganizationRank!="Leader")
					usr<<"You cannot promote someone to leader when you are not the leader."
					return
