/*
	-Make sure I save for very instrument:
...
mob/owner/verb/InstrumentTest()
	set name="Test Instrument System"
	set category="Staff"
	usr.PlayInstrument("SFX/LugiaFlute.dmi")
mob
	var
		PlayingInstrument=""
		InstrumentKnowledge
mob/proc/PlayInstrument(src.InstrumetKnowledge,Difficulty,Song)
	var/Beats=Song
	var/sound/Test=src.client.sound_system.file
	while(src.client.sound_system.file)
...
obj
	BoomBox//The objects name is 'BoomBox'
		...
		verb//The Object has a verb
			...
			Music(S as sound)
				if(usr.key=="CobraT1337"||usr.key=="XXSharingan123XX")
	...
	Ocarina
		...
		verb
			Play_Melody()
				...
				if(usr.key!=""||usr.key==""||usr.key==""||usr.key==""||usr.key=="")
