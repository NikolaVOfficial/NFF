...
//				walk_to(M,src)
	if(usr.GuideWeapons)
		for(var/obj/WEAPONS/W in view(10,usr))
			walk_to(W,src)
			step_to(W,src)
			step_towards(W,src)
			walk_towards(W,src)
		/*
			if(istype(W,/obj/WEAPONS/Kunai))
				var/obj/WEAPONS/Kunai/P=W
				if(P.Owner==usr)
					step_to(P,src)
					walk_towards(P,src)
			else if(istype(W,/obj/WEAPONS/Shuriken3))
				var/obj/WEAPONS/Shuriken3/P=W
				if(P.Owner==usr)
					step_to(P,src)
			else if(istype(W,/obj/WEAPONS/Shuriken))
				var/obj/WEAPONS/Shuriken/P=W
				if(P.Owner==usr)
					step_to(P,src)
			else if(istype(W,/obj/WEAPONS/Shuriken8))
				var/obj/WEAPONS/Shuriken8/P=W
				if(P.Owner==usr)
					step_to(P,src)
			else if(istype(W,/obj/WEAPONS/Senbon))
				var/obj/WEAPONS/Senbon/P=W
				if(P.Owner==usr)
					step_to(P,src)
			else if(istype(W,/obj/WEAPONS/WindmillShuriken))
				var/obj/WEAPONS/WindmillShuriken/P=W
				if(P.Owner==usr)
					step_to(P,src)
					*/
		...
		for(var/mob/Bugs/Kekkai/K in world)
			if(K.Owner==usr)
				if(K.Frozen)
					...
					walk_to(K,src)
	else if(usr.Clan=="Kiro")
		if(usr.ShurikenMode)
			for(var/obj/WEAPONS/Shuriken/A in view(10,usr))
				if(A.icon_state=="throw")
					walk_to(A,src)
					usr.chakra-=8
	else if(usr.Clan=="Inuzuka")
		for(var/mob/pet/P in world)
			if(P.Owner == usr&&P.Frozen == 0&&P.FrozenBind=="")
				if(prob(2))
					usr.Training+=0.1;if(usr.Training>50) usr.Training=50
