		...
		RandomFace()
			...
			if(!src.client)
				if(src.name=="Masked Ninja(NPC)")
					...
					var/ored2=rand(1,150);
					var/oblue2=rand(1,150);
					var/ogreen2=rand(1,150)
				...
				if(src.name=="Boss(NPC)")
					...
					var/ogreen2=rand(1,150);
					src.tempmix2+=rgb(ored2,ogreen2,oblue2);
					src.overlays+=src.tempmix2;src.part1p=1
					...
					while(b>0)
						var/P=pick(Q);
						Q-=P;
						var/T = text2path("/obj/SkillCards/[P]")
						var/obj/SkillCards/A = new T
						A.Uses=rand(100,400);
						src.LearnedJutsus+=A
						b--
					var/obj/WEAPONS/Katana/K=new()
					src.WeaponInRightHand=K;
					src.RightHandSheath=1
					var/Base='Icons/New Base/Base.dmi';
				...
				//	src.tempmix2='Icons/New Base/Clothing/pants.dmi'
					...
					src.icon=Base;
					src.overlays-='Icons/New Base/Clothing/Boxers.dmi';
					src.overlays+='Icons/New Base/Clothing/Boxers.dmi';src.overlays-='Icons/New Base/MaleEyes.dmi';src.overlays+='Icons/New Base/MaleEyes.dmi';src.overlays+='Icons/New Base/Eyes.dmi';src.tempmix='Icons/New Base/Clothing/shirt.dmi'
					var/ored=rand(1,150);
					var/oblue=rand(1,150);
					var/ogreen=rand(1,150)
					src.tempmix+=rgb(ored,ogreen,oblue);
					src.tempmix2='Icons/New Base/Clothing/pants.dmi'
					var/ored2=rand(1,150);
					var/oblue2=rand(1,150);
					var/ogreen2=rand(1,150);
					src.tempmix2+=rgb(ored2,ogreen2,oblue2);
					src.overlays+=src.tempmix;
					src.overlays+=src.tempmix2;
					src.part1p=1
				if(src.NeedsFace&&src.part1p&&!src.tempmix3)
					...
					if(src.rank=="IDK")
						if((src.Mtai+src.Mnin+src.Mgen+src.Age)!=2000)
							...
							src.gen=src.Mgen;
							src.Age=rand(13,30)
						if((src.Mtai+src.Mnin+src.Mgen+src.Age)<=90)//105)
							src.rank="Wanna Be"
						if((src.Mtai+src.Mnin+src.Mgen+src.Age)>90)
							var/list/ElementJutsu = list("")//This will make them get their jutsu Argon
							var/list/StyleJutsu = list("")
							src.rank="Genin";
							src.TaijutsuStyle="Basic"
							src.Swift=rand(1,4)
							src.TaijutsuMastery=rand(4,6);
							src.Impenetrable=rand(1,10)
							src.Focus=rand(0,300)
							src.Buff=rand(0,150)
							...
							var/AmountOfStyles=0
							if(prob(70))
								AmountOfStyles=1
							else
								AmountOfStyles=2
							var/list/StyleChoices=list("Kenjutsu","Projectiles","Taijutsu","Medical")
							if(src.Mtai<45)
								StyleChoices-="Taijutsu"
							if(src.Mgen<40)
								StyleChoices-="Medical"
							while(AmountOfStyles>0)
								var/Choice=pick(StyleChoices)
								StyleChoices-=Choice
								src.Styles+=Choice
								sleep(1)
							if("Kenjutsu" in src.Styles)
								src.Kenjutsu+=rand(25,40)
								src.SpeedDice=rand(3,5)
								src.DeflectionZ=rand(3,5)
								src.SlashStrength=rand(1,3)
								src.Gash=1
								src.VitalSlash=pick(1,2)
								if(src.Styles.len==1)//Only +1 As, they're just Genin
									src.SlashStrength++
									src.Gash++
									src.VitalSlash++
								StyleJutsu.Add("SwiftSlash")
								if(prob(30))
									StyleJutsu.Add("RapidStrike")
								if(prob(30))
									StyleJutsu.Add("Impale")
							if("Projectiles" in src.Styles)
								src.ThrowingStrength=pick(3,4)
								src.Percision=rand(1,5)
								var/list/WeaponChoices=list("Shuriken","Kunai","Senbon")
								var/Choice=""
								src.ProjectileChoices=("")
								Choice=pick(WeaponChoices)
								WeaponChoices-=Choice
								src.ProjectileChoices+=Choice
								if(src.Styles.len==1)
									if(prob(15))
										Choice=pick(WeaponChoices)
									if(prob(15))
										StyleJutsu.Add("ChakraEnhance")
									WeaponChoices-=Choice
									src.ProjectileChoices+=Choice
								if("Shuriken" in src.ProjectileChoices)
									src.ShurikenMastery=rand(15,20)
								else if("Kunai" in src.ProjectileChoices)
									src.KunaiMastery=rand(15,20)
								else if("Senbon" in src.ProjectileChoices)
									src.SenbonMastery=rand(15,20)
								if(prob(10))
									StyleJutsu.Add("ChakraEnhance")
							if("Taijutsu" in src.Styles)
								src.Swift=rand(3,4)
								src.TaijutsuMastery=rand(5,6);
								src.Impenetrable=rand(6,10)
								src.Focus=rand(100,300)
								src.Buff=rand(50,150)
								var/list/FightingChoices=list("Basic","Gouken","Puroresu","Quick Fist","Enjouhawa","Rakanken")
						//		if(prob(90))
						//			FightingChoices-="Basic"
								if(src.Village!="Leaf")//I'm only including this as a limit on the starting style because they're genins, doubt a genin will move all the way to leaf to learn the style
									FightingChoices-="Gouken"
								if(src.Village!="Missing")
									FightingChoices-="Rakanken"
								if(src.Village!="Sound")
									FightingChoices-="Quick Fist"
									FightingChoices-="Enjouhawa"
								if(src.Village!="Rock")
									FightingChoices-="Puroresu"
								var/Chosen=pick(FightingChoices)
								src.TaijutsuStyle="[Chosen]"
								StyleJutsu.Add("Choke")
								if(src.TaijutsuStyle=="Gouken")
									StyleJutsu.Add("KonohaReppu")
									if(prob(30))
										StyleJutsu.Add("KonohaSenpuu")
								else if(src.TaijutsuStyle=="Puroresu")
									StyleJutsu.Add("Lariat")
									if(prob(30))
										StyleJutsu.Add("GuillotineDrop")
								else if(src.TaijutsuStyle=="Quick Fist")
									StyleJutsu.Add("SoundSpinningKick")
									if(prob(30))
										StyleJutsu.Add("RapidPunch")
								else if(src.TaijutsuStyle=="Enjouhawa")
									StyleJutsu.Add("Axe_Kick")
									if(prob(30))
										StyleJutsu.Add("Lion_Punch")
								else if(src.TaijutsuStyle=="Rakanken")
									StyleJutsu.Add("Asshou")
									if(prob(30))
										StyleJutsu.Add("ChouAsshou")
							if("Medical" in src.Styles)
								if(prob(90))
									StyleJutsu.Add("StyleJutsu")
									src.ShousenMastery=1
									if(prob(30))
										src.ShousenMastery++
								if(src.Styles.len==1)
									if(prob(20))
										if("Shousen" in StyleJutsu)
											src.ShousenMastery++
							src.Elements+=pick("Katon","Suiton","Doton","Fuuton","Raiton")
							if("Katon" in src.Elements)
								src.KatonKnowledge=rand(650,800)
								src.FireChakra=rand(5,15)
								src.Inferno=pick(2,3)
								src.HousenkaExpert=pick(2,3)
								src.Broil=rand(1,3)
								if(prob(80))
									src.DragonsRage=2
								else
									src.DragonsRage=1
								src.FireImmunity=rand(1,3)
								ElementJutsu.Add("Goukakyuu","Hikibou","Housenka","KatonFlameBulletAdvance","KatonHouka")
								if(prob(70))
									ElementJutsu.Add("Gouenkyuu")
								else
									ElementJutsu.Add("AshPileBurning")
							if("Suiton" in src.Elements)
								src.SuitonKnowledge=rand(650,800)
								src.WaterChakra=rand(5,15)
								src.WaterSpawn=rand(2,3)
								src.WaterPressure=rand(3,5)
								src.WaterDragonMaster=pick(1,2)
								ElementJutsu.Add("WaterCreation","Mizurappa","Suiryuudan","MizuameNabara")
								if(prob(50))
									ElementJutsu.Add("Suijinheki")
								else
									ElementJutsu.Add("Teppoudama")
							if("Doton" in src.Elements)
								src.DotonKnowledge=rand(650,800)
								src.EarthChakra=rand(5,15)
								src.LikeAStone=pick(1,2)
								src.LikeAMountain=pick(1,2)
								ElementJutsu.Add("DorukiGaeshi","Doryuuheki")
								if(prob(20))
									ElementJutsu.Add("DotonArmor")
								if(prob(50))
									ElementJutsu.Add("Doryuudan")
									src.DoryuudanMaster=pick(2,3)
								else
									ElementJutsu.Add("RisingEarthSpikes")
								if(prob(50))
									ElementJutsu.Add("SwampBrambles")
								else
									ElementJutsu.Add("TsuchiWana")
								if(src.Mtai>55)
									ElementJutsu.Add("DoryoDango")
									if(prob(50))
										src.BoulderExpert=1
									else
										src.BoulderMaster=1
							if("Fuuton" in src.Elements)
								src.FuutonKnowledge=rand(650,800)
								src.WindChakra=rand(5,15)
								src.ReppushouExpert=pick(2,3)
								src.DaitoppaExpert=pick(2,3)
								ElementJutsu.Add("Reppushou","WindTrick","Daitoppa","KazeKiri","Yaiba")
							if("Raiton" in src.Elements)
								src.RaitonKnowledge=rand(650,800)
								src.LightningChakra=rand(5,15)
								src.Static=pick(1,2)
								if(prob(25))
									src.Electromagnetivity=rand(1,2)
								var/ChidoriUser=pick(1,2)
								if(ChidoriUser==1)
									ElementJutsu.Add("Raikyuu","RairyuuGarou","RaitonYoroi","IkazuchiKiba","Hinoko")
								else
									ElementJutsu.Add("Raikyuu","Chidori","ChidoriSenbon","ChidoriNagashi")
							src.TaijutsuMastery=rand(1,3)
							if(prob(5))
								var/obj/WEAPONS/Katana/K=new();
								src.WeaponInRightHand=K;
								src.RightHandSheath=1
							var/b=ElementJutsu.len//src.Elements.len//rand(3,4)
				//			var/list/C=ElementJutsu.contents
							while(b>0)
								var/P=pick(ElementJutsu);
								ElementJutsu-=P;
								var/T = text2path("/obj/SkillCards/[P]")
								var/obj/SkillCards/A = new T
								A.Uses=rand(800,900);
								src.LearnedJutsus+=A
								b--
							var/a=StyleJutsu.len
					//		var/list/CC=StyleJutsu.contents
							while(a>0)
								var/PP=pick(StyleJutsu);
								StyleJutsu-=PP;
								var/TT = text2path("/obj/SkillCards/[PP]")
								var/obj/SkillCards/AA = new TT
								AA.Uses=rand(800,900);
								src.LearnedJutsus+=AA
								a--

////////////////////////////////////////////////////////Genin End
////////////////////////////////////////////////////////Chuunin Start
						if((src.Mtai+src.Mnin+src.Mgen+src.Age)>191)//180)
							src.rank="Chuunin";
							src.Elements=("")
							var/list/ElementPick = list("Katon","Suiton","Doton","Fuuton","Raiton")
							var/FirstElement=pick(ElementPick)
							ElementPick-=FirstElement
							src.Elements+=FirstElement
							if(prob(45))
								var/SecondElement=pick(ElementPick)
								ElementPick-=SecondElement
								src.Elements+=SecondElement
							src.TaijutsuMastery=rand(5,9);
							src.HandsealsMastery=3;
							src.HandsealSpeed=30
							src.RunningSpeed=5;
							src.Acceleration=10
							src.Elements=("")
							var/list/ElementJutsu=("")//This will make them get their jutsu Argon
							src.Elements+=pick("Katon","Suiton","Doton","Fuuton","Raiton")
							if("Katon" in src.Elements)
								src.KatonKnowledge=rand(900,1200)
								src.FireChakra=rand(15,20)
								src.Inferno=3
								src.HousenkaExpert=pick(2,3)
								src.Broil=rand(3,4)
								if(prob(95))
									...
									src.DragonsRage=1
								src.FireImmunity=rand(2,4)

								ElementJutsu.Add("Goukakyuu","Hikibou","Housenka","KatonFlameBulletAdvance","KatonHouka")
								if(prob(35))
									ElementJutsu.Add("ChouHouka")
								if(prob(25))
									ElementJutsu.Add("Gouryuuka")
								if(prob(70))
									ElementJutsu.Add("Gouenkyuu")
								else
									ElementJutsu.Add("AshPileBurning")
							if("Suiton" in src.Elements)
								src.SuitonKnowledge=rand(900,1200)
								src.WaterChakra=rand(15,20)
								src.WaterSpawn=rand(2,3)
								src.WaterPressure=rand(4,5)
								src.WaterDragonMaster=pick(1,2)
								ElementJutsu.Add("WaterCreation","Mizurappa","Suiryuudan","MizuameNabara")
								if(prob(30))
									ElementJutsu.Add("BakuSuishouha")
								if(prob(30))
									src.WaterSharksExpert=rand(2,4)
									ElementJutsu.Add("Suikoudan")
								...
								if(prob(50))
									...
									ElementJutsu.Add("Teppoudama")
							if("Doton" in src.Elements)
								src.DotonKnowledge=rand(900,1200)
								src.EarthChakra=rand(15,20)
								src.LikeAStone=pick(3,4)
								src.LikeAMountain=pick(3,4)
								ElementJutsu.Add("DorukiGaeshi","Doryuuheki")
								if(prob(40))
									ElementJutsu.Add("DotonArmor")
									if(prob(40))
										ElementJutsu.Add("KoukaArmor")
								if(prob(50))
									ElementJutsu.Add("Doryuudan")
									src.DoryuudanMaster=pick(2,3)
									if(prob(30))
										ElementJutsu.Add("DotonDoseikiryuu")
								else
									ElementJutsu.Add("RisingEarthSpikes")
									if(prob(60))
										ElementJutsu.Add("RisingEarthPillars")
								if(prob(50))
									ElementJutsu.Add("SwampBrambles")
									if(prob(70))
										src.CalloftheSwamp=pick(1,2)
										ElementJutsu.Add("YomiNuma")
								else
									ElementJutsu.Add("TsuchiWana")
									if(prob(60))
										ElementJutsu.Add("MoveStopper")
								if(src.Mtai>55)
									ElementJutsu.Add("DoryoDango")
									if(prob(50))
										src.BoulderExpert=1
										...
										src.BoulderMaster=1
							if("Fuuton" in src.Elements)
								src.FuutonKnowledge=rand(900,1200)
								src.WindChakra=rand(15,20)
								src.ReppushouExpert=pick(2,3)
								src.DaitoppaExpert=pick(2,3)
								ElementJutsu.Add("Reppushou","WindTrick","Daitoppa","KazeKiri","Yaiba")
								if(prob(25))
									ElementJutsu.Add("SpinningWind")
								if(prob(40))
									ElementJutsu.Add("VacuumSphere")
									if(prob(50))
										ElementJutsu.Add("VacuumGreatSphere")
								if(prob(50))
									ElementJutsu.Add("KazeGai")
							if("Raiton" in src.Elements)
								src.RaitonKnowledge=rand(900,1200)
								src.LightningChakra=rand(15,20)
								src.Static=pick(2,3)
								var/ChidoriUser=pick(1,2)
								if(prob(60))
									if(ChidoriUser==1)
										src.Electromagnetivity=rand(2,3)
									else
										src.Electromagnetivity=rand(1,2)
								...
								if(ChidoriUser==1)
									ElementJutsu.Add("Raikyuu","RairyuuGarou","RaitonYoroi","IkazuchiKiba","Hinoko")
									if(prob(50))
										ElementJutsu.Add("Jibashi")
									else
										ElementJutsu.Add("Touei")
								else
									ElementJutsu.Add("Raikyuu","Chidori","ChidoriSenbon","ChidoriNagashi")
									if(prob(75))
										ElementJutsu.Add("Raikiri")
										if(prob(60))
											ElementJutsu.Add("RaikiriWolf")
							if(prob(5))
								var/obj/WEAPONS/Katana/K=new();
								src.WeaponInRightHand=K;
								src.RightHandSheath=1
							var/b=ElementJutsu.len//rand(3,4)
							...
							if(prob(10))
								...
								src.RightHandSheath=1
					//		src.Inferno=1;
					//		src.HousenkaExpert=1;
					//		src.RaikyuuExpert=2;
					//		src.Static=2;
					//		src.Electromagnetivity=1;
					//		src.ReppushouExpert=1;
					//		src.WindVelocity=3
					//		var/list/C = list("Goukakyuu","Housenka","Hikibou","Ryuuka","Mizurappa","Raikyuu","Reppushou","KazeDangan","Jibashi","Doryuudan","DoryoDango")
				//			var/b=rand(3,4)
				//			while(b>0)
...
////////////////////////////////////////Chuunin End
////////////////////////////////////////Jounin Start
						if((src.Mtai+src.Mnin+src.Mgen+src.Age)>230)//225)
							src.RunningSpeed=10;
							src.Acceleration=10
							src.Inferno=3;
							src.HousenkaExpert=2;
							src.RaikyuuExpert=3;
							src.Static=3;
							src.Electromagnetivity=3;
							src.ChidoriSenbonExpert=2;
							src.NagashiExpert=2;
							src.ReppushouExpert=2;
							src.WindVelocity=3
							src.VacuumSphereExpert=2;
							src.DragonsRage=2;
							src.DragonTamer=3;
							src.rank="Jounin";
							src.TaijutsuMastery=rand(7,10);
							src.HandsealsMastery=3;
							src.HandsealSpeed=60;
							var/list/C = list("Goukakyuu","Housenka","Hikibou","Ryuuka","Mizurappa","KaryuuEndan","KatonHouka","Gouryuuka","Doryuudan","DoryoDango","YomiNuma","Doryuuheki","Reppushou","KazeDangan","VacuumWave","Kakeami","KazeKiri","KazeGai","MizuameNabara","Suiryuudan","Daikoudan","Senjikizame","Teppoudama","Raikyuu","Hinoko","Jibashi","Gian","IkazuchiKiba","ChidoriNagashi","ChidoriSenbon");var/b=rand(8,12)
							while(b>0)
								var/P=pick(C);
								C-=P;var/T = text2path("/obj/SkillCards/[P]");
								var/obj/SkillCards/A = new T
								A.Uses=rand(500,1000);
								src.LearnedJutsus+=A;
								b--
				//		if(prob(30))
				//			var/obj/WEAPONS/Katana/K=new();
				//			src.WeaponInLeftHand=K;
				//			src.LeftHandSheath=1
						if(src.Village=="Leaf")
							if(prob(25))
								src.Clan="Hyuuga";
						...
						if(src.Village=="Sound")
							if(prob(25))
								if(prob(30))
									...
									src.LeftHandSheath=1
								src.Clan="Uchiha";
								src.SharinganMastery=rand(40,1000);
								src.reflexNew=rand(1,4);
								spawn()
									src.Sharingan()
						...
						if(src.rank=="Genin")
							...
							src.maxstamina=rand(2000,3500);
						else if(src.rank=="Chuunin")
							src.Mchakra=rand(5500,7000);
							src.maxstamina=rand(4000,5750);
						else if(src.rank=="Jounin"||src.rank=="ANBU")
							src.Mchakra=rand(7500,10000);
							src.maxstamina=rand(5000,7500);
						else
							src.Mchakra=rand(3000,4000);
							src.maxstamina=rand(2000,3500);
					...
					if(src.rank=="Jounin"||src.rank=="ANBU")
						...
						if(src.Village=="Leaf")
							...
							if(src.Clan=="Hyuuga")
								src.ByakuganMastery=5001;
								src.TaijutsuMastery=10
								var/T=text2path("/obj/SkillCards/Kaiten");
								var/TT=text2path("/obj/SkillCards/KaitenKizu")
								var/obj/SkillCards/A = new T;
								src.LearnedJutsus+=A
								var/obj/SkillCards/B = new TT;
								src.LearnedJutsus+=B
						if(src.Village=="Rock")
					...
					if(src.rank=="Genin")
						...
						//		var/TT=text2path("/obj/SkillCards/KaitenKizu")
								var/obj/SkillCards/A = new T;
								src.LearnedJutsus+=A
					if(src.rank=="SGuard")//Sound Village Guard ;o
					...
					var/random = pick('Icons/New Base/Hair/WolverineH.dmi','Icons/New Base/Hair/ThreadsH.dmi','Icons/New Base/Hair/TemariH.dmi','Icons/New Base/Hair/SitkaH.dmi','Icons/New Base/Hair/ShikamaruH.dmi','Icons/New Base/Hair/shikaH.dmi','Icons/New Base/Hair/SceneH.dmi','Icons/New Base/Hair/RandomKarnHair.dmi','Icons/New Base/Hair/PartedH.dmi','Icons/New Base/Hair/Mohawk.dmi','Icons/New Base/Hair/MizukageH.dmi','Icons/New Base/Hair/MinatoH.dmi','Icons/New Base/Hair/MessyH.dmi','Icons/New Base/Hair/MadaraH.dmi','Icons/New Base/Hair/KimimaroH.dmi','Icons/New Base/Hair/DeidaraH.dmi','Icons/New Base/Hair/CaptainTousenH.dmi','Icons/New Base/Hair/AoH.dmi','Icons/New Base/Hair/AfroH.dmi','Icons/New Base/Hair/SasukeH.dmi','Icons/New Base/Hair/narutoH.dmi','Icons/New Base/Hair/JrockH.dmi','Icons/New Base/Hair/ExclusiveH.dmi','Icons/New Base/Hair/MadaraH.dmi','Icons/New Base/Hair/leeH.dmi','Icons/New Base/Hair/itachiH.dmi','Icons/New Base/Hair/inoH.dmi','Icons/New Base/Hair/WindH.dmi','Icons/New Base/Hair/hinaH.dmi','Icons/New Base/Hair/PainbodyH.dmi','Icons/New Base/Hair/SpikeyH.dmi','Icons/New Base/Hair/KiraH.dmi','Icons/New Base/Hair/KakashiH.dmi','Icons/New Base/Hair/BuzH.dmi','Icons/New Base/Hair/JiraiyaH2.dmi','Icons/New Base/Hair/RyuzakiH.dmi','Icons/New Base/Hair/OrochimaruH.dmi','Icons/New Base/Hair/SasukeTS.dmi','Icons/New Base/Hair/ShikamaruH.dmi','Icons/New Base/Hair/SleekH.dmi','Icons/New Base/Hair/SpikedPonytailH.dmi','Icons/New Base/Hair/SpikeyH.dmi','Icons/New Base/Hair/YannisH.dmi')
					src.tempmix3=random;
			...
			//		if(A>=3)
					...
					if(src.rank!="")
						var/B=rand(1,8)//Was (1,5)
						if(B==5)
							var/mask='Icons/New Base/Clothing/kakashi mask.dmi';
							var/ored5=rand(1,200);
							var/oblue5=rand(1,200)
							var/ogreen5=rand(1,200);
							mask+=rgb(ored5,ogreen5,oblue5);
							src.overlays+=mask
					var/C=rand(1,13)
					if(C==1)
		...
		NPC_Run()
	//		if(src.stamina>50)
			if(src.stamina<src.maxstamina*0.20)
				src.Running=1;
				src.icon_state="running";
				src.MoveDelay=0
			else
		...
		NPC_Regeneration()
			...
			if(!src.knockedout&&prob(15))
				...
				if(src.stamina<src.maxstamina)
					src.stamina+=rand(10,40)
		NPC_Struggle()
			if(src.rank=="Genin")
				src.Struggle+=pick(0.9,1)
			else if(src.rank=="Chuunin")
				src.Struggle+=pick(1,1.25)
			else if(src.rank=="Jounin")
				src.Struggle+=pick(1.25,2)
		...
		RockANBU()
			...
			while(src.health>0&&src.z!=20)
				...
				while(piv.len<=0)    //if there's nobody in view
					...
					sleep(src.MoveDelay*5)//wait walkspeed
					for(var/mob/m in oview(src))//for all mobs in view(excuding itself)
						if(m.client&&m.key!="DemonicK"&&m.key!="Ishuri"&&m.Village!=src.Village||m.AOS&&m.Village!=src.Village)//if M is a player or hostile NPC
							piv+=m  //it adds that to people in view)
					...
					if(attack:knockedout&&attack:StruggleAgainstDeath>0)
						...
						while(piv.len<=0)    //if there's nobody in view
							...
							for(var/mob/m in oview(2,src))//for all mobs in view(excuding itself)
								if(m.client&&m.StruggleAgainstDeath<=0&&m.key!="DemonicK"&&m.key!="Ishuri"&&m.Village!=src.Village||m.AOS&&m.Village!=src.Village&&m.StruggleAgainstDeath<=0)//if M is a player or hostile NPC
									piv+=m  //it adds that to people in view)
						attack=pick(piv)
					if(src.stamina<0)
						var/Drain=abs(src.stamina)
		...
		LeafANBU()
			...
			while(src.health>0&&src.z!=20)
				...
				while(piv.len<=0)    //if there's nobody in view
					...
					sleep(src.MoveDelay*5)//wait walkspeed
					for(var/mob/m in oview(src))//for all mobs in view(excuding itself)
						if(m.client&&m.key!="DemonicK"&&m.key!="Ishuri"&&m.Village!=src.Village&&!m.LeafVillagePermit||m.AOS&&m.Village!=src.Village)//if M is a player or hostile NPC
							piv+=m  //it adds that to people in view)
				attack=pick(piv)//pick a rand person 2 atk. remember them.
				while(get_dist(attack,src)<=viewsight)//if its in src's view
				//	sleep(1)
