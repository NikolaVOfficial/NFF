...
//1500 is the maximum any normal clothing wear will cost in Yen.
				...
				switch(input(usr,"The [src.name] costs [y2k_Uncondense_Num(src.Price)] Ryo. Do you want to buy it?", text) in list ("Yes","No."))
					if("Yes")
						if(usr.Yen>=src.Price)
							usr<<sound('SFX/click1.wav',0);usr<<sound('SFX/Cash.ogg');usr<<"You bought a [src.name]!";usr.Yen-=src.Price
	...
	//				return
		...
		Range_Extender
			...
			Click()
				if(usr.Clan!="Basic")
					usr<<"You aren't Basic. Clan Scum."
					return
			//	if(!(usr.key in Subscribers))
...
//Throwing Type
		Bow
			...
			Click()
				Price=250000
				if(usr.key!=""&&usr.key!="Ishuri"||!usr.Subscriber)
				...
				switch(input(usr,"The [src.name] costs [y2k_Uncondense_Num(src.Price)] Ryo. Do you want to buy it?", text) in list ("Yes","No"))
					if("Yes")
						...
						if(usr.Yen >= give*src.Price&&give>0)
							if(give<0)
								world<<"<font color=red>[usr] attempted to buy a negative amount of [src] as a money glitch. He wanted to buy [give] to gain [give*Price*(-1)] Ryo.</font>. His key is [usr.key]";return
							usr<<sound('SFX/Cash.wav')
					...
					if("Yes")
						...
						if(usr.Yen >= give*src.Price&&give>0)
							...
							for(var/obj/WEAPONS/Shuriken3/O in usr.contents)
								counter+=1
							if(counter<=0)
								var/obj/WEAPONS/Shuriken3/B = new();B.loc = usr;B.ammount+=give;B.name= "[B.name]"
							else
								for(var/obj/WEAPONS/Shuriken3/O in usr.contents)
									O.ammount+=give
									O.name= "[O.name]"
				...
				switch(input(usr,"The [src.name] costs [y2k_Uncondense_Num(src.Price)] Ryo. Do you want to buy it?", text) in list ("Yes","No."))
					if("Yes")
						...
						if(usr.Yen >= give*src.Price&&give>0)
							...
							usr.Yen -= give*src.Price
							var/counter=0
							for(var/obj/WEAPONS/Shuriken8/O in usr.contents)
								counter+=1
							if(counter<=0)
								var/obj/WEAPONS/Shuriken8/B = new();B.loc = usr;B.ammount+=give;B.name= "[B.name]"
							else
								for(var/obj/WEAPONS/Shuriken8/O in usr.contents)
									O.ammount+=give
									O.name= "[O.name]"
...
//Staff Type
	...
	Supplies//Supplies category
		KunaiBomb
			...
			Price=50000
			Click()
				if(usr.rank!="Jounin"||usr.rank!="S.Jounin"||usr.rank!="Hokage"||usr.rank!="Amekoutei"||usr.rank!="Tsuchikage"||usr.rank!="Otokami")
					usr<<"Sorry. I don't think you're skilled enough yet to use these. Perhaps if you were a higher ninja rank, than maybe."
					return
		//		if(usr.key!="CobraT1337"&&usr.key!="Ishuri"&&!usr.Subscriber)
		//			usr<<"Will be able to be purchased very soon!";return
				if(usr.Village=="Leaf")
					Tax=Price*(LeafWTax/10)
				if(usr.Village=="Sound")
				...
				switch(input(usr,"[src.name] costs [src.Price] Ryo. Do you wish to buy it?", text) in list ("Yes","No."))
					if("Yes")
						if(usr.Yen>=src.Price)
							...
							if(usr.Yen>=give*src.Price)
								if(give>50)
									usr<<"You can't have more than 50 packs of Ointment.";
									return
								if(give<0)
	...
	//						usr.Yen-=src.Price
							...
							if(usr.Yen>=give*src.Price)
								...
								for(var/obj/RandomEquipment/Clay/O in usr.contents)
									counter+=1
								if(counter<=0)
									var/obj/RandomEquipment/Clay/B=new()
									B.loc=usr
									B.Ammount+=give
									B.ReCheckAmount()
									if(usr.Village=="Leaf") LeafVillagePool+=Tax
...
// NINJA CASH STORE ITEMS
				...
				switch(input(usr,"The [src.name] costs [y2k_Uncondense_Num(src.Price)] Ninja Cash. Do you want to buy it?",text) in list ("Yes","No"))
					if ("Yes")
						if(usr.NinjaCash>=src.Price)
							...
							var/obj/NinjaCash/Age_Scroll/B=new();B.loc=usr
							usr.SaveK()
							return
						else
			...
			Click()
				switch(input(usr,"The [src.name] costs [y2k_Uncondense_Num(src.Price)] Ninja Cash. Do you want to buy it?",text) in list ("Yes","No"))
					if ("Yes")
						if(usr.NinjaCash>=src.Price)
							usr<<"You bought a [src.name]!";usr.NinjaCash-=src.Price
							var/obj/NinjaCash/Knowledge_Scroll/B=new();B.loc=usr
							usr.SaveK()
							return
						else
							usr<<"You dont have enough money."
	...
	NinjaCash/Weapons
		...
		DarkBlade
			...
			Click()
				switch(input(usr,"The [src.name] costs [y2k_Uncondense_Num(src.Price)] Ninja Cash.(Black flame swordd) Do you want to buy it?",text) in list ("Yes","No"))
					if ("Yes")
						if(usr.NinjaCash>=src.Price)
							usr<<"You bought a [src.name]!";usr.NinjaCash-=src.Price
							var/obj/WEAPONS/DarkBlade/B=new(); B.loc=usr
							return
						else
							usr<<"You dont have enough money."
							return
		...
		LightBlade
			...
			icon_state ="ChakraKatana"
			Click()
				switch(input(usr,"The [src.name] costs [y2k_Uncondense_Num(src.Price)] Ninja Cash. (White flame sword) Do you want to buy it?",text) in list ("Yes","No"))
					if ("Yes")
						if(usr.NinjaCash>=src.Price)
							usr<<"You bought a [src.name]!";usr.NinjaCash-=src.Price
							var/obj/WEAPONS/LightBlade/B=new(); B.loc=usr
							return
						else
							usr<<"You dont have enough money."
							return
			...
			Click()
				switch(input(usr,"The [src.name] costs [y2k_Uncondense_Num(src.Price)] Ninja Cash. Do you want to buy it?",text) in list ("Yes","No"))
					if ("Yes")
						if(usr.NinjaCash>=src.Price)
							usr<<"You bought a [src.name]!";usr.NinjaCash-=src.Price
							var/obj/WEAPONS/Evanesence/B=new(); B.loc=usr
							return
						else
							usr<<"You dont have enough money."
							return
			...
			Click()
				switch(input(usr,"The [src.name] costs [y2k_Uncondense_Num(src.Price)] Ninja Cash. Do you want to buy it?",text) in list ("Yes","No"))
					if ("Yes")
						if(usr.NinjaCash>=src.Price)
							usr<<"You bought a [src.name]!";usr.NinjaCash-=src.Price
							var/obj/WEAPONS/RoseBladeSword/B=new(); B.loc=usr
							return
						else
							usr<<"You dont have enough money."
							return
			...
			Click()
				switch(input(usr,"The [src.name] costs [y2k_Uncondense_Num(src.Price)] Ninja Cash. Do you want to buy it?",text) in list ("Yes","No"))
					if ("Yes")
						if(usr.NinjaCash>=src.Price)
							usr<<"You bought a [src.name]!";usr.NinjaCash-=src.Price
							var/obj/WEAPONS/InvisibleKaito/B=new(); B.loc=usr
							return
						else
							usr<<"You dont have enough money."
							return
			...
			Click()
				switch(input(usr,"The [src.name] costs [y2k_Uncondense_Num(src.Price)] Ninja Cash. Do you want to buy it?",text) in list ("Yes","No"))
					if ("Yes")
						if(usr.NinjaCash>=src.Price)
							usr<<"You bought a [src.name]!";usr.NinjaCash-=src.Price
							var/obj/WEAPONS/Shibuki/B=new(); B.loc=usr
							return
						else
							usr<<"You dont have enough money."
							return
	...
	Scrolls
		...
		Adjustable_Room
			...
			Click()
				...
				switch(input(usr,"The [src.name] costs [y2k_Uncondense_Num(src.Price)] Ryo. Do you want to buy it?", text) in list ("Yes","No."))
					if("Yes")
						if(usr.Clan!="Basic"&&usr.Clan!="Satou")
							usr<<"I don't think your Clan allows Puppet Masters.";return
						if(usr.Yen>=src.Price)
							usr<<sound('SFX/click1.wav',0);usr<<sound('SFX/Cash.wav');usr<<"You bought a [src.name]!";usr.Yen-=src.Price
