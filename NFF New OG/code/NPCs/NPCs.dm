...
mob/Vet
	...
	verb
		Heal()
			...
			set category = "Dog"
			if(usr.Yen < 200)
				usr<<"You need 200 Yen to pay to heal your dog!"
			else
				if(usr.hasdog)
					usr<<"Okay, I'll begin the procedure. Just give me a few moments.."
					usr.FrozenBind="Healing Dog"
					spawn(45)
						usr.FrozenBind=""
						usr<<"All done! Your pet is fully healed!"
					if(usr.PetsHealth>usr.PetsMHealth*0.90)
						if(prob(60))
							usr.DogCompanionship+=rand(0.30,0.45)
					else if(usr.PetsHealth>usr.PetsMHealth*0.80)
						if(prob(55))
							usr.DogCompanionship+=rand(0.25,0.40)
					else if(usr.PetsHealth>usr.PetsMHealth*0.65)
						if(prob(45))
							usr.DogCompanionship+=rand(0.20,0.35)
