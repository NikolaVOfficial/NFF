mob/NPC/RandomNPCs/LeafVillagers
		...
		verb/Command()
			...
			if(src in get_step(usr,usr.dir))
				if(usr.CurrentMission=="Shin's Informational")
					var/style = "<style>BODY {margin:0;font:arial;background:black;\
						color:white;}</style>"
					sd_Alert(usr, "Oh, this...This must be information on the former Otokage, Dneezy. This was over hundreds of years ago! Thank you I needed this!","[src]",,,,0,"400x150",,style);usr.CurrentMission="Shin's Informational Complete";return
			if(src in get_step(usr,usr.dir))
				if(usr.CurrentMission=="Shin's Supplies")
					var/style = "<style>BODY {margin:0;font:arial;background:black;\
						color:white;}</style>"
					sd_Alert(usr, "Its been a long time since I've played the role *Ninja* but I think I'd like to kick back into gear. Thanks for the equipment!","[src]",,,,0,"400x150",,style);usr.CurrentMission="Shin's Supplies Complete";return
	...
	Ryu
		...
		verb/Command()
			...
			set hidden=1
			if(src in get_step(usr,usr.dir))
				if(usr.CurrentMission=="Ryu's Package")//if(usr.CurrentMission=="Check On Uchiha Jack")
					usr<<"Hey thanks, I needed this package.";usr.CurrentMission="Ryu's Package Complete";usr.OnMission=0;return
				if(usr.CurrentMission=="Ryu's Informational")
					usr<<"Hey thanks, I needed this information.";usr.CurrentMission="Ryu's Informational Complete";usr.OnMission=0;return
		//			var/style = "<style>BODY {margin:0;font:arial;background:black;\
		//				color:white;}</style>"
		//			sd_Alert(usr, "Wow busy day. So what do you need to tell me?","[src]",,,,0,"400x150",,style);sd_Alert(usr, "...","[src]",,,,0,"400x150",,style);sd_Alert(usr, "I see. Tell them I got the information. I can't believe it.","[src]",,,,0,"400x150",,style);usr.CurrentMission="Ryu's Informational Complete";usr.OnMission=0;return
					...
					var/style = "<style>BODY {margin:0;font:arial;background:black;\
						color:white;}</style>"
					sd_Alert(usr, "Back in the day, I was a renowned Village Guard for Leaf.","[src]",,,,0,"400x150",,style)
	Librarian
		name="Librarian"
		icon='HakumeiGetsu.dmi'
		icon_state="Moon2"
		...
		DblClick()
			if(src in get_step(usr,usr.dir))
				if(usr.CurrentMission=="Maru's Package")
					var/style = "<style>BODY {margin:0;font:arial;background:black;\
						color:white;}</style>"
					sd_Alert(usr, "Great! I needed this.","[src]",,,,0,"400x150",,style);usr.CurrentMission="Maru's Package Complete";return
				if(usr.CurrentMission=="Maru's Informational")
					var/style = "<style>BODY {margin:0;font:arial;background:black;\
						color:white;}</style>"
					sd_Alert(usr, "Wow busy day. So what do you need to tell me?","[src]",,,,0,"400x150",,style);sd_Alert(usr, "...","[src]",,,,0,"400x150",,style);sd_Alert(usr, "I see. Tell them I got the information. I can't believe it.","[src]",,,,0,"400x150",,style);usr.CurrentMission="Maru's Informational Complete";return
				else
					var/style = "<style>BODY {margin:0;font:arial;background:black;\
						color:white;}</style>"
		...
		verb/Command()
			...
			if(src in get_step(usr,usr.dir))
				if(usr.CurrentMission=="Sora's Informational")
					sd_Alert(usr, "My hair is so pretty...","[src]",,,,0,"400x150",,style);sd_Alert(usr, "...","[src]",,,,0,"400x150",,style);sd_Alert(usr, "...Oh no!!! I think it's going to fall out now!","[src]",,,,0,"400x150",,style);usr.CurrentMission="Sora's Informational Complete";return
				else
		...
		verb/Command()
			...
			if(src in get_step(usr,usr.dir))
				if(usr.CurrentMission=="Check On Uchiha Jack")
					...
					usr.CurrentMission="Check On Uchiha Jack Complete"
					usr.OnMission=0
					return
				var/style = "<style>BODY {margin:0;font:arial;background:black;\
					color:white;}</style>"
		...
		verb/Command()
			...
			if(src in get_step(usr,usr.dir))
				...
				if(usr.CurrentMission=="Dato's Package")
					var/style = "<style>BODY {margin:0;font:arial;background:black;\
						color:white;}</style>"
					sd_Alert(usr, "Great! I needed this.","[src]",,,,0,"400x150",,style);usr.CurrentMission="Dato's Package Complete";return
				if(usr.CurrentMission=="Dato's Informational")
					var/style = "<style>BODY {margin:0;font:arial;background:black;\
						color:white;}</style>"
					sd_Alert(usr, "Wow busy day. So what do you need to tell me?","[src]",,,,0,"400x150",,style);sd_Alert(usr, "...","[src]",,,,0,"400x150",,style);sd_Alert(usr, "I see. Tell them I got the information. I can't believe it.","[src]",,,,0,"400x150",,style);usr.CurrentMission="Dato's Informational Complete";return
				else
					var/randz=rand(1,5)
					if(randz==1)
						var/style = "<style>BODY {margin:0;font:arial;background:black;\
		...
		verb/Command()
			...
			if(src in get_step(usr,usr.dir))
				if(usr.CurrentMission=="Sozai's Package")
					var/style = "<style>BODY {margin:0;font:arial;background:black;\
						color:white;}</style>"
					sd_Alert(usr, "Great! I needed this.","[src]",,,,0,"400x150",,style);usr.CurrentMission="Sozai's Package Complete";return
				if(usr.CurrentMission=="Sozai's Informational")
					var/style = "<style>BODY {margin:0;font:arial;background:black;\
						color:white;}</style>"
					sd_Alert(usr, "Wow busy day. So what do you need to tell me?","[src]",,,,0,"400x150",,style);sd_Alert(usr, "...","[src]",,,,0,"400x150",,style);sd_Alert(usr, "I see. Tell them I got the information. I can't believe it.","[src]",,,,0,"400x150",,style);usr.CurrentMission="Sozai's Informational Complete";return
				else
					var/style = "<style>BODY {margin:0;font:arial;background:black;\
						color:white;}</style>"
		...
		verb/Command()
			...
			if(src in get_step(usr,usr.dir))
				...
				if(usr.Clan=="Shiroi")
					...
					sleep(10)
					switch(input(usr,"I can teach you a technique on behalf of your efforts to reach me here, for those who wish to use snow and your own Chakra Aura and water for more destructive purposes... It's an Icy Murder...", "Aura of Snow") in list ("Yes","No"))
						if("Yes")
							if(!(locate(/obj/SkillCards/SnowShuriken) in usr.LearnedJutsus))
								usr<<"You need to learn Snow Shuriken before you're able to Learn this jutsu!";return
							usr.LearnJutsu("Hyouton: Aura of Snow",25000,"SnowAura","Aura of Snow is a Jutsu not known by many Shirois due to the remaining Shiroi in the Snow Village keeping it secret. You'll have to figure out what it does on your own....The power of the coldness of Water and your own Chakra Aura may be your guide...","Ninjutsu")
						if("No")
