...
var/flames = 1000
 ...
 // icon_state = "1"
 //TorchNew
 // icon = 'Icons/Torch.dmi'
 // icon_state = "1"— Shokai - ProtonWave, Today 1:30 PM
 // 	icon = 'Icons/Soul.dmi'
 //	icon_state = "1"
//	density = 1
obj/proc
		...
		MiddleAccess
			...
			verb/Command()
				...
				if(Choice=="Announcement")
					if(AnnounceDelay)
						usr<<"Another announcement cannot be made yet."
					else
						view(usr,6)<<"What type of announcement is this?"
						//var/Announcement=input(usr,"Village? Organization? World? You can only make one announcement every 20 minutes.") in list("Village","Organization","World")
						if("Village")
							AnnounceDelay=1
							spawn(12000)
								AnnounceDelay=0
	...
	Door
		...
		verb/Command()
			...
			if(usr.dir==NORTH&&!src.open)
				...
				if(HandsealOne=="Rooster"&&HandsealTwo=="Snake"&&HandsealThree=="Dragon")
		...
		Enter(A)
			if(ismob(A))
				...
				while(M.loc==src)
					...
					if(M.ChakraPool<M.MaxChakraPool&&M.FoxTimeChamber)
						M.ChakraPool+=M.MaxChakraPool*0.005
...
obj
	Hidden_Ninja_Scroll
		...
		proc
			...
			ScrollCheck(Time)
				spawn(Time)
					if(VillageIn==""&&src.CanBeTaken)
						src.loc=locate(86,8,15)
						world<<"The scroll has been returned to it's original spawn location."
		verb
			Get()
				set src in oview(1)
