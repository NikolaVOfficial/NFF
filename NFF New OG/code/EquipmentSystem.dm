...
obj/Clothes
	BoneMask
		name="Skull Mask"
		icon='BoneClub/SkullMask.dmi'
		bodyarea="FaceTop"
		layer = MOB_LAYER + 1
		worn = 0
		Wear()
			if(!usr.BoneClubLeader&&!usr.BoneClubMember)
				del(src)
		//		return
			if(src.worn == 1)
				src:worn = 0
				usr.name = usr.oname
				usr.overlays -= 'BoneClub/SkullMask.dmi'
				usr << "You remove the [src.name]."
				src.suffix = ""
				return
			else
...
//TorsoTop
	...
	HunterNinClothing
		...
		Wear()
			if(!usr.HunterNinMember||usr.Village!="Rain")
				if(!usr.HunterNinMember)
					usr<<"It's a damn shame you're not a Hunter Nin."
					del(src)
					usr.SaveK()
				if(usr.Village!="Rain")
					usr<<"It's a damn shame you're not in Rain."
					del(src)
					usr.SaveK()
			else
				..()
