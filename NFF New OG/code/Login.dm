var/list/WipeBenefit=list()
mob/var/AgeTenImmunity=0
mob/var/tmp/LearningInDojo=0
mob/var/WipeBenefits=0
mob/proc/menu2()
	if(!src.menus)
		src.client.screen=null
mob/var/tmp/Screener
turf/CreationScreen
	layer=1000
	TitleScreen
		density=1
...
obj/CreationScreen/NewGame
	...
	Click()
		...
		for(var/a in flist("Saves/[firstletter]/[usr.ckey]/Characters/"))
			if(R.Find(a))
				characters+="[a]"
		if(length(characters) >= 4)
			usr << "<color font=red><big><b>You have no character slots left! ([length(characters)] / 4)</b></big></font>"
			return
		usr<<sound('SFX/click1.wav',0)
		...
		Kieru
			...
			Click()
				if(usr.loggedin)
					...
					return
				usr<<sound('SFX/click1.wav',0)
				usr.loc=locate(37,182,42)
				var/count=0
				for(var/mob/M in world)
					if(M.client&&M.Clan=="Kieru")
						count++
				if(count>30)
					usr<<"There are currently [count] [src.name] on in case you want diversity"
			Description
			...
			Yes
				...
				Click()
					if(usr.Clan=="")
						usr.Clan="Kieru"
						usr<<"This clan is still a work in progress, as of now it only has 3 Clan Jutsu."
						usr.GoNext()
		Kiro
			icon_state="Kiro"
			Click()
	...
	Physique
		...
		Click()
			...
			if(Number>usr.StartingPoints)
				...
				return
			usr.TaiSkill=Number
//			else if(usr.key=="Hakudori"||usr.key=="JyuonJoon"||usr.key=="RokujinSanbi"||usr.key=="Nakagama"||usr.key=="Mighty123")
			if(usr.Clan=="Basic")
...
turf/Finished
	...
	Click()
		...
		if(usr.TaiSkill<5||usr.NinSkill<5||usr.GenSkill<5)
			...
			return
//		if(usr.Clan!="Basic"&&usr.key!="Bishamon121"&&usr.key!="Hakudori"&&usr.key!="JyuonJoon"&&usr.key!="Shrouded Light"&&usr.key!="RokujinSanbi"&&usr.key!="Nakagama"&&usr.key!="Mighty123")
		else if(usr.Clan!="Basic"&&usr.key!="Bishamon121"&&usr.key!="Shrouded Light"&&usr.key!="Mighty123")
			if((usr.TaiSkill+usr.NinSkill+usr.GenSkill+usr.StartingPoints)>50)
				...
				return
	//	else if(usr.key!="Bishamon121"&&usr.key!="Hakudori"&&usr.key!="JyuonJoon"&&usr.key!="RokujinSanbi"&&usr.key!="Nakagama"&&usr.key!="Shrouded Light"&&usr.key!="Mighty123")
		else if(usr.Clan!="Basic"&&usr.key!="Bishamon121"&&usr.key!="Shrouded Light"&&usr.key!="Mighty123")
		...
		if(usr.choseeyes&&usr.baseselected)
			...
			if(usr.Clan=="Kusakin")
				if(prob(15))
					usr.MokutonSageAgeCapable=1
					usr.MokutonSageAge=pick(18,19,20,21)
			if(usr.Clan=="Uchiha")
				usr.LearnedJutsus+= new /obj/SkillCards/Sharingan
				//var/Chance=100
				//if(prob(Chance))
...
//					Chance=35
				if(prob(Chance))
					usr.CanGetMangekyo=1
					var/ASA=list("obito","itachi","sasuke","star","bladed","gridlock","shuriken","shisui","gate","argon","crescent","nova")
			...
			if(usr.Clan=="Iwazuka")
				...
				usr.LearnedJutsus+= new /obj/SkillCards/ClayKunai
				usr.LearnedJutsus+= new /obj/SkillCards/ClayWall
			if(usr.Clan=="Kieru")
				usr.FireE=1
				usr.WindE=1
				usr.EarthE=1
			//	usr.ElementalPoints-=12
			if(usr.Clan=="Shiroi")
...
//				usr.ElementalPoints+=60
			...
			if(usr.Clan=="Basic")
				switch(usr.SecondName)
					...
					if("Hatake")
						...
						usr.WhiteChakra=1
						var/E = list("Byakugan Right", "Sharingan Right")
						usr.Doujutsu = pick(E)
					...
					if("Namikaze")
						...
						usr.LearnedJutsus+= new /obj/SkillCards/FlyingThunderGod
					if("Momochi")
	...
	//Sound
				...
				usr<<"<b>A beginning tutorial written by Kunihiko.</br>http://s8.zetaboards.com/NarutoFinalFight/topic/8211276/1/"
				usr.rank ="Genin"
				usr.score=0
				...
				usr.CanNavigate=1
				if(usr.SecondName=="Hatake")
					var/obj/Clothes/RightEyepatch/E = new()
					E.loc = usr
				if(usr.Clan=="Uchiha")
				...
				switch(input(usr,"Would you like to spawn at the Newbie's Dojo, an area where you can train in safety and learn how to play NFF?", "Enter the Tutorial Zone?") in list ("Yes","No"))
					if("Yes")
						usr.loc=locate(135,37,35)//Dojo
						usr<<"This is the Newbie's Dojo, an area to learn how to play NFF! Talk to the NPCs by pressing F to learn a few essentials if you are new. Head South to leave the Safe Zone. Note: You can't return back here."
					else
						if(usr.Clan=="Aburame")
							usr.loc=locate(130,183,1)
						else if(usr.Clan=="Akimichi")
							usr.loc=locate(104,183,1)
							usr.dir=NORTH
						else if(usr.Clan=="Kieru")
							usr.loc=locate(42,61,35)
						else if(usr.Clan=="Fuuma")
							usr.loc=locate(137,25,29)
						else if(usr.Clan=="Hoshigaki")
							usr.loc=locate(174,9,29)
			...
			//Sound
							...
							if(usr.Village=="Rock")
							//	usr.loc=locate(161,35,34)
								usr.loc=locate(79,114,34)
				switch(input(usr,"Have you looked over the NFF Rules? NOTE: You will be help responsible if you break a Rule!", "Rules") in list ("Yes","What Rules?"))
					if("What Rules?")
						usr.RuleGuide()
				if(usr.key in Subscribers)
					usr.Yen+=150000
					...
					while(x<=3)
						...
						x++
						sleep(1)
					usr<<"Thanks for helping support Naruto Final Fight!"
