...
mob/GainedAfterLogIn/verb
	SoundVolume()
		set name="Sound Effects"
		usr<<"You are changing the volume of Sound Effects"
		var/Vol = input(usr,"Your current volume for sound effects is [usr.client.sound_system.sound_volume]. Input the number you wish to change it to.") as num
		if(Vol>=100)
			Vol=100
		if(Vol<=0)
			Vol=0
		usr.client.sound_system.sound_volume=Vol
//		spawn()
//			usr.client.sound_system.PlaySound()
	MusicVolume()
		set name="Music Volume"
		usr<<"You are changing the volume of Music"
		var/Vol = input(usr,"Your current volume for Music is [usr.client.sound_system.music_volume]. Input the number you wish to change it to.") as num
		if(Vol>=100)
			Vol=100
		if(Vol<=0)
			Vol=0
		usr.client.sound_system.music_volume=Vol
		usr.client.sound_system.PlayMusic(usr.client.sound_system.current_music)
...
//	ChakraCircle
		...
		Top1
			...
			screen_loc="WEST:13,NORTH-2:-7"//"WEST:6,NORTH-2:-6"//was WEST:10
			number=1
		Top2
			icon_state="2-0"

			screen_loc="WEST+1:13,NORTH-2:-7"//"WEST+1:7,NORTH-2:-6"//"WEST+1:10,NORTH-2:-6"
			number=2
		Top3
			icon_state="3-0"

			screen_loc="WEST+2:13,NORTH-2:-7"//"WEST+2:10,NORTH-2:-6"
			number=3
		Top4
			icon_state="4-0"

			screen_loc="WEST:13,NORTH-1:-7"//"WEST:6,NORTH-1:-6"//WEST:10,NORTH-1:-6"
			number=4
		Top5
			icon_state="5-0"

			screen_loc="WEST+1:13,NORTH-1:-7"//"WEST+1:7,NORTH-1:-6"//WEST+1:10,NORTH-1:-6"
			number=5
		Top6
			icon_state="6-0"

			screen_loc="WEST+2:13,NORTH-1:-7"//WEST+2:10,NORTH-1:-6
			number=6
		Top7
			icon_state="7-0"

			screen_loc="WEST:13,NORTH:-7"//WEST:10,NORTH:-6"
			number=7
		Top8
			icon_state="8-0"

			screen_loc="WEST+1:13,NORTH:-7"//WEST+1:10,NORTH:-6"
			number=8
		Top9
			icon_state="9-0"

			screen_loc="WEST+2:13,NORTH:-7"//"WEST+2:10,NORTH:-6"
			number=9
	...
	ChakraRim
		...
		Part13
			icon_state="3,1"
			screen_loc="WEST+3,NORTH-2:10"
		Part14
			icon_state="2,1"
			screen_loc="WEST+2,NORTH-2:10"
		Part15
			icon_state="3,2"
			screen_loc="WEST+3,NORTH-1:10"
		Part16
			icon_state="2,2"
			screen_loc="WEST+2,NORTH-1:10"
		Part17
			icon_state="3,3"
			screen_loc="WEST+3,NORTH:10"
		Part18
			icon_state="2,3"
			screen_loc="WEST+2,NORTH:10"
	HealthSegment
		var
			Num=0
			Width=160
		...
		if(src.TypeOfHud=="Swirl")
			for(var/obj/HUD/ChakraCircle/A in src.client.screen)
				...
				if(src.chakra>src.Mchakra*0.95)
					...
					if(istype(A,/obj/HUD/ChakraCircle/Top9/))
						A.icon_state="glow9"
				else if(src.chakra>src.Mchakra*0.75&&src.chakra<=src.Mchakra*0.95)
					if(istype(A,/obj/HUD/ChakraCircle/Top1/))
						sleep(2);flick("glow1",A)
					if(istype(A,/obj/HUD/ChakraCircle/Top2/))
						sleep(2);flick("glow2",A)
					...
					if(istype(A,/obj/HUD/ChakraCircle/Top9/))
						sleep(2);flick("glow9",A)
				else if(src.chakra>src.Mchakra*0.25&&src.chakra<=src.Mchakra*0.75)
					if(istype(A,/obj/HUD/ChakraCircle/Top1/))
						A.icon_state="1"
					if(istype(A,/obj/HUD/ChakraCircle/Top2/))
						A.icon_state="2"
					...
					if(istype(A,/obj/HUD/ChakraCircle/Top9/))
						A.icon_state="9"
				else if(src.chakra<=src.Mchakra*0.25&&src.chakra>0)
					if(istype(A,/obj/HUD/ChakraCircle/Top1/))
						flick("flick1",A)
					if(istype(A,/obj/HUD/ChakraCircle/Top2/))
						flick("flick2",A)
