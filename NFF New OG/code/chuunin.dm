...
mob/ChuuninJoin/verb/SpectateChuunin()
	...
	if(!(locate(usr in Spectators)))
		...
		usr<<"You're already going to spectate this Chuunin exam!";return
mob/ChuuninHelp/verb/ChuuninAnnounce(txt as text)//world <<
	set category = "Chuunin"
	set name = "Announce Message to Contestants"
	for(var/mob/M in OnlinePlayers)
		spawn()
			if(M.TakingPartInChuunins)
...
mob/Kage
	verb
		...
		//		usr<<"You need to have at least 20 Genin on to start a chuunin!";return
			...
			if(!Chuunintime&&!Chuunintime2)
				usr.verbs+=new/mob/ChuuninHelp/verb/ChuuninAnnounce()
				...
				for(var/mob/M in OnlinePlayers)
					...
					else if(M.client&&(M.rank=="S.Jounin"||M.rank=="Jounin"))
...
//				world<<"<b>If you wish to watch Chuunin exams go to the Exam room and talk to the ticket guy!"
				...
				for(var/mob/M in OnlinePlayers)
					M.verbs-=new/mob/ChuuninJoin/verb/JoinChuunin()
					M.verbs-=new/mob/ChuuninJoin/verb/SpectateChuunin()
				world<<"<font size=3><font color=red>Chuunin exam has now started!</font>"
				world<<"<font size=3><font color=red>Part 1 being selected...</font>"
				sleep(30)
		//		WhichType=1
				WhichType=pick(1,2)
				if(WhichType==1)
					world<<"<font size=3><font color=red> Rock Village Round One Selected!</font>"
				else
					world<<"<font size=3><font color=red> Iron Country Round One Selected!</font>"
				...
				if(WhichType==1)
					for(var/mob/M in OnlinePlayers)
						if(M.TakingPartInChuunins==1)
							M.loc=locate(rand(5,12),rand(8,11),38)
							M.BoulderStrike=0
							M.TakingPartInChuunins=1
							M<<"Reach the end without getting hit by the bolders. A bolder will immediatly knock you out, however blocking a boulder will only push you back. Make it to the end!"
			...
			//			M.loc=locate(1,187,32)
			//			M<<"<b><font color=red>Welcome to Part 1! This part is severly easy and a test of agility and endurance for Ninja. The point of this test is to make it across the opsticals to the other region. Head downwards, but of course there is a path. As a ninja you must be able to get through opsticals and find your way in a moderate time limit. You have 3 minutes!"
		//		world<<"<font size=3><font color=red>The contestants are going through part 1 currently!</font>"
				sleep(1100)
...
//				world<<"<b>If you wish to watch Chuunin exams go to the Exam room and talk to the ticket guy!"
				...
				//	spawn()
					spawn()
						...
						C<<"Jutsu usage is not allowed while watching these matches. Attempting to abuse into the Arena Fights will result in Punishment as it is counted as abuse."
