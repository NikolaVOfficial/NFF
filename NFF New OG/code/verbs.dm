mob/GainedAfterLogIn/verb/Send_File(mob/m in view(usr),F as file)
	...
	switch(alert(m,"[usr] is trying to send you [F].  Do you accept the file?","**File Transfer**","Yes","No"))
		if("Yes")
			m<<"Tranfers have been disabled."
			return
//			alert(usr,"[m] accepted the file","**File Accepted**")
//			m<<ftp(F)
		if("No")
			m<<"Transfers have been disabled."
			return
...
mob/GainedAfterLogIn/verb/FixMeImStuck()
	set hidden=1
	if(usr.FrozenBind=="WaitingForTournament")
		usr<<"You know exactly why your stuck, you joined the tournament, it even says you're supposed to be frozen..."
		return
	if(usr.ImStuck)
...
mob/GainedAfterLogIn/verb/IgnoreOOC()
	...
	set category = "Commands"
//	if(usr.key!="CobraT1337"&&usr.key!="Ishuri")
...
mob/GainedAfterLogIn/verb
	...
	//		msg = "Wow these penises in my mouth taste quite sweet."
		...
		if(findtext(msg,"byond://")||findtext(msg,"www.byond.com/games/"))
			usr<<"If you want to advertise, advertise on the forums. http://z8.invisionfree.com/NarutoFinalFight/index.php?act=idx"
			return
		else if(!worldC)
			...
			if(!worldC)
				...
				//	usr << "Spam rate exceeded please wait a moment and try again!"//tell him to wait for a second
					...
					for(var/mob/M in viewers(8,usr))
						if(istype(M,/mob/AstralSoul/))
							var/mob/UR=M.Owner
							if(findtext(msg,usr.name)) //If your name is in your text
								if(UR != usr)
									if(!(usr.name in UR.namesknown)) //If they don't know your realname...
										UR.namesknown += usr.name
							if(usr.TitleName!=""&&!usr.InHenge)
								if(usr.name in UR.namesknown)
									UR<<"<font face=trebuchet MS><b><font color=[rgb(rand(250,254),0,0)]>{[usr.TitleName]}[usr]<font color=white> Says: [html_encode(msg)]</font></b>"
								else
									UR<<"<font face=trebuchet MS><b><font color=[rgb(rand(250,254),0,0)]>{[usr.TitleName]}[usr.Village]<font color=white> Says: [html_encode(msg)]</font></b>"
							else
								if(usr.name in UR.namesknown)
									UR<<"<font face=trebuchet MS><b><font color=red>[usr]<font color=[rgb(rand(250,254),0,0)]> Says: [html_encode(msg)]</font></b>"
								else if(!usr.InHenge)
									UR<<"<font face=trebuchet MS><b><font color=[rgb(rand(250,254),0,0)]>[usr.Village] Ninja<font color=white> Says: [html_encode(msg)]</font></b>"
								else
									UR<<"<font face=trebuchet MS><b><font color=[rgb(rand(250,254),0,0)]>[usr.name]<font color=white> Says: [html_encode(msg)]</font></b>"
						if(istype(M,/mob/Clones/))
							var/mob/Clones/MX=M
							if(MX.CloneType=="KBunshin")
								var/mob/U=M.Owner
...
mob/GainedAfterLogIn/verb/Who()
	...
	for(var/mob/M in world)
		if(M.client&&M.KeyVar==""&&M.key!="DemonicK"&&M.key!="Ishuri"&&M.key!="TheIvelShadow"&&M.key!="DarkLinkofthePast"&&M.key!=""&&M.key!=""&&M.loggedin) //Ivel requested silent login
			count += 1
			html += "<tr"
			if(M==usr)
	...
	var/count2=0
	for(var/mob/M in world)
		if (M.client&&M.key!="Roflcakesk"&&M.key!="Darkaizer"&&M.key!="DemonicK"&&M.key!="Ishuri"&&!M.loggedin)
			count2 += 1
			html += "<tr"
			if(M==usr)
				html+=" style=\"color: red; font-weight: bold;\""
...
mob/GainedAfterLogIn/verb/Savenow()
	...
	var/calcLag = abs(world.cpu - 100)
	if(calcLag<=20)
		usr<<"The server is undergoing heavy usage and saving is currently disabled for safety reasons."
		return
	...
	if(usr.cansave)
		...
		fdel("Saves/[firstletter]/[src.ckey]/Characters/[src.savename].sav")
		var/savefile/F=new("Saves/[firstletter]/[src.ckey]/Characters/[src.savename].sav")
