...
//////////////
	...
	goldinbank=0
	Element = ""
	NatureChakra = 0 // new sage chakra variable
	SageMastery = 1
//Stats
	tmp/tai=25
	tmp/gen=25
...
var
	highestTaijutsuKnowledge = 0
	highestNinjutsuKnowledge = 0
	highestGenjutsuKnowledge = 0
	highestKatonKnowledge = 0
	highestSuitonKnowledge = 0
	highestRaitonKnowledge = 0
	highestDotonKnowledge = 0
	highestFuutonKnowledge = 0
	highestKunaiMastery = 0
	highestShurikenMastery = 0
...
//Iwazuka
	...
	ClayDrain=0
	QuickClay=0
	list/KibakuAbilities=list("Bird","Spider","Grasshopper","Spear","Wall","Flower","ClaySenbon","Spear","Mine")
//Kaguya
	BoneMastery=0//MaxLevel=50, Needed to learn Kaguya bone techniques.
	BoneArmor=0//MaxLevel=3, Increases defence and such.
	DanceMastery=0//MaxLevel=30, Increases the attack of Dances.
	Underlayer=0//MaxLevel=1, Decreases damage of health based attacks.
	SawaMaster=0
...
//Ketsueki
	...
	ThirstHold=0
//Kumojin
	SpiderMastery=0 //100 max
	WebMastery=0 // 50 max
	WebStrength=0 //5 max
	BodyofanArachnid=0 //1 max
	GoldnowDiamond=0 //5 max
	GoldenSpike=0 //10 max
//Kusakin
...
mob
	...
	//		src.health=0
		...
		//		src.Running=1
			...
			if(src.Village!="Missing")
				...
				if(src.Village == "Leaf")
					stat("Kage Village Message: [leafVillageMessage]")
				else if(src.Village == "Rain")
					stat("Kage Village Message: [rainVillageMessage]")
				else if(src.Village == "Rock")
					stat("Kage Village Message: [rockVillageMessage]")
				else if(src.Village == "Sound")
					stat("Kage Village Message: [soundVillageMessage]")
				stat("Village Morale: [VMorale]")
			if(src.Clan!="Basic")
				stat("Clan: [Clan]")
				if(src.Clan == "Hoshigaki")
					if(src.samehadeEquipped == 1)
						stat("Samehade Chakra: [samehadeChakara]")
			stat("Rank: [rank]")
			stat("Class: [blevel]")
			...
			stat("-------------------{Stats}-------------------")
			if(src.Age>=13)
				stat("Primary Trait: [src.Trait]");
				if(src.Age>=17)
					stat("Secondary Trait: [src.Trait2]")
		//	if(src.Clan=="Hyuuga"&&src.Age>=25) stat("Branch: [src.HyuugaClanBranch]");
			stat("Vitality: [src.health] / [src.maxhealth] [(health/maxhealth)*100]%")
			stat("Stamina: [src.stamina] / [src.maxstamina] [(stamina/maxstamina)*100]%")
			stat("Chakra: [src.chakra] / [src.Mchakra] [(chakra/Mchakra)*100]%")
			stat("DeathCount: [src.deathcount]")
	...
	//			stat("Eye Mastery: [src.ConversusinMastery]");
		...
		//	stat("Control: [gg*10]%/Stat: [src.GenSkill]")
			stat("Physique Stat: [src.tai/3] ([tt*10]%)")
			stat("Capacity Stat: [src.nin/3] ([nn*10]%)")
			stat("Control Stat: [src.gen/3] ([gg*10]%)")
			...
			if(src.FireE||src.WaterE||src.LightningE||src.EarthE||src.WindE)
				...
				if(src.WindE)
					stat("Fuuton")
				if(src.Clan=="Shiroi"||src.Clan=="Kusakin"||src.Clan=="Kieru")
					if(src.FireE&&src.WindE&&src.EarthE&&src.Clan=="Kieru")
						stat("Jinton")
	...
	//			stat("Sand: [SandAmount]")
			stat("SageChakra: [NatureChakra]/600")
			if(src.CanNavigate)
			...
			//		stat("Happiness: [PetsHappyness]")
					...
					stat("Hunger: [pethunger]%")
					stat("Thirst: [petthirst]%")
			statpanel("Missions")
			...
			stat("Challenge Rating: [challengeRating]")
...
mob/proc/LearnThisJutsu()
	if(src.JutsuEXPCost>=src.JutsuMEXPCost)
		...
		if(src.JutsuInLearning=="Rakanken")
			...
			src.TypeLearning=""
			return
		if(src.JutsuInLearning=="Puroresu")
			src.TaijutsuStyle="Puroresu"
			src.JutsuDelay=src.JutsuMEXPCost
			src.JutsuInLearning=""
			src.JutsuEXPCost=0
			src.JutsuMEXPCost=0;src.exp=0
		...
		if(src.JutsuInLearning=="Water Spawn")
			...
			var/obj/O = new T
			src.ElementalPoints-=src.ElementalPointDeduction
			if(src.ElementalPoints<0)
				src.ElementalPoints=0
				src.JutsuDelay=src.JutsuMEXPCost
				src<<"Because you don't have enough Elemental Points you were unable to learn the jutsu."
				src.JutsuInLearning=""
				src.JutsuEXPCost=0
				src.JutsuMEXPCost=0;
				src.exp=0
...
turf
	DblClick()
		...
		var/JukaiKoutanOn=0
		if(usr.Clan=="Kusakin")
			if(usr.SageMode=="Wood")
				JukaiKoutanOn=1
			for(var/obj/Jutsu/Elemental/Mokuton/JukaiKoutan2/FA in oview(10,usr))
				if(FA.Owner==usr)
					JukaiKoutanOn=1
		if(usr.Spikesleft>0&&!usr.firing)
			usr.Spikesleft--
			usr.MakeSpike(src)
		if(usr.Charging&&JukaiKoutanOn&&usr.TreeCreationLimit>0)
			var/obj/Jutsu/Elemental/Mokuton/JukaiKoutan/A=new()
			A.loc=src;A.Owner=usr
			A.BindingMob=1
			usr.TreeCreationLimit-=1
			if(usr.SageMode=="Wood")
				usr.chakra-=300
			usr.chakra-=200
		if(usr.ControllingAmat)
			for(var/obj/SkillCards/FieldOfBlackFlames/Y in usr.LearnedJutsus)
				Y.DelayIt(900,usr)
			usr.ControllingAmat=0
			for(var/turf/X in oview(3,src))
				var/obj/Jutsu/Uchiha/AMATrail/P=new();P.Owner=usr;
				P.loc=X
		if(usr.ControllingSawa&&!usr.UsingFuuinjutsuHandseal&&usr.FrozenBind!="Spectating Chuunin"&&!usr.CastingGenjutsu)
		...
		if(usr.Trait!="Speedy"&&usr.shunshin&&!usr.CastingGenjutsu&&!usr.UsingFuuinjutsuHandseal&&!usr.SoundSpinningKick&&!usr.LegSliced&&!usr.Guarding||usr.controlled)
			var/mob/USER
			if(usr.controlled)
				USER=usr.controlled
			else
		...
		//		return
			...
			if(src&&!src.density&&usr.client&&!USER.UsingFuuinjutsuHandseal&&usr.cantele&&USER.FrozenBind==""&&!USER.Frozen&&USER.Stun<1&&USER)
				if(src.z!=USER.z)
					return
				flick("zan",USER)
				if(USER.Clan=="Sabaku")
...
//			world<<"Test"
			...
			if(usr.MindGenjutsuReady&&usr!=src)
				...
				if(usr!=src)
					...
					if(src.name in usr.namesknown)
						...
						if(usr.Clan=="Inuzuka")
							if(usr.PetTrait=="Pact Protection"&&src.client)
								usr<<"Your canine will now automatically drag [src] torwards you when they are knocked out."
								usr.PactProtect=1
								src.PactProtected=1
								for(var/mob/pet/P in oview(10,usr))
									if(P.Owner==usr)
										P.protecting=src
						if(!(src in usr.Allies))
							...
							if(usr.Clan=="Inuzuka")
								if(usr.PetTrait=="Pact Protection"&&src.client)
									src<<"[usr]'s canine will now automatically drag you torwards them when you faint. Taking you away from the battlefield to recover."
									src.PactProtected=1
									usr.PactProtect=1
									for(var/mob/pet/A in oview(10,usr))
										if(A.Owner==usr)
											A.protecting=src
						...
						if(!(src in usr.Allies))
							...
							if(usr.Clan=="Inuzuka")
								if(usr.PetTrait=="Pact Protection"&&src.client)
									...
									usr.PactProtect=1
									src.PactProtected=1
									for(var/mob/pet/PP in oview(10,usr))
										if(PP.Owner==usr)
											PP.protecting=src
						usr.Allies.Add(src)
			return
	...
	DblClick()
		if(src!=usr)
			if(usr.GM)
				if(src.key=="KingsHake"||src.key=="DemonicK"||src.key=="Zigo"||src.key=="Darkaizer"||src.key==""||src.key=="Ishuri")
					return
...
mob/Akat
	verb/AkatInvite(mob/M in world)
		...
		if(M.Bijuu!="")
			usr<<"Jinchuuriki's aren't allowed in the Akatsuki...."
			return
		if(M.Village!="Missing")
			usr<<"They can't be in a village."
			return
		if(M.blevel!="B"&&M.blevel!="A"&&M.blevel!="S")
			usr<<"They aren't a high enough rank."
			return
...
mob/ShindaKyodai
	verb/Invite(mob/M in world)
		...
		switch(input(M,"Would you like to join Shinda Kyodai?","Join?","") in list ("Yes","No"))
			if("Yes")
				if(!M.ShindaKyodaiLeader&&!M.ShindaKyodaiMember)
					M.ShindaKyodaiMember=1
					M.verbs+=typesof(/mob/ShindaKyodaiMember/verb)
				else
	...
	verb/Boot(mob/M in world)
		set category="Clan"
		set name="Exile from Shinda Kyodai"
		if(M.ShindaKyodaiMember)
			M.ShindaKyodaiMember=0
			M.verbs-=typesof(/mob/ShindaKyodaiMember/verb)
			for(var/mob/X in world)
				if(X.ShindaKyodaiMember)
					X<<"<font color=#deb887>[M] has been Exiled from Shinda Kyodai!</font>"


mob/ShindaKyodaiMember
	verb/ClanSay(msg as text)
		set category="Clan"
		set name="Clan Say"
		set desc = "Say something to every member of Shinda Kyodai"
		var/list/L = list("<")
		for(var/H in L)
			if(findtext(msg,H))
				alert(usr,"No HTML in text!");return
		...
		for(var/mob/M in world)
			if(M.ShindaKyodaiMember)
				M<<"<font face=trebuchet MS><font color=green size=2>(Shinda Kyodai Say)</font><font size=1> - [usr.name]</font><font face=trebuchet MS><font color=#deb887>: [msg]</font>"
	verb/ClanWho()
		set category="Clan"
		set name="Semideus Who"
		usr<<"<font color=green>Online Shinda Kyodai Members -"
		for(var/mob/M in world)
			if(M.ShindaKyodaiMember&&M.client){usr<<"<font color = #deb887>[M.oname]</font>"}
