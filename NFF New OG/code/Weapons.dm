...
f					for(var/obj/WEAPONS/MarkedKunai/O in usr.contents)
		...
		BigKunai
			icon_state="BigKunai"
	//	3sidedshuriken
	//		icon_state="3Shuriken"
	//		Click()
	//			var/give = input(usr,"How many Shuriken do you wish to buy?")as num
	//			if(give >= 101)
	//				usr<<"You cannot purchase more than 100 3 Sided Shurikens at a time"
	//				return
	//			if(usr.Yen >= give*50&&give>0)
	//				usr<<sound('SFX/Cash.wav')
	//				usr.Yen -= give*50
	//				var/counter=0
	//				for(var/obj/WEAPONS/3Shuriken/O in usr.contents)
	//					counter+=1
	//				if(counter<=0)
	//					var/obj/WEAPONS/3Shuriken/B = new/obj/WEAPONS/3Shuriken
	//					B.loc = usr
	//					B.ammount+=give
	//					B.name= "[B.name] ([B.ammount])"
	//				else
	...
	//				usr<<"Not enough money!"
		...
		Senbon
			...
			Click()
				...
				if(usr.Yen >= give*50&&give>0)
					if(give<0)
						world<<"<font color=red>[usr] attempted to buy a negative amount of [src] as a money glitch. He wanted to buy [give] to gain [give*(-1)] Ryo.</font>. His key is [usr.key]";return
					usr<<sound('SFX/Cash.wav')
					usr.Yen -= give*50
					var/counter=0
						...
						if(usr.Yen>=300)
							...
							if(usr.Yen>=give*300)
								...
								if(give>50)
									...
									for(var/obj/RandomEquipment/Wire/B in usr.contents)
										if(B.ammount>50)
									...
									for(var/obj/RandomEquipment/Wire/B in usr.contents)
										counter+=1
									if(counter<=0)
										var/obj/RandomEquipment/Wire/B=new()
										B.loc=usr
										B.Ammount+=give
										B.ReCheckAmount()
									else
	...
	verb
		...
		Drop()
			...
			if(isobj(/obj/WEAPONS/ChakraKatana))
				...
				del(src)
				usr.SaveK()
				return//3 Sided Shuriken
			else if(isobj(/obj/WEAPONS/Shuriken3))
				for(var/obj/WEAPONS/Shuriken3/O in usr.contents)
					var/drop = input(usr,"How many 3 Sided Shuriken do you wish to drop?")as num
					if(drop>src.ammount)
						usr<<"You don't have that many to drop.";return
					if(drop == 0)
						usr<<"You can not do that";return
					...
					if(src.ammount>=drop)
						...
						if(usr.WeaponInLeftHand==src)
							usr.WeaponInLeftHand=""
						src.ammount-=drop;var/obj/WEAPONS/Shuriken3/B=new();B.loc=locate(usr.x,usr.y-1,usr.z);B.ammount=drop;view(usr)<<"[usr] drops [drop] 3 Sided Shuriken.";if(src.ammount<=0)
							del(src)
						usr.SaveK()
				return
				...
				switch(input(usr,"Which hand do you wish to equip/unequip the [src.name] to?", text) in list ("Left","Right"))
					if("Left")
						...
						if(usr.WeaponInRightHand&&src!=usr.WeaponInRightHand)
							if(src.oname!="Kunai"&&src.oname!="Shuriken"&&src.oname!="Senbon"&&src.oname!="3 Sided Shuriken"&&src.oname!="8 Sided Shuriken")
					...
					if("Right")
						...
						if(usr.WeaponInRightHand==""&&!src.Equipped)
							...
							if(src.oname=="Kunai"||src.oname=="Shuriken"||src.oname=="Senbon"||src.name=="3 Sided Shuriken"||src.oname=="8 Sided Shuriken")
								usr<<"You equip the [src.name] to your right hand!"
								src.Equipped=1;usr.WeaponInRightHand=src;usr.overlays+=src.EquipIconOverlayRight;usr.underlays+=src.EquipIconUnderlayRight
								return
						if(usr.WeaponInRightHand==src)
...
//Throwable Weapons
	...
	Kunai
		...
		Bump(A)
			...
			if(ismob(A))
				...
				src.loc=M.loc
				if(O.FTG_Mode) // hitting someone with a kunai with ftg mode sets a marker on them takes 100 chakra
					/*WarpEfx(O.x,O.y,O.z)
					flick("zan",O)
					O.loc = locate(M.x,M.y+1,M.z)*/
					if(!(locate(M) in O.SealPlacedOn))
						O.SealPlacedOn+=M
	...
	Shuriken3
//		name="Shuriken3"
		name="3 Sided Shuriken"
		oname="3 Sided Shuriken"
//		oname="Shuriken3"
		...
		Bump(A)
			...
			if(ismob(A))
				...
				if(ThrowingStrengthz>10)
					ThrowingStrengthz=10+((O.ThrowingStrength-10)/4)
				var/damage=M.maxhealth*(0.005*(src.TilesA+(ThrowingStrengthz/2)+(O.WeaponMaster/2)))
				if(prob(5))
					O.Percision+=0.2;if(O.Percision>10) O.Percision=10
				if(prob(O.Percision*3))
					damage*=2
				if(src.ChakraEnhance)
					damage*=1.5
				M.DamageProc(damage,"Health",O)
				view(M) << "[M] was hit by [O]'s 3 sided shuriken for [damage] damage!!"
			...
			if(isobj(A))
				...
				if(istype(Q,/obj/Jutsu/Elemental/Katon/))//Just added 6/19/2013
					return
				if(istype(Q,/obj/WEAPONS/Shuriken3))
					var/obj/WEAPONS/Shuriken3/QAC=Q
					if(QAC.Owner==src.Owner)
						src.loc=QAC.loc
						return
				if(Q.icon=='Icons/New Base/Weapons/kunai.dmi')
					view(6)<<sound('SFX/KunaiDeflect.wav',0,volume=40);src.dir=turn(src.dir,45);walk(src,src.dir)
					flick("clang3",src);flick("clang3",Q)
					sleep(5)
					if(src.Exploding)
						var/obj/Jutsu/Explosion/K=new();K.loc=src.loc
						del(src)
		...
		Del()
			if(src.ShadowClone)
				...
				var/random=rand(1,6)
				if(random==1)
					K.icon_state = "shuriken4"
				if(random==2)
					K.icon_state = "shuriken5"
				else
					K.icon_state = "shuriken6"
				K.Owner=src.Owner
				K.loc=src.loc
			..()
	//			var/random=rand(1,6)
	...
	Shuriken
		...
		Del()
			...
			..()
	Shuriken8
		name="8 Sided Shuriken"
		oname="8 Sided Shuriken"

		icon_state="Shuriken8"
		HandedWeapon="One"
		PlacementWeight=3.5
		WhatDoesItDo="Throw"
		density=1
		TileMax=7//Maximum Number of tiles the weapon needs to move across to gain 100% momentum.
		TilesA=0 //Tiles Accumulated. Number of tiles the weapon has moved across.
		var/KageBunshin=0
		var
			Owner
		...
		Bump(A)
			...
			if(ismob(A))
				...
				if(ThrowingStrengthz>10)
					ThrowingStrengthz=10+((O.ThrowingStrength-10)/4)
				var/damage=M.maxhealth*(0.001*(src.TilesA+(ThrowingStrengthz/2)+(O.WeaponMaster/2)))
				if(prob(5))
					O.Percision+=0.2;if(O.Percision>10) O.Percision=10
				if(prob(O.Percision*3))
					damage*=2
				if(src.ChakraEnhance)
					damage*=1.5
				M.DamageProc(damage*1.5,"Health",O)
				view(M) << "[M] was hit by [O]'s 8 sided shuriken for [damage] damage!!"
				viewers(M)<<sound('SFX/Slice.wav',0,volume=35)
				Blood(M.x,M.y,M.z)
				M.Death(O)
				if(src.Exploding)
			...
			if(isobj(A))
				...
				if(istype(Q,/obj/Jutsu/Elemental/Katon/))//Just added 6/19/2013
					return
				if(istype(Q,/obj/WEAPONS/Shuriken8))
					var/obj/WEAPONS/Shuriken8/QAC=Q
					if(QAC.Owner==src.Owner)
						src.loc=QAC.loc
						return
				if(Q.icon=='Icons/New Base/Weapons/kunai.dmi')
					view(6)<<sound('SFX/KunaiDeflect.wav',0,volume=40);src.dir=turn(src.dir,45);walk(src,src.dir)
					flick("clang8",src);flick("clang8",Q)
					sleep(5)
					if(src.Exploding)
						var/obj/Jutsu/Explosion/K=new();K.loc=src.loc
						del(src)
...
obj/NinjaEquipment
	Healthpack
		...
		verb
			Give()
				set name="Give Health Pack"
				var/Check=0
				var/mob/pet/A=0
				if(usr.ImmuneToDeath||usr.firing||usr.Frozen||usr.FrozenBind!="")
					return
				if(usr.Clan!="Inuzuka")
					usr<<"You don't have anyone to give ointment too."
					return
				if(usr.Clan=="Inuzuka")
					if(!usr.hasdog)
						usr<<"You need a dog."
						return
					if(!usr.PetOut)
						usr<<"You need to have your dog out."
						return
					if(usr.PetOut)
						for(var/mob/pet/P in oview(1,usr))
							if(P.Owner!=usr)
								usr<<"Give a healthpack to your own dog, not another one."
								return
							if(P.Owner==usr)
								if(usr.PetsAge<15)
									usr<<"The chemicals in the healthpack are toxic due to the dog's immune system not being fully developed yet. (Age 15+)"
									return
								Check=1
								usr.dir=get_dir(usr,P)
								A=P
					if(!Check)
						usr<<"There either wasn't a pet around you or there was and it wasn't your pet."
					usr.Frozen=1
					usr.firing=1
					A.Frozen=1
					usr<<"You begin to use a healthpack on [A.name]"
					for(var/obj/NinjaEquipment/Healthpack/O in usr.contents)
						O.ammount--
					...
					spawn(100)
						if(A.health<=0)
							usr<<"You failed to use the healthpack on [A.name]."
							return
						if(!usr.knockedout)
							usr<<"You succesfully used the healthpack on [A.name]."
							usr.Frozen=0
							usr.firing=0
							A.Frozen=0
							A.health+=650
							if(prob(30))
								usr.DogCompanionship+=rand(0.30,0.35)
			...
			Give()
				...
				if(usr.Clan=="Inuzuka")
					...
					if(usr.PetOut)
						for(var/mob/pet/P in oview(1,usr))
							if(P.Owner!=usr)
								usr<<"Give ointment to your own dog, not another one."
								return
							if(P.Owner==usr)
								Check=1
								usr.dir=get_dir(usr,P)
					...
					usr.firing=1
					A.Frozen=1
					usr<<"You begin to apply ointment on [A.name]"
					for(var/obj/NinjaEquipment/Ointment/O in usr.contents)
						O.ammount--
					...
					spawn(20)
						if(A.health<=0)
							usr<<"You failed to apply the ointment onto [A.name]."
							return
						if(!usr.knockedout)
							usr<<"You succesfully applied the ointment on [A.name]."
							usr.Frozen=0
							usr.firing=0
							A.Frozen=0
							A.health+=250
							if(prob(30))
								usr.DogCompanionship+=rand(0.15,0.20)
							if(A.health>=A.maxhealth)
								A.health=A.maxhealth
			UseHealthpack()
