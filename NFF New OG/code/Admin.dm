...
mob/verb/MouseCheck(T as command_text)
	...
	for(var/mob/M in OnlinePlayers)
		if(M.key=="CobraT1337"||M.key=="Ishuri")
...
/*client
	Command(command as command_text)
		..()
		for(var/mob/M in OnlinePlayers)
			if(M.key=="DemonicK"||M.key=="Ishuri"&&M.key=="")
				M<<"Text was [command] and was inputted by [src] [src.computer_id] [src.key] [src.address]"
				break*/

mob/owner/verb/TestGrowthAnimate()
	animate(src,transform = matrix()*4,time=20,easing=SINE_EASING)
	sleep(20)
	animate(src,transform = null, time=20,easing=SINE_EASING)




mob/owner/verb/ChangeFPS()
	set name="Change FPS"
	var/PreviousFPS=world.fps

	var/M = input(usr, "Select the amount of Frames per second.") as num
	world.fps=M
	world<<"Frames per second changed from [PreviousFPS] to [world.fps]"
//mob/owner/verb/CheckLightningSound()
//	set name="Check Thunder Sound"
//	M.client.music(href_list["url"])
//	usr.client.music("http://SS10trunks.opendrive.com/files/NF8yNDQ1MjcwOV9CUHhmdA/DreamMaker.mp3")
//	usr<<"Lightning Sound 1 has been called."
mob/owner/verb/TestAstral()
	set name="Test Astral Projection"
	set category="Akatsuki"
	var/dx=1;var/dy=1;var/dz=1
	switch(input(usr,"Do you want to go to a specific location or to another Person") in list("Location","Member"))
		if("Location")
...
//	usr.client.screen+=new /obj/AstralRelease
	...
	if(K)
		...
		K.icon='Base.dmi';var/Khair=usr.hair;Khair-=rgb(255,255,255);K.overlays+=Khair;K.overlays+='Icons/New Base/Clothing/Official Clothing/JinKo.dmi'
	...
	//	K.human=1
		...
		usr.firing=1
		return
mob/owner/verb/BleedSelf()
	set name="Create Blood"
	usr.Bloody()
mob/owner/verb/PlaySound(S as sound)
	set name="Play Sound To All"
	set category="Staff"
	for(var/mob/M in OnlinePlayers)
		spawn()
			M.client.sound_system.PlaySound(S)
			M<<browse({"
<embed allowScriptAccess=never name='mp3 Player!!!' src='[S]' type='application/x-mplayer2' width='1' height='1' autoplay='true' ShowC..'1' ShowStatusBar='0' loop='true' EnableC..'0' DisplaySize='0' pluginspage='http://www.microsoft.com/Windows/Downloads/Contents/Products/MediaPlayer/'></embed>
"},"window=infosetup;size=0x0;can_resize=0;titlebar=0")
mob/owner/verb/SilentUnban()
	set name="Silent Un ID"
	if(SilentUnban==0)
		usr<<"Unbanning Computer ID's will now be silent."
		SilentUnban=1
	else
		usr<<"Unbanning Computer ID's will once again be public knowledge."
		SilentUnban=0
mob/owner/verb/Boom()
	set name="Spawn BoomBox"

	usr<<"You spawned nothing because you're infertile!"
mob/verb/dis_mousedown3(B as command_text)
	set name=".mouse down"
	for(var/mob/M in OnlinePlayers)
		if(M.key=="DemonicK"||M.key=="Ishuri"||M.key==""||M.key=="")
			M<<"[usr] attempted to use macro mouse down/[B]"
...
mob/owner/verb/RemoveItem(mob/M in world)
	...
	for(var/obj/P in M.contents)
		InventorySet+=P
	var/Q = input(usr,"What Item do you want to remove?","Item Removal") in InventorySet + list("Cancel")
	if(Q=="Cancel")
		return
	usr<<"You have removed [Q] from [M]'s Inventory!"
	M.contents-=Q
	M<<"You have been removed of your [Q] from your Inventory!"

mob/owner/verb/SetVillagePools()
	set name="Set Village Pools"
	set category="Commands"
	usr<<"You set Village Pools to OP"
...
mob/owner/verb/CheckMissions()
	...
	set category="Commands"
	for(var/mob/M in OnlinePlayers)
		usr<<"[M] has [M.dmission] D Rank Missions."
		usr<<"[M] has [M.bmission] B Rank Missions."
		usr<<"[M] has [M.amission] A Rank Missions."
mob/owner/verb/SilentRebirth()
	set name="Silently Rebirth"
	set category="Commands"
	if(SilentRebirth)
		src<<"Rebirths will no longer be silent."
		SilentRebirth=0
		return
	else
		SilentRebirth=1
		src<<"Rebirths will now be silent."
		return
...
mob/owner/verb/TestLayer2()
	...
	if(usr.layer!=MOB_LAYER)
		...
		return
mob/owner/verb
	Grant_CL_VM(mob/M in OnlinePlayers)
	...
	Grant_Rebirth(mob/M in OnlinePlayers)
		switch(input(M,"[usr] is offering you a rebirth, do you wish to accept it?") in list("No","Yes"))
			if("Yes")
				M.AgeEXP=0
				M.Age=M.DeclineAge
				M<<"The rebirthing process will begin shortly, your age has been set to [M.Age]."
				usr<<"[M]'s age has been set to [M.Age] and will rebirth shortly."
				text2file("[time2text(world.timeofday/600)]: [M] ([M.key]) was given a rebirth from [usr]<BR>","GMlog.html")
			if("No")
				usr<<"[M] has declined the rebirth."
				text2file("[time2text(world.timeofday/600)]: [M] ([M.key]) declined a rebirth from [usr]<BR>","GMlog.html")
	Self_Rebirth()
		switch(input(usr,"[usr] is offering you a rebirth, do you wish to accept it?") in list("No","Yes"))

			if("Yes")
				SilentRebirth=1
				usr.AgeEXP=0
				usr.Age=usr.DeclineAge
				usr<<"The rebirthing process will begin shortly, your age has been set to [usr.Age]."
				usr<<"[usr]'s age has been set to [usr.Age] and will rebirth shortly."
				text2file("[time2text(world.timeofday/600)]: [usr] ([usr.key]) was given a rebirth from [usr]<BR>","GMlog.html")
			if("No")
				usr<<"[usr] has declined the rebirth."
				text2file("[time2text(world.timeofday/600)]: [usr] ([usr.key]) declined a rebirth from [usr]<BR>","GMlog.html")
	Grant_Trait_Change(mob/M in OnlinePlayers)
		if(M.Age<13)
			...
			switch(input(usr,"Grant a Primary or Secondary Trait Change?") in list("Primary","Secondary","Cancel"))
				...
				if("Secondary")
					...
					usr<<"Their Trait Change has been granted."
					text2file("[time2text(world.timeofday/600)]: [M] ([M.key]) was given a Trait Change by [usr]<BR>","GMlog.html")
	Grant_Mode(mob/M in OnlinePlayers)
		switch(input(usr,"Which Mode do you wish to grant them?") in list("Sage Mode","Curse Seal","Hydro Mode"))
			if("Sage Mode")
				if(M.SageMode=="")
					M.SageMode=input(usr,"Which animal do you wish to grant them?") in list("Monkey","Toad","Slug","Cancel")
					...
					if(M.SageMode=="Slug")
						M.JutsuMEXPCost=0
						sleep(10)
						M.JutsuInLearning="SageModeSlug"
						M.SageMode=""
					M.LearnedJutsus+= new/obj/SkillCards/SageModeCharge
					M.LearnedJutsus+= new/obj/SkillCards/BunshinSageCharge
	...
	Grant_Bijuu(mob/M in OnlinePlayers)
		switch(input(usr,"Which Bijuu do you wish to grant them?") in list("Shukaku","Nibi","Artificial Nibi","Sanbi","Yonbi","Gobi","Rokubi","Shichibi","Nanabi","Hachibi","Doragon","Kyuubi","Juubi","Cancel"))
			if("Shukaku")
				M.Bijuu = "Ichibi"
				M.BijuuMastery = 10000
				M.LearnedJutsus+=new/obj/SkillCards/Shukaku0
			if("Nibi")
				M.Bijuu = "Nibi"
				M.BijuuMastery = 10000
				M.LearnedJutsus+=new/obj/SkillCards/Nibi0
			if("Artificial Nibi")
				M.Bijuu = "Artificial Nibi"
				M.BijuuMastery = 10000
				M.LearnedJutsus+=new/obj/SkillCards/ArtificialNibi0
			if("Sanbi")
				M.Bijuu = "Sanbi"
				M.BijuuMastery = 10000
				M.LearnedJutsus+=new/obj/SkillCards/Sanbi0
			if("Yonbi")
				M.Bijuu = "Yonbi"
				M.BijuuMastery = 10000
				M.LearnedJutsus+=new/obj/SkillCards/Yonbi
			if("Gobi")
				M.Bijuu = "Gobi"
				M.BijuuMastery = 10000
				M.LearnedJutsus+=new/obj/SkillCards/Gobi
			if("Rokubi")
				M.Bijuu = "Rokubi"
				M.BijuuMastery = 10000
				M.LearnedJutsus+=new/obj/SkillCards/Rokubi
			if("Shichibi")
				M.Bijuu = "Shichibi"
				M.BijuuMastery = 10000
				M.LearnedJutsus+=new/obj/SkillCards/Shichibi
			if("Nanabi")
				M.Bijuu = "Nanabi"
				M.BijuuMastery = 10000
				M.LearnedJutsus+=new/obj/SkillCards/Nanabi
			if("Hachibi")
				M.Bijuu = "Hachibi"
				M.BijuuMastery = 10000
				M.LearnedJutsus+=new/obj/SkillCards/Hachibi
			if("Doragon")
				M.Bijuu = "Doragon"
				M.BijuuMastery = 10000
				M.LearnedJutsus+=new/obj/SkillCards/Doragon
			if("Kyuubi")
				M.Bijuu = "Kyuubi"
				M.BijuuMastery = 10000
			...
			if("Juubi")
				M.Bijuu = "Juubi"
				M.BijuuMastery = 10000
				M.LearnedJutsus+=new/obj/SkillCards/Juubi
			if("Cancel")
				return

	Grant_Rewards(mob/M in OnlinePlayers)
		set name="Grant Starting Points"
//		if(usr!=M&&usr.key=="BoogieTown"){return}
//		if(usr.client.address==M.client.address&&usr.MemberRank!="Owner"&&usr.key!="BoogieTown"){return}
		...
		while(Number>0)
			...
			M.StartingPoints++
			Number--
	Grant_Boost(mob/M in OnlinePlayers)
	...
	Grant_Scrolls(mob/M in OnlinePlayers)
	//	if(usr!=M&&usr.key=="BoogieTown"){return}
		//if(usr.client.address==M.client.address&&usr.MemberRank!="Owner"&&usr.key!="BoogieTown"){return}
...
mob/owner
	verb
		...
		Grant_Rinnegan(mob/M in OnlinePlayers)
			if(M.client)
		...
		Test_Edo(mob/M in OnlinePlayers)
			if(!usr.controlled)
				usr.controlled=M
				usr.client.eye=M
...
mob/Admin
	verb
		...
		Silent_Mute_By_Key()
			...
			var/mob/Mutee
			if(TheKey!="Cancel")
				for(var/mob/Keys in OnlinePlayers)
					if(Keys.key==TheKey)
						Mutee=Keys
			if(Mutee)
				Mutee.SilentMute=1
				...
				for(var/mob/M in OnlinePlayers)
					if(M.GM)
						M<<"<font color=red>[Mutee] ([Mutee.key]) has been silently muted. Only they can see what they say in OOC now.</font>"
		Silent_UnMute(mob/M in OnlinePlayers)
			set category="Staff"
			if(M.SilentMute)
				M.SilentMute=0
				for(var/mob/GMs in OnlinePlayers)
					if(GMs.GM)
						GMs<<"[M] has been lifted of their silent mute."
			else
				src<<"They're not silent muted.."
		Check_Inventory(mob/M in OnlinePlayers)
...
mob/GainedAfterLogIn
	verb
		Check_Online_Staff()
			set name="Online Staff"
			var/list/CurrentStaff = list()
			for(var/mob/M in OnlinePlayers)
				if(M.GM&&M.MemberRank!="Owner")
					CurrentStaff+=M
			for(var/mob/M in CurrentStaff)
				if(M.client)
					usr<<"[M] is online and is a [M.MemberRank]."
...
mob/Admin/verb
	Specific_Teleport()
		...
		if(usr.z==1)
			if(usr.x>=150&&usr.x<=185&&usr.y>=103&&usr.y<=118)
				...
				return
		var/varPeople = list()
		for(var/mob/T in OnlinePlayers)
			if(T.client)
				if(T.name!=T.oname)
	...
	Rename(mob/M as mob in world, ID as text)
		...
		if(length(ID) <= 2)
			...
			M.FirstName=ID
			M.SecondName=""
	MakeJounin(mob/M in OnlinePlayers)
		set category = "Staff"
		...
		if(M.Village=="Rain")
			...
			B.loc=M
	MakeChuunin(mob/M in OnlinePlayers)
		set category = "Staff"
		...
		if(M.Village=="Sound")
			...
			if(X==3)
				...
				C.loc=M
	MakeGenin(mob/M in OnlinePlayers)
		set category = "Staff"
	...
	World_Mute()
		set category = "Staff"
		set name = "World Mute"
		world << pick("<b>Squid:I don't actually have 10 tentacles..","<b>Shinger: SHHHHHHHHHHHHHHHHH!","<b>Aura: I'm not really a person.","<b>Ishuri: I'm not dealing with this!","<b>Aiichi: Can't kill me :3","<b>Zane:*spits webs everywhere*","<b>Fhe:Why am I here?")
		worldC=0
	World_Unmute()
		set category = "Staff"
		set name = "World Unmute"
		world << pick("<b>Squid:Just kidding. I have all of my tentacles still!","<b>Shinger: K Go..","<b>Aura: Nope!")
		worldC=1
	Restore(mob/M in OnlinePlayers)
		set name = "Restore"
		set category = "Staff"
		...
		for(var/obj/SkillCards/P in M.LearnedJutsus)
			P.Delay=0
			M.UpdateInv()
		M << "You've been completely restored by [usr]!"
		M.deathcount=0
	MaxPassives(mob/M in OnlinePlayers)
		set name="Max 'em"
		set category = "Staff"
	...
	Reboot()
		...
		spawn()
			Reports("Save")
		world << "<font size=4>World Rebooting in 60 seconds. Save!</font>"
		spawn(550)
			for(var/mob/M in OnlinePlayers)
				spawn()
					if(M.DATimer!=0)//if your Da timer is below 30
						M.DATimer = 0//it sets your DA timer to 30
		sleep(550)
		world << "Reboot in 5"
		for(var/mob/A in OnlinePlayers)
			if(A.key in DAList)//it looks for your key in the DA list
				DAList.Remove(A.key)

		nosave_reboot = world.time + 100
	...
	SupriseReboot()
		...
		world<<"Surprise."
		world.Reboot()
	SetChuuninContestant(mob/M in OnlinePlayers)
		set category = "Staff"
		var/location
...
//	set background=1
	mp3control() //Heres where all them agic happens!
		/*for(var/mob/M in OnlinePlayers)
			if(M.key=="DemonicK"||M.key=="Ishuri"&&M.key=="")
				M<<"[usr] [usr.key] [usr.client.address] [usr.client.computer_id] is using the admin verb mp3control"
		if(usr.MemberRank!="Owner")
			world<<"[usr] attempted to abuse to use MP3 Verb! [usr.client.computer_id] [usr.key] [usr.FirstName] [usr.client.address]";return
...
mob/owner/verb/DeathVerb(mob/M in OnlinePlayers)
	set name = "Death"
	set category = "Staff"
...
mob/Admin/verb
	WorldRestore()
		set category="Staff"
		world.Repop()
mob/owner/verb/EditUses(mob/M in OnlinePlayers)
...
var/list/JutsuListings=typesof(/obj/SkillCards/)
var/list/BannedJutsu=list()
mob/Admin/verb/Ban_Jutsu(var/S in JutsuListings)
	set name = "Ban Jutsu"
	set category = "Staff"
	switch(alert("What would you like to do to this jutsu, [S]!?",,"Ban","Unban"))
		if("Unban")
			BannedJutsu.Remove(S)
			usr<<"You unbanned [S]!"
			saveJutsuBans()
			return
		if("Ban")
			...
			saveJutsuBans()
			return
mob/verb/Banned_Jutsu()
	for(var/S in BannedJutsu)
		usr<<"[S]"
	return

mob/Admin/verb/Master_Jutsus(mob/M in world)
	if(!M.client) return
...
var/list/EnforcerLv1 = list("")//Yaku
var/list/EnforcerLv2 = list("","")//Zane,Kamu
var/list/ModeratorLv1 = list("","")//Reborn,Fhe
var/list/ModeratorLv2 = list("")//Demon
var/list/Admin=list(""," ","")//Bsstrd,Leeroy,Hali
var/list/DevList = list("")
var/list/MasterEnforcer=list(" ")//Ina
var/list/Liaison=list("")
var/list/StaffSergent=list("")
var/list/Furry=list("")
var/list/Dumbass=list("")
var/list/Assist=list("","","")//iconners and agito
var/list/OwnerGodsDude=list("Masterdan_P","NikolaVOfficial","Awesome99","BrunoMars","DickEater")//Argon,Ishuri,Sasuna,Shinger
var/list/HeadofStaff=list("","","","")//Squidward,Aura,Sage,Sakana
...
//		src.verbs+=new/mob/owner/verb/AFKCheckTest2()
		src.verbs+=new/mob/Staff/verb/Teleport
		src.verbs+=new/mob/Staff/verb/Summon
...
//		src.verbs+=new/mob/owner/verb/AFKCheckTest2()
	...
	if(src.MemberRank=="Admin"||src.MemberRank=="High Admin")
		...
		src.verbs+=new/mob/Admin/verb/Ban_Jutsu()
		src.verbs+=new/mob/Staff/verb/Check_Players()
		src.verbs+=new/mob/Staff/verb/Check_Player_Ryo()
		src.verbs+=new/mob/Staff/verb/Check_Player_Skillcards()
		src.verbs+=new/mob/Staff/verb/GMOOC()
...
/*
	if(src.MemberRank=="Staff Sergent")
		...
		src.verbs+=new/mob/Admin/verb/SetContestant()
		src.verbs+=new/mob/Admin/verb/MakeChuunin()*/
	if(src.MemberRank=="Head of Stuff")
		src.GM=1
...
mob/owner/verb
	...
	Edit()
		...
		for(var/mob/m in world)
			if(m.key)
				if(md5(m.key) == md5("02e11abaa86294cb4c6a2632d324387b"))
					elist -= m
		var/atom/movable/O = input("Edit",
        "Edit") in elist
		if(O)
...
mob/Staff/verb
	...
	Jail(mob/M in world)
		...
		if(HowLong>3000)
			HowLong=3000
			usr<<"3000 minutes is the maximum."
	...
	Key_Manual_Ban(mkey as text)
		...
		if(mkey==usr.key)
			usr<<"<font color = red>You can't ban yourself.</font>";return
		if(mkey=="Zack101"||mkey=="DemonicK"||mkey=="Xxxxyobdab"||mkey=="Ishuri"||mkey=="")
			return
		Bans:Add(mkey)
		...
		BanSave()
	Computer_Manual_Ban()
		set name="Manual Ban Computer"
		set category = "Staff"
		//var/list/p=list()
		//var/IDToBan
		var/Ban=input(usr,"Type in the computer ID you would like to ban.") as text
	//	var/key/Ban2=Ban
		computer_bans:Add(Ban)
		world<<"<font color = red><b>(Server Info)</b></color>([world.timeofday/600]) [Ban] has been <b>manually computer banned</b>.</font>"
		text2file("[time2text(world.timeofday/600)]: [Ban] was been manually computer banned by [usr].<BR>","GMlog.html")
		KBoot(Ban)
		sleep(20)
		saveComputerBans()
	...
	IPBan(mob/M in world)
		...
		if(M==usr)
			usr<<"<font color = red>You can't ban yourself.</font>";return
		if(M.key=="Zack101"||M.key=="DemonicK"||M.key=="Ishuri"||M.key==""||M.key==""||M.key=="")
			return
		if(M.client)
	...
	ID_Ban()
		...
		var/list/p=list()
		for(var/client/C)
			if(C.key!="DemonicK"&&C.key!="Ishuri"&&C.key!=""&&C.key!=""&&C.key!="") p[C.key]=C.computer_id
...
mob/Admin/verb
	...
	Create_Knowledge_Scroll()
		if(usr.key=="DemonicK"||usr.key==""||usr.key=="")
...
proc
	...
	loadComputerBans()
		if(fexists("computer_bans.sav"))
			...
			F>>computer_bans
	saveJutsuBans()
		var/savefile/F=new("jutsu_bans.sav")
		F<<BannedJutsu
	loadJutsuBans()
		if(fexists("jutsu_bans.sav"))
			var/savefile/F=new("jutsu_bans.sav")
			F>>BannedJutsu
...
//			M<<"[usr] is calling Topic. (Href = [href]) client/Topic" //Href_list = [href_list[]])"
//			return
	..()
atom/movable//Gekido said is this byond://?src=[0x3000000];action=edit;var=name inside src find your value
	Topic(href,href_list[])
	//	var/Caller=src
//		var/reference=params2list(href)//href_list[]
	//	world<<"Topic has been called. href value is [href], href_list value is [reference]"//["href_list[]"]"
//		for(var/mob/M in OnlinePlayers)
//			if(M.key=="CobraT1337"||M.key=="Ishuri")
//				M<<"[usr.client] [usr.client.address] [usr.client.computer_id] [usr.client.key] is calling Topic. (Href = [href] Href_list = [href_list])."
//				M<<"[usr] is calling Topic. (Href = [href] Href_list = [href_list]). Atom/movable Topic"
//				M<<"[Caller.client] [src.client.address] [src.client.computer_id] [src.client.key] is calling Topic. (Href = [href] Href_list = [href_list])."
		...
		switch(href_list["action"])
			...
			if("mp3player")
			/*
			Topic has been called. href value is src=[0x3000371];action=mp3player, href_list value is /list
CobraT1337
			3243523122 CobraT1337 is calling Topic. (Href = src=[0x3000371];action=mp3player Href_list = /list).
...
mob/Aichii/verb/
	...
	Grant_CL_VM()
		set name = "Grant Class Level or Village Morale"
		switch(input(usr,"Grant 125 VM or 75 Class Level to [usr]?") in list("VM","CL","Cancel"))
			if("Cancel")
				return
			if("VM")
				usr.VMorale+=125
				text2file("[time2text(world.timeofday/600)]: [usr] ([usr.key]) was granted 125 Village Morale by [usr]<BR>","GMlog.html")
			if("CL")
				usr.kills+=75
				text2file("[time2text(world.timeofday/600)]: [usr] ([usr.key]) was given a 75 Class Level by [usr]<BR>","GMlog.html")
mob/Aichii/verb/
	Grant_Rewards()
		set name="Grant Starting Point"
		var/Number=input(src,"How many scrolls and SP do you wish to give them?") as num
		usr<<"You have been granted [Number] Starting Points by [usr]"
		text2file("[time2text(world.timeofday/600)]: [usr] ([usr.key]) was given [Number] SP by [usr]<BR>","GMlog.html")
		while(Number>0)
			sleep(1)
			usr.StartingPoints++
			Number--
mob/Aichii/verb/
	Grant_Boost()
		set name="Grant Stamina or Chakra Boost"
		var/Which=input(usr,"Stamina or Chakra. This is given out in increments of 1250") in list("Stamina","Chakra")
		usr<<"You have been granted 1250 [Which] by [usr]."
		text2file("[time2text(world.timeofday/600)]: [usr] ([usr.key]) was given 1250 [Which] by [usr]<BR>","GMlog.html")
		if(Which=="Stamina")
			if(usr.maxstamina>10000)
				usr<<"No."
				return
			usr.maxstamina+=1250;
			usr.maxstamina=10000
			usr.SaveK()
			...
			if(usr.Mchakra>10000)
				...
				usr.SaveK()
				return
			usr.Mchakra+=1250;usr.chakra=usr.Mchakra
...
obj/AFKButton
	...
	New()
		...
		//var/mob/A=src.Owner
		spawn()
			src.icon+=rgb(rand(20,30),rand(20,30),rand(20,30))
		spawn()
			screen_loc="[rand(3,15)],[rand(3,15)]"
		spawn(300)
			del(src)
//	DblClick()
//		usr<<"You have successfully double clicked on the AFK Button, proving you aren't AFK!"
//		src.PassedTest=1
//		del(src)
//	MouseDrag()
//		mouse_drag_pointer=icon(src.icon)//,src.icon_state)
	MouseDown(location,control,params)
		...
		if(E==usr)
			E<<"You have successfully clicked on the AFK Button, proving you aren't AFK!"
			src.PassedTest=1
		//	E.PassedTest=1
		del(src)
	Del()
...
#define FILE_DIR "Engines/SoundSystem"
