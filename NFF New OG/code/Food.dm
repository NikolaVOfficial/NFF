...
obj/Food
	...
	verb
		...
		Give()
			...
			if(usr.Clan!="Inuzuka")
				usr<<"You don't have anyone to give this too."
				return
			if(usr.Clan=="Inuzuka")
				if(!usr.hasdog)
					...
					return
				if(!usr.PetOut)
					usr<<"Send out your dog so you can give it the food!"
					return
				for(var/obj/Food/P in usr.contents)
					FoodList+=P
				var/obj/Food/Q = input(usr,"What Item do you want to give to [usr.PetName]?","Feed Dog") in FoodList + list("Cancel")
				if(Q=="Cancel")
					return
				usr.contents-=Q
				//var/WaitTime=Q.FoodTimeInSeconds
				var/mob/pet/B
				for(var/mob/pet/A in view(20,usr))
					if(A.Owner==usr)
						A=B
						walk_towards(B,usr)
				var/Time=50
				B.Frozen=1
				usr<<"[usr.PetName] starts to chomp down [Q.name]!"
				spawn(Time)
					B.Frozen=0
					if(Q.name==usr.PetFavoriteFood)
						usr<<"[usr.PetName] absolutely loved the [Q.name]!"
						if(prob(50))
							usr.DogCompanionship+=rand(0.60,0.90)
					else
						usr<<"[usr.PetName] enjoyed the [Q.name]."
						if(prob(30))
							usr.DogCompanionship+=rand(0.35,0.65)
...
obj/Drinks
	...
	verb
		...
		Drink()
			...
			if(usr.Clan=="Satou"&&src.name=="Sake"&&usr.SatouDrunk)
				usr.SatouDrunkStack++
				if(usr.SatouDrunkStack<=1)
					usr<<"You drink another bottle of Sake..."
				else if(usr.SatouDrunkStack>=2&&usr.SatouDrunkStack<=3)
					usr<<"You drink another bottle of Sake..you're getting pretty wasted.."
				else if(usr.SatouDrunkStack>=4&&usr.SatouDrunkStack<5)
					if(prob(25))
						usr<<"Your body isn't able to handle this much.."
						usr.health-=1500
						usr.Death(usr)
						usr.SatouDrunkStack--
					usr<<"You're ridiculously drunk...your body shouldn't drink anymore.."
				else if(usr.SatouDrunkStack>=6)
					if(prob(40))
						usr<<"Your body isn't able to handle this much.."
						usr.health-=1500
						usr.Death(usr)
						usr.SatouDrunkStack--
					usr<<"Your body is in danger from alcohol.."
			...
			if(usr.Clan=="Satou"&&src.name=="Sake"&&!usr.SatouDrunk)
				if(prob(40))
					...
					spawn()
				//		world<<"Satou Drunk Boost proc about to be called."
						usr.SatouDrunkBoost()
					usr.SatouDrunk=1
...
mob/proc/SatouDrunkBoost()
	if(src.SatouDrunk==1)
		...
		var/Q=150//how long it lastes in seconds.
//		src.EnduranceAddOn+=100
		var/RS=src.RunningSpeed;
		var/PSS=src.reflexNew
		while(src.SatouDrunk==1&&Q!=0)//Stat boost WAS 1.3
	//		src<<"Satou drunk proc being called. Your drunk is [src.SatouDrunkStack]. Time Left [Q]"
	//		src<<"Stats: Nin is [src.nin]."
			src.nin=src.Mnin*(1.08+(src.SatouDrunkStack*0.12))
			src.tai=src.Mtai*(1.08+(src.SatouDrunkStack*0.12))
			src.gen=src.Mgen*(1.08+(src.SatouDrunkStack*0.12))
			if(src.SatouDrunkStack>=1&&src.SatouDrunkStack<2&&prob(5))
				var/Maxxxxx=src.LostBuzz+3//makes it so screen spin less with more drinking.
				var/randz=rand(1,Maxxxxx)
				if(randz>=4)
				...
				if(randz==1)
					src.client:dir=EAST
			else if(src.SatouDrunkStack>=2&&src.SatouDrunkStack<=3&&prob(20))
				var/Maxxxx1=src.LostBuzz+1//makes it so screen spin less with more drinking.
				var/randz=rand(1,Maxxxx1)
				if(randz>=4)
					src.client:dir=NORTH
				if(randz==3)
					src.client:dir=SOUTH
				...
				if(randz==1)
					src.client:dir=EAST
			else if(src.SatouDrunkStack>=4&&src.SatouDrunkStack<5&&prob(35))
				var/Maxxxx2=6//makes it so screen spin less with more drinking.
				var/randz=rand(1,Maxxxx2)
				if(randz>=4)
					src.client:dir=NORTH
				if(randz==3)
					src.client:dir=SOUTH
				...
				if(randz==1)
					src.client:dir=EAST
			else if(src.SatouDrunkStack>=5&&prob(10*src.SatouDrunkStack))
				var/Maxxxx3=4//makes it so screen spin less with more drinking.
				var/randz=rand(1,Maxxxx3)
				if(randz>=4)
					src.client:dir=NORTH
				if(randz==3)
					src.client:dir=SOUTH
			...
			if(src.Suiken==1)
				src.reflexNew=3
			sleep(9)
			if(src.SatouDrunkStack<=2)
				Q--
			else if(src.SatouDrunkStack>=3&&src.SatouDrunkStack<4&&prob(60))
				Q--
			else if(src.SatouDrunkStack>=4&&prob(50))
				Q--
		Q=120;
		src.client:dir=NORTH
		src<<"You lose your buzz...."
		src.SatouDrunkStack=0
		src.nin=src.Mnin
		src.tai=src.Mtai
		src.gen=src.Mgen
		src.EnduranceAddOn-=100
