...
//Challange system 1.1
	...
	if(M.acceptingChallenges == 0)
		...
		return
	usr.BetAmount = input(usr,"How much ninja cash do you want to bet?")
	if(usr.BetAmount>>usr.NinjaCash)
		usr<<"You don't have that much!"
		return
	if(usr.BetAmount>>M.NinjaCash)
		usr<<"They don't have that much!"
		return
	else
		global.challengeOpenWorld = 1
		usr.challenged = 0
		usr.challenge = M
		M.challenged = 1
		world << "<b><font size=3 color=green>[M] has been RANKED challenged by [usr] for [BetAmount] Ninja Cash! [M] has 45 seconds to respond!</font></b>"
		sleep(20)
		var/tmp/i
		for(i= 0, i<=9, i++)
			sleep(50)
		...
		if (global.challengeAccepted == 0)
			...
			while(global.challengeOpenWorld == 1)
				...
				if(M.Dead)
					spawn()
						usr.client.sound_system.MusicFade(usr.client.sound_system.music_volume, 0, 20, 20)
						M.client.sound_system.MusicFade(M.client.sound_system.music_volume, 0, 20, 20)
				/*
					...
					*/
					...
					usr.NinjaCash+=(usr.BetAmount*2)
					M.NinjaCash-=(usr.BetAmount*2)
					/*
...
mob/GainedAfterLogIn/verb/Accept()
	...
	//		return
		...
		//	if(WhichSong==1)
			spawn()
				var/sound/ArenaMusic=(pick('BeatAngel.mid','EndOfThought.mid','RaisingFightingSpirit.mid','StrongAndStrike.mid','FullForce.mid','FightingOfSpirit.mid','Fatalize.mid'))
			//	spawn()
				src.client.sound_system.PlayMusic(ArenaMusic)
				M.client.sound_system.PlayMusic(ArenaMusic)
	...
	if(Type=="Normal")
		...
		if(global.challengeAccepted == 0)
			...
			while(global.challengeOpenWorld == 1)
				...
				if(Challenged.Dead)
					spawn()
						Challenger.client.sound_system.MusicFade(Challenger.client.sound_system.music_volume, 0, 20, 20)
						Challenged.client.sound_system.MusicFade(Challenged.client.sound_system.music_volume, 0, 20, 20)
