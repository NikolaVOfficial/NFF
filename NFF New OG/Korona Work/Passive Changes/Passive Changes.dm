...
mob/proc/DodgeCalculation(mob/M,Involvement="Taijutsu")  //mob/M = Dodger. src = Attacker
	...
	if(M.CursedSealType=="Air"&&M.CursedSealActivated)
		M.dodgeRate+=30
...
mob/proc/Dodge()
	if(src.FrozenBind!=""||src.Frozen||src.resting||src.Stun>1)
		return
	if(src.Suiken)
		src<<"You clumsily dodge the technique!"
	else
		src<<"You dodge the technique!"
	if (src.Dodging==1)
		if(src.Clan=="Hyuuga"&&src.bya)
...
mob/proc/Dodge2()
	if(src.FrozenBind!=""||src.name=="Sand"&&!src.client)
		return
	if(src.Suiken)
		src<<"You clumsiliy dodge the technique!"
	else
		src<<"You dodge the technique!"
	flick("zan",src)
	src.Dodging=0
mob/proc/Quick2()
	if(src.Sliced||src.LegSliced||src.Frozen||src.FrozenBind!=""||src.Stun||src.knockedout||src.Status=="Asleep")
		return
	if(src.Shibari||src.hyoushou||src.caught||src.firing)
		return
	var/Destination = get_steps(src,src.dir,3)
	var/Route = getline(src,Destination)-src.loc
	for(var/turf/T in Route)
		if(locate(/obj/Doors) in T)
			return
		if(locate(/obj/Jutsu/Elemental/Mokuton/JukaiKoutan2)||(/obj/Jutsu/Elemental/Hyouton/DemonMirror) in T)
			return
		if(locate(/mob/IndigoRingBarrier)||(/mob/ChakraBarrier)||(/mob/ReflectionBarrier) in T)
			return
		if(T.density)
			return
